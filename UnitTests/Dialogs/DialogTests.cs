...
╚══└───────────────┘", output);
		...
		public void Dialog_Opened_From_Another_Dialog ()
		{
			...
			Application.Iteration += (s, a) => {
				iterations++;
				if (iterations == 0) {
					Assert.True (btn1.ProcessKey (new (Key.Enter, new KeyModifiers ())));
      ...
      └──────────────────────────────────────────────────────────────────┘";
					...
					Assert.True (btn2.ProcessKey (new (Key.Enter, new KeyModifiers ())));
      ...
      └──────────────────────────────────────────────────────────────────┘", output);
					...
					Assert.True (Application.Current.ProcessKey (new (Key.Enter, new KeyModifiers ())));
				} else if (iterations == 3) {
					TestHelpers.AssertDriverContentsWithFrameAre (expected, output);

					Assert.True (btn3.ProcessKey (new (Key.Enter, new KeyModifiers ())));
