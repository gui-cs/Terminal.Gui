...
public class KeyboardEventTests {
	...
	public void KeyPress_Handled_Cancels ()
	{
		...
		view.KeyDown += (s, e) => {
			...
			Assert.Equal (ConsoleDriverKey.N, e.Key);
		};
	...
	public void KeyDown_Handled_True_Stops_Processing ()
	{
		...
		view.KeyDown += (s, e) => {
			Assert.Equal (ConsoleDriverKey.A, e.Key);
	...
	public void Events_Are_Called_With_Only_Key_Modifiers (bool shift, bool alt, bool control)
	{
		...
		view.KeyDown += (s, e) => {
			Assert.Equal (ConsoleDriverKey.Null, e.Key & ~ConsoleDriverKey.CtrlMask & ~ConsoleDriverKey.AltMask & ~ConsoleDriverKey.ShiftMask);
