...
public class DimTests
{
        ...
        field.KeyDown += (s, k) =>
                         {
                             if (k.KeyCode == KeyCode.Enter)
                             {
                                 ...
                                 var label = new Label { X = 0, Y = view.ContentArea.Height, /*Width = 20,*/ Text = field.Text };
                                 view.Add (label);
    ...
    public void Only_DimAbsolute_And_DimFactor_As_A_Different_Procedure_For_Assigning_Value_To_Width_Or_Height ()
    {
        ...
        t.Ready += (s, e) =>
                   ...
                   #if DEBUG
                       Assert.Equal ($"Combine(View(Width,Button(v1){v1.Frame})-View(Width,Button(v3){v3.ContentArea}))", v5.Width.ToString ());
                    #else
                       Assert.Equal ($"Combine(View(Height,Button(){v1.Frame})-View(Height,Button(){v3.ContentArea}))", v5.Height.ToString ( ));
                   #endif
