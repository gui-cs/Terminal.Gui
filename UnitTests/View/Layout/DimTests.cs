	...
	public class LayoutTests_DimTests {
		...
		public void Dim_Add_Operator ()
		{
			...
			Application.Iteration += (s, a) => {
				while (count < 20) field.OnKeyDown (new (Key.Enter, new KeyModifiers ()));
...
└────────────────────┘",
		...
		public void Dim_Add_Operator_With_Text ()
		{
			...
			Application.Iteration += (s, a) => {
				while (count < 21) {
					field.OnKeyDown (new (Key.Enter, new KeyModifiers ()));
					if (count == 20) {
						field.OnKeyDown (new (Key.Enter, new KeyModifiers ()));
						break;
					}
		...
		public void Dim_Subtract_Operator ()
		{
			...
			Application.Iteration += (s, a) => {
				while (count > 0) field.OnKeyDown (new (Key.Enter, new KeyModifiers ()));
