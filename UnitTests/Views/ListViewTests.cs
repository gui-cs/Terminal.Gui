...
namespace Terminal.Gui.ViewsTests {
	public class ListViewTests {
		...
		/// the <see cref="View.OnKeyPress"/> returns the appropriate result
		/// </summary>
		[Fact]
		public void ListViewProcessKeyReturnValue_WithMultipleCommands ()
		{
			...
			Assert.True (lv.OnKeyPress (ev), "The first time we move down 2 it should be possible");
			...
			Assert.False (lv.OnKeyPress (ev), "We cannot move down so will not respond to this");
		}
		...
		public void KeyBindings_Command ()
		{
			...
			Assert.Equal (-1, lv.SelectedItem);
			Assert.True (lv.OnKeyPress (new (Key.CursorDown)));
			Assert.Equal (0, lv.SelectedItem);
			Assert.True (lv.OnKeyPress (new (Key.CursorUp)));
			Assert.Equal (0, lv.SelectedItem);
			Assert.True (lv.OnKeyPress (new (Key.PageDown)));
			Assert.Equal (2, lv.SelectedItem);
			Assert.Equal (2, lv.TopItem);
			Assert.True (lv.OnKeyPress (new (Key.PageUp)));
			Assert.Equal (0, lv.SelectedItem);
			Assert.Equal (0, lv.TopItem);
			Assert.False (lv.Source.IsMarked (lv.SelectedItem));
			Assert.True (lv.OnKeyPress (new (Key.Space)));
			Assert.True (lv.Source.IsMarked (lv.SelectedItem));
			var opened = false;
			lv.OpenSelectedItem += (s, _) => opened = true;
			Assert.True (lv.OnKeyPress (new (Key.Enter)));
			Assert.True (opened);
			Assert.True (lv.OnKeyPress (new (Key.End)));
			Assert.Equal (2, lv.SelectedItem);
			Assert.True (lv.OnKeyPress (new (Key.Home)));
			Assert.Equal (0, lv.SelectedItem);
		}
