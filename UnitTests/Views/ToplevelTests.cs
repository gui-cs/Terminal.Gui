...
namespace Terminal.Gui.ViewsTests {
	public class ToplevelTests {
		...
		public void KeyBindings_Command ()
		{
			...
			Assert.True (Application.OnKeyPress (new (Application.QuitKey)));
			Assert.False (isRunning);
			Assert.True (Application.OnKeyPress (new (Key.Z | Key.CtrlMask)));
			Assert.False (Application.OnKeyPress (new (Key.F5)));

			Assert.True (Application.OnKeyPress (new (Key.Tab)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tvW1, top.MostFocused);
			Assert.True (Application.OnKeyPress (new (Key.Tab)));
			Assert.Equal ($"\tFirst line Win1{Environment.NewLine}Second line Win1", tvW1.Text);
			Assert.True (Application.OnKeyPress (new (Key.Tab | Key.ShiftMask)));
			Assert.Equal ($"First line Win1{Environment.NewLine}Second line Win1", tvW1.Text);
			Assert.True (Application.OnKeyPress (new (Key.Tab | Key.CtrlMask)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf2W1, top.MostFocused);
			Assert.True (Application.OnKeyPress (new (Key.Tab)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf1W1, top.MostFocused);
			Assert.True (Application.OnKeyPress (new (Key.CursorRight)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf1W1, top.MostFocused);
			Assert.True (Application.OnKeyPress (new (Key.CursorDown)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tvW1, top.MostFocused);
			Assert.True (Application.OnKeyPress (new (Key.I | Key.CtrlMask)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf2W1, top.MostFocused);
			Assert.True (Application.OnKeyPress (new (Key.Tab | Key.ShiftMask)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tvW1, top.MostFocused);
			Assert.True (Application.OnKeyPress (new (Key.CursorLeft)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf1W1, top.MostFocused);
			Assert.True (Application.OnKeyPress (new (Key.CursorUp)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf2W1, top.MostFocused);
			Assert.True (Application.OnKeyPress (new (Key.Tab | Key.CtrlMask)));
			Assert.Equal (win2, top.Focused);
			Assert.Equal (tf1W2, top.MostFocused);
			Assert.True (Application.OnKeyPress (new (Key.Tab | Key.CtrlMask | Key.ShiftMask)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf2W1, top.MostFocused);
			Assert.True (Application.OnKeyPress (new (Application.AlternateForwardKey)));
			Assert.Equal (win2, top.Focused);
			Assert.Equal (tf1W2, top.MostFocused);
			Assert.True (Application.OnKeyPress (new (Application.AlternateBackwardKey)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf2W1, top.MostFocused);
			Assert.True (Application.OnKeyPress (new (Key.CursorUp)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tvW1, top.MostFocused);
			Assert.True (Application.OnKeyPress (new (Key.B | Key.CtrlMask)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf1W1, top.MostFocused);
			Assert.True (Application.OnKeyPress (new (Key.CursorDown)));
			Assert.Equal (win1, top.Focused);
			...
			Assert.True (Application.OnKeyPress (new (Key.End | Key.CtrlMask)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tvW1, top.MostFocused);
			Assert.Equal (new Point (16, 1), tvW1.CursorPosition);
			Assert.True (Application.OnKeyPress (new (Key.F | Key.CtrlMask)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf2W1, top.MostFocused);

			Assert.True (Application.OnKeyPress (new (Key.L | Key.CtrlMask)));
		}
		...
		public void KeyBindings_Command_With_OverlappedTop ()
		{
			...
			win1.Running = true;
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Application.QuitKey)));
			Assert.False (isRunning);
			Assert.False (win1.Running);
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.Z | Key.CtrlMask)));
			Assert.False (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.F5)));

			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.Tab)));
			Assert.True (win1.IsCurrentTop);
			Assert.Equal (tvW1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.Tab)));
			Assert.Equal ($"\tFirst line Win1{Environment.NewLine}Second line Win1", tvW1.Text);
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.Tab | Key.ShiftMask)));
			Assert.Equal ($"First line Win1{Environment.NewLine}Second line Win1", tvW1.Text);
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.Tab | Key.CtrlMask)));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf2W1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.Tab)));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf1W1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.CursorRight)));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf1W1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.CursorDown)));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tvW1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.I | Key.CtrlMask)));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf2W1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.Tab | Key.ShiftMask)));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tvW1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.CursorLeft)));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf1W1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.CursorUp)));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf2W1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.Tab)));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			...
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.Tab | Key.CtrlMask | Key.ShiftMask)));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf1W1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Application.AlternateForwardKey)));
			Assert.Equal (win2, Application.OverlappedChildren [0]);
			Assert.Equal (tf2W2, win2.MostFocused);
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Application.AlternateBackwardKey)));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf1W1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.CursorDown)));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tvW1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.B | Key.CtrlMask)));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf1W1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.CursorDown)));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tvW1, win1.MostFocused);
			Assert.Equal (new Point (0, 0), tvW1.CursorPosition);
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.End | Key.CtrlMask)));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tvW1, win1.MostFocused);
			Assert.Equal (new Point (16, 1), tvW1.CursorPosition);
			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.F | Key.CtrlMask)));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf2W1, win1.MostFocused);

			Assert.True (Application.OverlappedChildren [0].ProcessKeyPressEvent (new (Key.L | Key.CtrlMask)));
		}
...
└──────────────────┘", output);
		...
		public void Activating_MenuBar_By_Alt_Key_Does_Not_Throw ()
		{
			...
			var exception = Record.Exception (() => topChild.ProcessKeyPressEvent (new (Key.AltMask)));
			Assert.Null (exception);
		}
