...
namespace Terminal.Gui.ViewsTests {
	public class ToplevelTests {
		...
		public void KeyBindings_Command ()
		{
			...
			Assert.True (Application.OnKeyPressed (new (Key.Z | Key.CtrlMask)));
			Assert.False (Application.OnKeyPressed (new (Key.F5)));

			Assert.True (Application.OnKeyPressed (new (Key.Tab)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tvW1, top.MostFocused);
			Assert.True (Application.OnKeyPressed (new (Key.Tab)));
			Assert.Equal ($"\tFirst line Win1{Environment.NewLine}Second line Win1", tvW1.Text);
			Assert.True (Application.OnKeyPressed (new (Key.Tab | Key.ShiftMask)));
			Assert.Equal ($"First line Win1{Environment.NewLine}Second line Win1", tvW1.Text);
			Assert.True (Application.OnKeyPressed (new (Key.Tab | Key.CtrlMask)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf2W1, top.MostFocused);
			Assert.True (Application.OnKeyPressed (new (Key.Tab)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf1W1, top.MostFocused);
			Assert.True (Application.OnKeyPressed (new (Key.CursorRight)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf1W1, top.MostFocused);
			Assert.True (Application.OnKeyPressed (new (Key.CursorDown)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tvW1, top.MostFocused);
			Assert.True (Application.OnKeyPressed (new (Key.I | Key.CtrlMask)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf2W1, top.MostFocused);
			Assert.True (Application.OnKeyPressed (new (Key.Tab | Key.ShiftMask)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tvW1, top.MostFocused);
			Assert.True (Application.OnKeyPressed (new (Key.CursorLeft)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf1W1, top.MostFocused);
			Assert.True (Application.OnKeyPressed (new (Key.CursorUp)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf2W1, top.MostFocused);
			Assert.True (Application.OnKeyPressed (new (Key.Tab | Key.CtrlMask)));
			Assert.Equal (win2, top.Focused);
			Assert.Equal (tf1W2, top.MostFocused);
			Assert.True (Application.OnKeyPressed (new (Key.Tab | Key.CtrlMask | Key.ShiftMask)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf2W1, top.MostFocused);
			...
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tvW1, top.MostFocused);
			Assert.True (Application.OnKeyPressed (new (Key.B | Key.CtrlMask)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf1W1, top.MostFocused);
			Assert.True (Application.OnKeyPressed (new (Key.CursorDown)));
			Assert.Equal (win1, top.Focused);
			...
			Assert.True (Application.OnKeyPressed (new (Key.End | Key.CtrlMask)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tvW1, top.MostFocused);
			Assert.Equal (new Point (16, 1), tvW1.CursorPosition);
			Assert.True (Application.OnKeyPressed (new (Key.F | Key.CtrlMask)));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf2W1, top.MostFocused);

			Assert.True (Application.OnKeyPressed (new (Key.L | Key.CtrlMask)));
		}
