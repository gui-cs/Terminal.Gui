...
namespace Terminal.Gui.ViewsTests {
	public class ToplevelTests {
		...
		public void Application_Top_GetLocationThatFits_To_Driver_Rows_And_Cols ()
		{
			...
			Application.Iteration += (s, a) => {
				if (iterations == 0) {
					...
					Application.Top.OnHotKeyPressed (new (Key.CtrlMask | Key.R, new KeyModifiers ()));
					...
					Application.Top.OnHotKeyPressed (new (Key.CtrlMask | Key.C, new KeyModifiers ()));
					...
					Application.Top.OnHotKeyPressed (new (Key.CtrlMask | Key.Q, new KeyModifiers ()));
				}
				iterations++;
			};
		...
		public void KeyBindings_Command ()
		{
			...
			Assert.True (top.Focused.OnKeyPressed (new (Application.QuitKey, new KeyModifiers ())));
			Assert.False (isRunning);
			Assert.True (top.Focused.OnKeyPressed (new (Key.Z | Key.CtrlMask, new KeyModifiers ())));
			Assert.False (top.Focused.OnKeyPressed (new (Key.F5, new KeyModifiers ())));

			Assert.True (top.Focused.OnKeyPressed (new (Key.Tab, new KeyModifiers ())));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tvW1, top.MostFocused);
			Assert.True (top.Focused.OnKeyPressed (new (Key.Tab, new KeyModifiers ())));
			Assert.Equal ($"\tFirst line Win1{Environment.NewLine}Second line Win1", tvW1.Text);
			Assert.True (top.Focused.OnKeyPressed (new (Key.BackTab | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal ($"First line Win1{Environment.NewLine}Second line Win1", tvW1.Text);
			Assert.True (top.Focused.OnKeyPressed (new (Key.Tab | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf2W1, top.MostFocused);
			Assert.True (top.Focused.OnKeyPressed (new (Key.Tab, new KeyModifiers ())));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf1W1, top.MostFocused);
			Assert.True (top.Focused.OnKeyPressed (new (Key.CursorRight, new KeyModifiers ())));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf1W1, top.MostFocused);
			Assert.True (top.Focused.OnKeyPressed (new (Key.CursorDown, new KeyModifiers ())));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tvW1, top.MostFocused);
			Assert.True (top.Focused.OnKeyPressed (new (Key.I | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf2W1, top.MostFocused);
			Assert.True (top.Focused.OnKeyPressed (new (Key.BackTab | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tvW1, top.MostFocused);
			Assert.True (top.Focused.OnKeyPressed (new (Key.CursorLeft, new KeyModifiers ())));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf1W1, top.MostFocused);
			Assert.True (top.Focused.OnKeyPressed (new (Key.CursorUp, new KeyModifiers ())));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf2W1, top.MostFocused);
			Assert.True (top.Focused.OnKeyPressed (new (Key.Tab | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (win2, top.Focused);
			Assert.Equal (tf1W2, top.MostFocused);
			Assert.True (top.Focused.OnKeyPressed (new (Key.Tab | Key.CtrlMask | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf2W1, top.MostFocused);
			Assert.True (top.Focused.OnKeyPressed (new (Application.AlternateForwardKey, new KeyModifiers ())));
			Assert.Equal (win2, top.Focused);
			Assert.Equal (tf1W2, top.MostFocused);
			Assert.True (top.Focused.OnKeyPressed (new (Application.AlternateBackwardKey, new KeyModifiers ())));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf2W1, top.MostFocused);
			Assert.True (top.Focused.OnKeyPressed (new (Key.CursorUp, new KeyModifiers ())));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tvW1, top.MostFocused);
			Assert.True (top.Focused.OnKeyPressed (new (Key.B | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf1W1, top.MostFocused);
			Assert.True (top.Focused.OnKeyPressed (new (Key.CursorDown, new KeyModifiers ())));
			Assert.Equal (win1, top.Focused);
			...
			Assert.True (top.Focused.OnKeyPressed (new (Key.End | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tvW1, top.MostFocused);
			Assert.Equal (new Point (16, 1), tvW1.CursorPosition);
			Assert.True (top.Focused.OnKeyPressed (new (Key.F | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (win1, top.Focused);
			Assert.Equal (tf2W1, top.MostFocused);

			Assert.True (top.Focused.OnKeyPressed (new (Key.L | Key.CtrlMask, new KeyModifiers ())));
		}
		...
		public void KeyBindings_Command_With_OverlappedTop ()
		{
			...
			win1.Running = true;
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Application.QuitKey, new KeyModifiers ())));
			Assert.False (isRunning);
			Assert.False (win1.Running);
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.Z | Key.CtrlMask, new KeyModifiers ())));
			Assert.False (Application.OverlappedChildren [0].OnKeyPressed (new (Key.F5, new KeyModifiers ())));

			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.Tab, new KeyModifiers ())));
			Assert.True (win1.IsCurrentTop);
			Assert.Equal (tvW1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.Tab, new KeyModifiers ())));
			Assert.Equal ($"\tFirst line Win1{Environment.NewLine}Second line Win1", tvW1.Text);
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.BackTab | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal ($"First line Win1{Environment.NewLine}Second line Win1", tvW1.Text);
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.Tab | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf2W1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.Tab, new KeyModifiers ())));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf1W1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.CursorRight, new KeyModifiers ())));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf1W1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.CursorDown, new KeyModifiers ())));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tvW1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.I | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf2W1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.BackTab | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tvW1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.CursorLeft, new KeyModifiers ())));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf1W1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.CursorUp, new KeyModifiers ())));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf2W1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.Tab, new KeyModifiers ())));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			...
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.Tab | Key.CtrlMask | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf1W1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Application.AlternateForwardKey, new KeyModifiers ())));
			Assert.Equal (win2, Application.OverlappedChildren [0]);
			Assert.Equal (tf2W2, win2.MostFocused);
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Application.AlternateBackwardKey, new KeyModifiers ())));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf1W1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.CursorDown, new KeyModifiers ())));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tvW1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.B | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf1W1, win1.MostFocused);
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.CursorDown, new KeyModifiers ())));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tvW1, win1.MostFocused);
			Assert.Equal (new Point (0, 0), tvW1.CursorPosition);
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.End | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tvW1, win1.MostFocused);
			Assert.Equal (new Point (16, 1), tvW1.CursorPosition);
			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.F | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (win1, Application.OverlappedChildren [0]);
			Assert.Equal (tf2W1, win1.MostFocused);

			Assert.True (Application.OverlappedChildren [0].OnKeyPressed (new (Key.L | Key.CtrlMask, new KeyModifiers ())));
		}
...
└──────────────────┘", output);
		...
		public void Activating_MenuBar_By_Alt_Key_Does_Not_Throw ()
		{
			...
			var exception = Record.Exception (() => topChild.OnHotKeyPressed (new (Key.AltMask, new KeyModifiers { Alt = true })));
			Assert.Null (exception);
		}
