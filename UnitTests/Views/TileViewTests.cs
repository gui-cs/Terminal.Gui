...
11111│22222
     │     ";
			...
			line.ProcessKey (new OldKeyEvent (Key.CursorRight, new KeyModifiers ()));
...
└────┴────┘";
		...
		{
			var tileView = Get11By3TileView (out var line);
			tileView.ProcessHotKey (new OldKeyEvent (tileView.ToggleResizable, new KeyModifiers ()));
...
111111◊2222
      │     ";
			...
			// and 2 to the left
			line.ProcessKey (new OldKeyEvent (Key.CursorLeft, new KeyModifiers ()));
			line.ProcessKey (new OldKeyEvent (Key.CursorLeft, new KeyModifiers ()));
			tileView.Draw ();
...
22222222222";
			...
			// Keyboard movement on splitter should have no effect if it is not focused
			line.ProcessKey (new OldKeyEvent (Key.CursorDown, new KeyModifiers ()));
			tileView.SetNeedsDisplay ();
			tileView.Draw ();
			TestHelpers.AssertDriverContentsAre (looksLike, output);
		}
...
─────◊─────";
			...
			// And 2 up
			line.ProcessKey (new OldKeyEvent (Key.CursorUp, new KeyModifiers ()));
			line.ProcessKey (new OldKeyEvent (Key.CursorUp, new KeyModifiers ()));
			tileView.SetNeedsDisplay ();
			tileView.Draw ();
			looksLike =
