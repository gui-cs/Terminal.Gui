...
namespace Terminal.Gui.ViewsTests {
	public class TextFieldTests {
		...
		public void SelectedStart_Greater_Than_CursorPosition_All_Selection_Is_Overwritten_On_Typing ()
		{
			...
			_textField.ProcessKeyPressed (new ((Key)0x75, new KeyModifiers ())); // u
		...
		public void WordForward_With_No_Selection ()
		{
			...
			while (_textField.CursorPosition < _textField.Text.Length) {
				_textField.ProcessKeyPressed (new (Key.CursorRight | Key.CtrlMask, new KeyModifiers ()));
				switch (iteration) {
				case 0:
					Assert.Equal (4, _textField.CursorPosition);
					Assert.Equal (-1, _textField.SelectedStart);
		...
		public void WordBackward_With_No_Selection ()
		{
			...
			while (_textField.CursorPosition > 0) {
				_textField.ProcessKeyPressed (new (Key.CursorLeft | Key.CtrlMask, new KeyModifiers ()));
				switch (iteration) {
				case 0:
		...
		public void WordBackward_With_Selection ()
		{
			...
			while (_textField.CursorPosition > 0) {
				_textField.ProcessKeyPressed (new (Key.CursorLeft | Key.CtrlMask | Key.ShiftMask, new KeyModifiers ()));
				switch (iteration) {
				case 0:
					Assert.Equal (31, _textField.CursorPosition);
					Assert.Equal (32, _textField.SelectedStart);
		...
		public void Used_Is_True_By_Default ()
		{
			...
			_textField.ProcessKeyPressed (new ((Key)0x73, new KeyModifiers ())); // s
			Assert.Equal ("TAB to jumusp between text fields.", _textField.Text);
			_textField.ProcessKeyPressed (new ((Key)0x65, new KeyModifiers ())); // e
			Assert.Equal ("TAB to jumusep between text fields.", _textField.Text);
			_textField.ProcessKeyPressed (new ((Key)0x64, new KeyModifiers ())); // d
		...
		public void ProcessKey_Backspace_From_End ()
		{
			...
			// now delete the C
			tf.ProcessKeyPressed (new (Key.Backspace, new KeyModifiers ()));
		...
		public void Cancel_TextChanging_ThenBackspace ()
		{
			...
			tf.EnsureFocus ();
			tf.ProcessKeyPressed (new (Key.A, new KeyModifiers ()));
			Assert.Equal ("A", tf.Text);

			// cancel the next keystroke
			tf.TextChanging += (s, e) => e.Cancel = e.NewText == "AB";
			tf.ProcessKeyPressed (new (Key.B, new KeyModifiers ()));
		...
		public void KeyBindings_Command ()
		{
			...
			Assert.True (tf.ProcessKeyPressed (new (Key.DeleteChar, new KeyModifiers ())));
			Assert.Equal ("This is a test.", tf.Text);
			tf.CursorPosition = 0;
			Assert.True (tf.ProcessKeyPressed (new (Key.DeleteChar, new KeyModifiers ())));
			Assert.Equal ("his is a test.", tf.Text);
			tf.ReadOnly = true;
			Assert.True (tf.ProcessKeyPressed (new (Key.D | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("his is a test.", tf.Text);
			Assert.True (tf.ProcessKeyPressed (new (Key.Delete, new KeyModifiers ())));
			Assert.Equal ("his is a test.", tf.Text);
			tf.ReadOnly = false;
			tf.CursorPosition = 1;
			Assert.True (tf.ProcessKeyPressed (new (Key.Backspace, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			tf.CursorPosition = 5;
			Assert.True (tf.ProcessKeyPressed (new (Key.Home | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal ("is is", tf.SelectedText);
			tf.CursorPosition = 5;
			tf.SelectedStart = -1;
			Assert.Null (tf.SelectedText);
			Assert.True (tf.ProcessKeyPressed (new (Key.Home | Key.ShiftMask | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal ("is is", tf.SelectedText);
			tf.CursorPosition = 5;
			tf.SelectedStart = -1;
			Assert.Null (tf.SelectedText);
			Assert.True (tf.ProcessKeyPressed (new (Key.A | Key.ShiftMask | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal ("is is", tf.SelectedText);
			tf.CursorPosition = 5;
			tf.SelectedStart = -1;
			Assert.Null (tf.SelectedText);
			Assert.True (tf.ProcessKeyPressed (new (Key.End | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal (" a test.", tf.SelectedText);
			tf.CursorPosition = 5;
			tf.SelectedStart = -1;
			Assert.Null (tf.SelectedText);
			Assert.True (tf.ProcessKeyPressed (new (Key.End | Key.ShiftMask | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal (" a test.", tf.SelectedText);
			tf.CursorPosition = 5;
			tf.SelectedStart = -1;
			Assert.Null (tf.SelectedText);
			Assert.True (tf.ProcessKeyPressed (new (Key.E | Key.ShiftMask | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal (" a test.", tf.SelectedText);
			tf.CursorPosition = 5;
			tf.SelectedStart = -1;
			Assert.Null (tf.SelectedText);
			Assert.True (tf.ProcessKeyPressed (new (Key.Home, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal (0, tf.CursorPosition);
			tf.CursorPosition = 5;
			Assert.Null (tf.SelectedText);
			Assert.True (tf.ProcessKeyPressed (new (Key.Home | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal (0, tf.CursorPosition);
			tf.CursorPosition = 5;
			Assert.Null (tf.SelectedText);
			Assert.True (tf.ProcessKeyPressed (new (Key.A | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal (0, tf.CursorPosition);
			tf.CursorPosition = 5;
			tf.SelectedStart = -1;
			Assert.Null (tf.SelectedText);
			Assert.True (tf.ProcessKeyPressed (new (Key.CursorLeft | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal ("s", tf.SelectedText);
			Assert.True (tf.ProcessKeyPressed (new (Key.CursorUp | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal ("is", tf.SelectedText);
			Assert.True (tf.ProcessKeyPressed (new (Key.CursorRight | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal ("s", tf.SelectedText);
			Assert.True (tf.ProcessKeyPressed (new (Key.CursorDown | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Null (tf.SelectedText);
			tf.CursorPosition = 7;
			tf.SelectedStart = -1;
			Assert.Null (tf.SelectedText);
			Assert.True (tf.ProcessKeyPressed (new (Key.CursorLeft | Key.ShiftMask | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal ("a", tf.SelectedText);
			Assert.True (tf.ProcessKeyPressed (new (Key.CursorUp | Key.ShiftMask | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal ("is a", tf.SelectedText);
			Assert.True (tf.ProcessKeyPressed (new ((Key)((int)'B' + Key.ShiftMask | Key.AltMask), new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal ("is is a", tf.SelectedText);
			tf.CursorPosition = 3;
			tf.SelectedStart = -1;
			Assert.Null (tf.SelectedText);
			Assert.True (tf.ProcessKeyPressed (new (Key.CursorRight | Key.ShiftMask | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal ("is ", tf.SelectedText);
			Assert.True (tf.ProcessKeyPressed (new (Key.CursorDown | Key.ShiftMask | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal ("is a ", tf.SelectedText);
			Assert.True (tf.ProcessKeyPressed (new ((Key)((int)'F' + Key.ShiftMask | Key.AltMask), new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal ("is a test.", tf.SelectedText);
			Assert.Equal (13, tf.CursorPosition);
			Assert.True (tf.ProcessKeyPressed (new (Key.CursorLeft, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Null (tf.SelectedText);
			...
			Assert.True (tf.ProcessKeyPressed (new (Key.End, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal (13, tf.CursorPosition);
			tf.CursorPosition = 0;
			Assert.True (tf.ProcessKeyPressed (new (Key.End | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal (13, tf.CursorPosition);
			tf.CursorPosition = 0;
			Assert.True (tf.ProcessKeyPressed (new (Key.E | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal (13, tf.CursorPosition);
			tf.CursorPosition = 0;
			Assert.True (tf.ProcessKeyPressed (new (Key.CursorRight, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal (1, tf.CursorPosition);
			Assert.True (tf.ProcessKeyPressed (new (Key.F | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.Equal (2, tf.CursorPosition);
			tf.CursorPosition = 9;
			tf.ReadOnly = true;
			Assert.True (tf.ProcessKeyPressed (new (Key.K | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			tf.ReadOnly = false;
			Assert.True (tf.ProcessKeyPressed (new (Key.K | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a t", tf.Text);
			Assert.Equal ("est.", Clipboard.Contents);
			Assert.True (tf.ProcessKeyPressed (new (Key.Z | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.True (tf.ProcessKeyPressed (new (Key.Y | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a t", tf.Text);
			Assert.True (tf.ProcessKeyPressed (new (Key.Backspace | Key.AltMask, new KeyModifiers ())));
			Assert.Equal ("is is a test.", tf.Text);
			Assert.True (tf.ProcessKeyPressed (new (Key.Y | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a t", tf.Text);
			Assert.True (tf.ProcessKeyPressed (new (Key.CursorLeft | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a t", tf.Text);
			Assert.Equal (8, tf.CursorPosition);
			Assert.True (tf.ProcessKeyPressed (new (Key.CursorUp | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a t", tf.Text);
			Assert.Equal (6, tf.CursorPosition);
			Assert.True (tf.ProcessKeyPressed (new ((Key)((int)'B' + Key.AltMask), new KeyModifiers ())));
			Assert.Equal ("is is a t", tf.Text);
			Assert.Equal (3, tf.CursorPosition);
			Assert.True (tf.ProcessKeyPressed (new (Key.CursorRight | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a t", tf.Text);
			Assert.Equal (6, tf.CursorPosition);
			Assert.True (tf.ProcessKeyPressed (new (Key.CursorDown | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a t", tf.Text);
			Assert.Equal (8, tf.CursorPosition);
			Assert.True (tf.ProcessKeyPressed (new ((Key)((int)'F' + Key.AltMask), new KeyModifiers ())));
			Assert.Equal ("is is a t", tf.Text);
			Assert.Equal (9, tf.CursorPosition);
			Assert.True (tf.Used);
			Assert.True (tf.ProcessKeyPressed (new (Key.InsertChar, new KeyModifiers ())));
			Assert.Equal ("is is a t", tf.Text);
			Assert.Equal (9, tf.CursorPosition);
			...
			Assert.Equal ("is a", tf.SelectedText);
			Assert.Equal ("est.", Clipboard.Contents);
			Assert.True (tf.ProcessKeyPressed (new (Key.C | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a t", tf.Text);
			Assert.Equal ("is a", Clipboard.Contents);
			Assert.True (tf.ProcessKeyPressed (new (Key.X | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is  t", tf.Text);
			Assert.Equal ("is a", Clipboard.Contents);
			Assert.True (tf.ProcessKeyPressed (new (Key.V | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("is is a t", tf.Text);
			Assert.Equal ("is a", Clipboard.Contents);
			Assert.Equal (7, tf.CursorPosition);
			Assert.True (tf.ProcessKeyPressed (new (Key.K | Key.AltMask, new KeyModifiers ())));
			Assert.Equal (" t", tf.Text);
			Assert.Equal ("is is a", Clipboard.Contents);
			tf.Text = "TAB to jump between text fields.";
			Assert.Equal (0, tf.CursorPosition);
			Assert.True (tf.ProcessKeyPressed (new (Key.DeleteChar | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("to jump between text fields.", tf.Text);
			tf.CursorPosition = tf.Text.Length;
			Assert.True (tf.ProcessKeyPressed (new (Key.Backspace | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("to jump between text fields", tf.Text);
			Assert.True (tf.ProcessKeyPressed (new (Key.T | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ("to jump between text fields", tf.SelectedText);
			Assert.True (tf.ProcessKeyPressed (new (Key.D | Key.CtrlMask | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal ("", tf.Text);
		}
		...
		public void DeleteSelectedText_InsertText_DeleteCharLeft_DeleteCharRight_Cut ()
		{
			...
			Assert.True (tf.ProcessKeyPressed (new (Key.D2, new KeyModifiers ())));
		...
		public void HistoryText_IsDirty_ClearHistoryChanges ()
		{
			...
			Assert.True (tf.ProcessKeyPressed (new (Key.A, new KeyModifiers ())));
		...
		public void TestSetTextAndMoveCursorToEnd_WhenExistingSelection (string newText)
		{
			...
			tf.ProcessKeyPressed (new (Key.CursorLeft, new KeyModifiers ()));

			tf.ProcessKeyPressed (new (Key.CursorLeft | Key.ShiftMask, new KeyModifiers { Shift = true }));
			tf.ProcessKeyPressed (new (Key.CursorLeft | Key.ShiftMask, new KeyModifiers { Shift = true }));
...
ắ", output);
		...
		public void WordBackward_WordForward_Mixed ()
		{
			...
			tf.ProcessKeyPressed (new (Key.CtrlMask | Key.CursorLeft, new KeyModifiers () { Ctrl = true }));
			Assert.Equal (15, tf.CursorPosition);
			tf.ProcessKeyPressed (new (Key.CtrlMask | Key.CursorLeft, new KeyModifiers () { Ctrl = true }));
			Assert.Equal (12, tf.CursorPosition);
			tf.ProcessKeyPressed (new (Key.CtrlMask | Key.CursorLeft, new KeyModifiers () { Ctrl = true }));
			...
			tf.ProcessKeyPressed (new (Key.CtrlMask | Key.CursorRight, new KeyModifiers () { Ctrl = true }));
			Assert.Equal (5, tf.CursorPosition);
			tf.ProcessKeyPressed (new (Key.CtrlMask | Key.CursorRight, new KeyModifiers () { Ctrl = true }));
			Assert.Equal (10, tf.CursorPosition);
			tf.ProcessKeyPressed (new (Key.CtrlMask | Key.CursorRight, new KeyModifiers () { Ctrl = true }));
