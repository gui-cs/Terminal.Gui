...
namespace Terminal.Gui.ViewsTests {
	public class MenuTests {
		...
		new MenuBarItem ("_File", new MenuItem [] {
			...
			Assert.True (menu.ProcessHotKey (new KeyEventArgs (Key.F9, new KeyModifiers ())));
...
";
		...
		new MenuBarItem ("_File", new MenuItem [] {
			...
			Assert.True (mCurrent.OnKeyPressed (new KeyEventArgs (Key.CursorDown, new KeyModifiers ())));
			Assert.True (menu.IsMenuOpen);
			Assert.Equal ("_File", miCurrent.Parent.Title);
			Assert.Equal ("_Save", miCurrent.Title);

			Assert.True (mCurrent.OnKeyPressed (new KeyEventArgs (Key.CursorUp, new KeyModifiers ())));
			Assert.True (menu.IsMenuOpen);
			Assert.Equal ("_File", miCurrent.Parent.Title);
			Assert.Equal ("_New", miCurrent.Title);
		...
		new MenuBarItem ("_About", "Top-Level", () => miAction ="About")
			...
			Assert.True (menu.OnKeyPressed (new KeyEventArgs (Key.CursorLeft, new KeyModifiers ())));
			Assert.True (menu.IsMenuOpen);
			Assert.Equal ("_About", GetCurrentMenuBarItemTitle ());
			Assert.Equal ("_About", GetCurrentMenuTitle ());

			Assert.True (menu.OnKeyPressed (new KeyEventArgs (Key.CursorRight, new KeyModifiers ())));
			Assert.True (menu.IsMenuOpen);
			Assert.Equal ("_File", GetCurrentMenuBarItemTitle ());
			Assert.Equal ("_New", GetCurrentMenuTitle ());

			Assert.True (menu.OnKeyPressed (new KeyEventArgs (Key.Esc, new KeyModifiers ())));
			Assert.False (menu.IsMenuOpen);
			...
			Assert.Equal ("_File", GetCurrentMenuBarItemTitle ());
			Assert.Equal ("_New", GetCurrentMenuTitle ());
			Assert.True (menu.OnKeyPressed (new KeyEventArgs (Key.C | Key.CtrlMask, new KeyModifiers ())));
			Assert.False (menu.IsMenuOpen);
			Assert.Equal ("Closed", GetCurrentMenuBarItemTitle ());
			Assert.Equal ("None", GetCurrentMenuTitle ());

			Assert.True (menu.OnKeyPressed (new KeyEventArgs (Key.CursorDown, new KeyModifiers ())));
			Assert.True (menu.IsMenuOpen);
			Assert.Equal ("_File", GetCurrentMenuBarItemTitle ());
			Assert.Equal ("_New", GetCurrentMenuTitle ());
			Assert.True (menu.OnKeyPressed (new KeyEventArgs (Key.Esc, new KeyModifiers ())));
			...
			Assert.True (menu.OnKeyPressed (new KeyEventArgs (Key.Enter, new KeyModifiers ())));
			Assert.True (menu.IsMenuOpen);
			Assert.Equal ("_File", GetCurrentMenuBarItemTitle ());
			Assert.Equal ("_New", GetCurrentMenuTitle ());

			Assert.False (mCurrent.OnKeyPressed (new KeyEventArgs (Key.Tab, new KeyModifiers ())));
			Assert.True (menu.IsMenuOpen);
			Assert.True (Application.Top.OnKeyPressed (new KeyEventArgs (Key.Tab, new KeyModifiers ())));
			Assert.False (menu.IsMenuOpen);
			Assert.Equal ("Closed", GetCurrentMenuBarItemTitle ());
			Assert.Equal ("None", GetCurrentMenuTitle ());
			...
			Assert.True (mCurrent.OnKeyPressed (new KeyEventArgs (Key.CursorLeft, new KeyModifiers ())));
			Assert.True (menu.IsMenuOpen);
			Assert.Equal ("_About", GetCurrentMenuBarItemTitle ());
			Assert.Equal ("_About", GetCurrentMenuTitle ());

			Assert.True (mCurrent.OnKeyPressed (new KeyEventArgs (Key.CursorRight, new KeyModifiers ())));
			Assert.True (menu.IsMenuOpen);
			Assert.Equal ("_File", GetCurrentMenuBarItemTitle ());
			Assert.Equal ("_New", GetCurrentMenuTitle ());

			Assert.True (mCurrent.OnKeyPressed (new KeyEventArgs (Key.Esc, new KeyModifiers ())));
			Assert.False (menu.IsMenuOpen);
			Assert.Equal ("Closed", GetCurrentMenuBarItemTitle ());
			Assert.Equal ("None", GetCurrentMenuTitle ());
			...
			Assert.Equal ("_File", GetCurrentMenuBarItemTitle ());
			Assert.Equal ("_New", GetCurrentMenuTitle ());
			Assert.True (mCurrent.OnKeyPressed (new KeyEventArgs (Key.Enter, new KeyModifiers ())));
			Assert.False (menu.IsMenuOpen);
			Assert.Equal ("Closed", GetCurrentMenuBarItemTitle ());
			Assert.Equal ("None", GetCurrentMenuTitle ());
...
";
			...
			Assert.True (menu.ProcessHotKey (new KeyEventArgs (Key.F9, null)));
			Application.Top.Draw ();
			expected = @"
 Numbers  
┌────────┐
...
";
			...
			Assert.True (Application.Top.Subviews [1].OnKeyPressed (new KeyEventArgs (Key.CursorDown, null)));
			Application.Top.Draw ();
			expected = @"
 Numbers                 
┌────────┐               
...
";
			...
			Assert.True (Application.Top.Subviews [2].OnKeyPressed (new KeyEventArgs (Key.CursorLeft, null)));
			Application.Top.Draw ();
			expected = @"
 Numbers  
┌────────┐
...
";
			...
			Assert.True (Application.Top.Subviews [1].OnKeyPressed (new KeyEventArgs (Key.Esc, null)));
			Application.Top.Draw ();
			expected = @"
 Numbers
";
...
";
			...
			Assert.True (Application.Top.Subviews [1].OnKeyPressed (new KeyEventArgs (Key.CursorDown, null)));
			Assert.True (Application.Top.Subviews [1].OnKeyPressed (new KeyEventArgs (Key.Enter, null)));
			Application.Top.Draw ();
			expected = @"
 Numbers       
┌─────────────┐
...
";
			...
			Assert.True (Application.Top.Subviews [2].OnKeyPressed (new KeyEventArgs (Key.Enter, null)));
			Application.Top.Draw ();
			expected = @"
 Numbers  
┌────────┐
...
";
			...
			Assert.True (menu.OnKeyPressed (new (Key.N, null)));
			Application.MainLoop.RunIteration ();
...
";
			...
			Assert.True (menu.OnKeyPressed (new (Key.CursorRight, null)));
			Assert.True (menu.OnKeyPressed (new (Key.C, null)));
			Application.MainLoop.RunIteration ();
		...
		new MenuBarItem ("_" + expectedMenu.Menus[1].Title, new MenuItem [] {
			...
			Assert.True (Application.Top.Subviews [1].OnKeyPressed (new (Key.N, null)));
			Application.MainLoop.RunIteration ();
			Assert.True (newAction);
			...
			Assert.True (Application.Top.Subviews [1].OnKeyPressed (new (Key.C, null)));
			Application.MainLoop.RunIteration ();
			Assert.True (copyAction);
		}
		...
		new MenuBarItem (expectedMenu.Menus[1].Title, new MenuItem [] {
			...
			// Open second
			Assert.True (Application.Top.Subviews [1].OnKeyPressed (new (Key.CursorRight, null)));
			Assert.True (menu.IsMenuOpen);
			Application.Top.Draw ();
			TestHelpers.AssertDriverContentsAre (expectedMenu.expectedSubMenuOpen (1), output);
		...
		new MenuBarItem ("Format", new MenuItem [] {
			...
			Assert.True (menu.openMenu.OnKeyPressed (new KeyEventArgs (Key.CursorRight, new KeyModifiers ())));
			Assert.True (menu.IsMenuOpen);
			Assert.False (tf.HasFocus);
			Application.Top.Draw ();
			TestHelpers.AssertDriverContentsAre (expectedMenu.expectedSubMenuOpen (1), output);
			...
			// Left - Edit
			Assert.True (menu.openMenu.OnKeyPressed (new KeyEventArgs (Key.CursorLeft, new KeyModifiers ())));
			Assert.True (menu.IsMenuOpen);
			Assert.False (tf.HasFocus);
			Application.Top.Draw ();
			TestHelpers.AssertDriverContentsAre (expectedMenu.expectedSubMenuOpen (1), output);
		...
		new MenuBarItem ("File", new MenuItem [] {
			...
			Assert.False (menu.ProcessHotKey (new KeyEventArgs (Key.F9, new KeyModifiers ())));
			Assert.False (menu.IsMenuOpen);

			Assert.True (menu.ProcessHotKey (new KeyEventArgs (Key.F10 | Key.ShiftMask, new KeyModifiers ())));
			Assert.True (menu.IsMenuOpen);
			Assert.True (menu.ProcessHotKey (new KeyEventArgs (Key.F10 | Key.ShiftMask, new KeyModifiers ())));
			Assert.False (menu.IsMenuOpen);
		}
...
00000000000000", attributes);
		...
		public void MenuBar_With_Action_But_Without_MenuItems_Not_Throw ()
		{
			...
			Assert.False (Application.Top.OnKeyDown (new KeyEventArgs (Key.AltMask, new KeyModifiers { Alt = true })));
			Assert.True (menu.OnKeyPressed (new KeyEventArgs (Key.CursorRight, new KeyModifiers ())));
			Assert.True (menu.OnKeyPressed (new KeyEventArgs (Key.CursorRight, new KeyModifiers ())));
		}
...
└──────────────────────────────────────┘", output);
				...
				Assert.True (win.ProcessHotKey (new KeyEventArgs (Key.F9, new KeyModifiers ())));
				top.Draw ();
				TestHelpers.AssertDriverContentsWithFrameAre (@"
┌──────────────────────────────────────┐
│ File  Edit                           │
...
└──────────────────────────────────────┘", output);
				...
				Assert.True (top.ProcessHotKey (new KeyEventArgs (Key.F9, new KeyModifiers ())));
				top.Draw ();
				TestHelpers.AssertDriverContentsWithFrameAre (@"
┌──────────────────────────────────────┐
│ File  Edit                           │
...
└──────────────────────────────────────┘", output);
				...
				Assert.True (top.Subviews [0].OnKeyPressed (new KeyEventArgs (Key.CursorRight, new KeyModifiers ())));
				Application.Refresh ();
				TestHelpers.AssertDriverContentsWithFrameAre (@"
┌──────────────────────────────────────┐
│ File  Edit                           │
...
└──────────────────────────────────────┘", output);
				...
				Assert.True (((MenuBar)top.Subviews [0]).openMenu.OnKeyPressed (new KeyEventArgs (Key.CursorRight, new KeyModifiers ())));
				top.Draw ();
				TestHelpers.AssertDriverContentsWithFrameAre (@"
┌──────────────────────────────────────┐
│ File  Edit                           │
...
└──────────────────────────────────────┘", output);
		...
		public void AllowNullChecked_Get_Set ()
		{
			...
			Assert.True (menu.ProcessHotKey (new KeyEventArgs (Key.F9, new KeyModifiers ())));
			Assert.True (menu.openMenu.OnKeyPressed (new KeyEventArgs (Key.Enter, new KeyModifiers ())));
			Application.MainLoop.RunIteration ();
...
";
		...
		public void MenuBarItem_Children_Null_Does_Not_Throw ()
		{
			...
			var exception = Record.Exception (() => menu.ProcessColdKey (new KeyEventArgs (Key.Space, new KeyModifiers ())));
		...
		public void CanExecute_ProcessHotKey ()
		{
			...
			Assert.True (top.ProcessHotKey (new KeyEventArgs (Key.N | Key.AltMask, new KeyModifiers () { Alt = true })));
			Application.MainLoop.RunIteration ();
			Assert.NotNull (win);
		...
		public void Visible_False_Key_Does_Not_Open_And_Close_All_Opened_Menus ()
		{
			...
			Assert.True (menu.ProcessHotKey (new KeyEventArgs (menu.Key, new KeyModifiers ())));
			Assert.True (menu.IsMenuOpen);
