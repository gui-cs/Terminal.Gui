	...
	public class TextViewTests {
		...
		public void WordForward_With_No_Selection ()
		{
			...
			while (_textView.CursorPosition.X < _textView.Text.Length) {
				_textView.ProcessKey (new OldKeyEvent (Key.CursorRight | Key.CtrlMask, new KeyModifiers ()));
				switch (iteration) {
				case 0:
		...
		public void WordBackward_With_No_Selection ()
		{
			...
			while (_textView.CursorPosition.X > 0) {
				_textView.ProcessKey (new OldKeyEvent (Key.CursorLeft | Key.CtrlMask, new KeyModifiers ()));
				switch (iteration) {
				case 0:
		...
		public void WordForward_With_Selection ()
		{
			...
			while (_textView.CursorPosition.X < _textView.Text.Length) {
				_textView.ProcessKey (new OldKeyEvent (Key.CursorRight | Key.CtrlMask | Key.ShiftMask, new KeyModifiers ()));
				switch (iteration) {
				case 0:
					Assert.Equal (4, _textView.CursorPosition.X);
					Assert.Equal (0, _textView.CursorPosition.Y);
		...
		public void WordBackward_With_Selection ()
		{
			...
			while (_textView.CursorPosition.X > 0) {
				_textView.ProcessKey (new OldKeyEvent (Key.CursorLeft | Key.CtrlMask | Key.ShiftMask, new KeyModifiers ()));
				switch (iteration) {
				case 0:
					Assert.Equal (31, _textView.CursorPosition.X);
					Assert.Equal (0, _textView.CursorPosition.Y);
		...
		public void Kill_To_End_Delete_Forwards_Copy_To_The_Clipboard_And_Paste ()
		{
			...
			while (!iterationsFinished) {
				switch (iteration) {
				case 0:
					_textView.ProcessKey (new OldKeyEvent (Key.K | Key.CtrlMask, new KeyModifiers ()));
					Assert.Equal (0, _textView.CursorPosition.X);
					Assert.Equal (0, _textView.CursorPosition.Y);
					...
					break;
				case 1:
					_textView.ProcessKey (new OldKeyEvent (Key.DeleteChar | Key.CtrlMask | Key.ShiftMask, new KeyModifiers ()));
					Assert.Equal (0, _textView.CursorPosition.X);
					Assert.Equal (0, _textView.CursorPosition.Y);
				...
				case 2:
					...
					// Paste
					_textView.ProcessKey (new OldKeyEvent (Key.Y | Key.CtrlMask, new KeyModifiers ()));
		...
		public void Kill_To_Start_Delete_Backwards_Copy_To_The_Clipboard_And_Paste ()
		{
			...
			while (!iterationsFinished) {
				switch (iteration) {
				case 0:
					_textView.ProcessKey (new OldKeyEvent (Key.K | Key.AltMask, new KeyModifiers ()));
					Assert.Equal (0, _textView.CursorPosition.X);
					Assert.Equal (1, _textView.CursorPosition.Y);
					...
					break;
				case 1:
					_textView.ProcessKey (new OldKeyEvent (Key.Backspace | Key.CtrlMask | Key.ShiftMask, new KeyModifiers ()));
					Assert.Equal (23, _textView.CursorPosition.X);
					Assert.Equal (0, _textView.CursorPosition.Y);
		...
		public void Kill_Delete_WordForward ()
		{
			...
			while (!iterationsFinished) {
				_textView.ProcessKey (new OldKeyEvent (Key.DeleteChar | Key.CtrlMask, new KeyModifiers ()));
				switch (iteration) {
				case 0:
					Assert.Equal (0, _textView.CursorPosition.X);
					Assert.Equal (0, _textView.CursorPosition.Y);
		...
		public void Kill_Delete_WordBackward ()
		{
			...
			while (!iterationsFinished) {
				_textView.ProcessKey (new OldKeyEvent (Key.Backspace | Key.CtrlMask, new KeyModifiers ()));
				switch (iteration) {
				case 0:
					Assert.Equal (22, _textView.CursorPosition.X);
					Assert.Equal (0, _textView.CursorPosition.Y);
		...
		public void Copy_Or_Cut_Null_If_No_Selection ()
		{
			_textView.SelectionStartColumn = 0;
			_textView.SelectionStartRow = 0;
			_textView.ProcessKey (new OldKeyEvent (Key.C | Key.CtrlMask, new KeyModifiers ())); // Copy
			Assert.Equal ("", _textView.SelectedText);
			_textView.ProcessKey (new OldKeyEvent (Key.W | Key.CtrlMask, new KeyModifiers ())); // Cut
			Assert.Equal ("", _textView.SelectedText);
		}
		...
		public void Copy_Or_Cut_And_Paste_With_Selection ()
		{
			...
			_textView.ProcessKey (new OldKeyEvent (Key.Y | Key.CtrlMask, new KeyModifiers ())); // Paste
			Assert.Equal ("TAB to jump between text fields.", _textView.Text);
			_textView.SelectionStartColumn = 20;
			_textView.SelectionStartRow = 0;
			_textView.ProcessKey (new OldKeyEvent (Key.W | Key.CtrlMask, new KeyModifiers ())); // Cut
		...
		public void Cut_Not_Allowed_If_ReadOnly_Is_True ()
		{
			...
			_textView.ProcessKey (new OldKeyEvent (Key.C | Key.CtrlMask, new KeyModifiers ())); // Copy
			Assert.Equal ("text", _textView.SelectedText);
			_textView.ProcessKey (new OldKeyEvent (Key.W | Key.CtrlMask, new KeyModifiers ())); // Selecting is set to false after Cut.
			Assert.Equal ("", _textView.SelectedText);
		...
		public void TextChanged_Event_NoFires_OnTyping ()
		{
			...
			_textView.ProcessKey (new OldKeyEvent (Key.Y, new KeyModifiers ()));
			Assert.Equal (1, eventcount);
		...
		public void Used_Is_True_By_Default ()
		{
			_textView.CursorPosition = new Point (10, 0);
			Assert.Equal ("TAB to jump between text fields.", _textView.Text);
			_textView.ProcessKey (new OldKeyEvent ((Key)0x75, new KeyModifiers ())); // u
			Assert.Equal ("TAB to jumup between text fields.", _textView.Text);
			_textView.ProcessKey (new OldKeyEvent ((Key)0x73, new KeyModifiers ())); // s
			Assert.Equal ("TAB to jumusp between text fields.", _textView.Text);
			_textView.ProcessKey (new OldKeyEvent ((Key)0x65, new KeyModifiers ())); // e
			Assert.Equal ("TAB to jumusep between text fields.", _textView.Text);
			_textView.ProcessKey (new OldKeyEvent ((Key)0x64, new KeyModifiers ())); // d
		...
		public void TabWidth_Setting_To_Zero_Keeps_AllowsTab ()
		{
			...
			Assert.True (_textView.AllowsReturn);
			Assert.True (_textView.Multiline);
			_textView.ProcessKey (new OldKeyEvent (Key.Tab, new KeyModifiers ()));
			Assert.Equal ("\tTAB to jump between text fields.", _textView.Text);
			_textView.ProcessKey (new OldKeyEvent (Key.BackTab | Key.ShiftMask, new KeyModifiers ()));
			Assert.Equal ("TAB to jump between text fields.", _textView.Text);
		}
		...
		public void AllowsReturn_Setting_To_True_Changes_Multiline_To_True_If_It_Is_False ()
		{
			...
			Assert.Equal (4, _textView.TabWidth);
			Assert.True (_textView.AllowsTab);
			_textView.ProcessKey (new OldKeyEvent (Key.Enter, new KeyModifiers ()));
			Assert.Equal (Environment.NewLine +
		...
		public void Tab_Test_Follow_By_CursorLeft_And_Then_Follow_By_CursorRight ()
		{
			...
			Application.Iteration += (s, a) => {
				...
				while (col > 0) {
					col--;
					_textView.ProcessKey (new OldKeyEvent (Key.CursorLeft, new KeyModifiers ()));
					Assert.Equal (new Point (col, 0), _textView.CursorPosition);
					leftCol = GetLeftCol (leftCol);
					Assert.Equal (leftCol, _textView.LeftColumn);
				}
				while (col < 100) {
					col++;
					_textView.ProcessKey (new OldKeyEvent (Key.CursorRight, new KeyModifiers ()));
					Assert.Equal (new Point (col, 0), _textView.CursorPosition);
					leftCol = GetLeftCol (leftCol);
					Assert.Equal (leftCol, _textView.LeftColumn);
				}
		...
		public void Tab_Test_Follow_By_Home_And_Then_Follow_By_End_And_Then_Follow_By_BackTab_With_Text ()
		{
			...
			Application.Iteration += (s, a) => {
				...
				_textView.ProcessKey (new OldKeyEvent (Key.Home, new KeyModifiers ()));
				col = 0;
				Assert.Equal (new Point (col, 0), _textView.CursorPosition);
				leftCol = 0;
				Assert.Equal (leftCol, _textView.LeftColumn);

				_textView.ProcessKey (new OldKeyEvent (Key.End, new KeyModifiers ()));
...
", output);
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.Backspace, new KeyModifiers ())));
			Application.Refresh ();
			Assert.Equal (0, tv.LeftColumn);
			TestHelpers.AssertDriverContentsAre (@"
aaa
...
", output);
		...
		public void BottomOffset_Sets_To_Zero_Adjust_TopRow ()
		{
			...
			tv.ProcessKey (new OldKeyEvent (Key.CtrlMask | Key.End, new KeyModifiers ()));
		...
		public void RightOffset_Sets_To_Zero_Adjust_leftColumn ()
		{
			...
			tv.ProcessKey (new OldKeyEvent (Key.End, new KeyModifiers ()));
		...
		public void LeftColumn_Add_One_If_Text_Length_Is_Equal_To_Width ()
		{
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.CursorRight, new KeyModifiers ())));
		...
		public void KeyBindings_Command ()
		{
			...
			tv.CanFocus = false;
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.CursorLeft, new KeyModifiers ())));
			tv.CanFocus = true;
			Assert.False (tv.ProcessKey (new OldKeyEvent (Key.CursorLeft, new KeyModifiers ())));
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.CursorRight, new KeyModifiers ())));
			Assert.Equal (new Point (1, 0), tv.CursorPosition);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.End | Key.CtrlMask, new KeyModifiers ())));
			...
			Assert.False (tv.ProcessKey (new OldKeyEvent (Key.CursorRight, new KeyModifiers ())));
			Assert.NotNull (tv.Autocomplete);
			Assert.Empty (g.AllSuggestions);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.F, new KeyModifiers ())));
			tv.Draw ();
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.Z | Key.CtrlMask, new KeyModifiers ())));
			tv.Draw ();
			Assert.Equal ($"This is the first line.{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.", tv.Text);
			Assert.Equal (new Point (23, 2), tv.CursorPosition);
			Assert.Empty (tv.Autocomplete.Suggestions);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.R | Key.CtrlMask, new KeyModifiers ())));
			tv.Draw ();
			Assert.Equal ($"This is the first line.{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.F", tv.Text);
			Assert.Equal (new Point (24, 2), tv.CursorPosition);
			Assert.Empty (tv.Autocomplete.Suggestions);
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.Enter, new KeyModifiers ())));
			...
			Assert.Empty (g.AllSuggestions);
			Assert.Empty (tv.Autocomplete.Suggestions);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.PageUp, new KeyModifiers ())));
			Assert.Equal (24, tv.GetCurrentLine ().Count);
			Assert.Equal (new Point (24, 1), tv.CursorPosition);
			Assert.True (tv.ProcessKey (new OldKeyEvent (((int)'V' + Key.AltMask), new KeyModifiers ())));
			Assert.Equal (23, tv.GetCurrentLine ().Count);
			Assert.Equal (new Point (23, 0), tv.CursorPosition);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.PageDown, new KeyModifiers ())));
			Assert.Equal (24, tv.GetCurrentLine ().Count);
			Assert.Equal (new Point (23, 1), tv.CursorPosition); // gets the previous length
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.V | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (28, tv.GetCurrentLine ().Count);
			Assert.Equal (new Point (23, 2), tv.CursorPosition); // gets the previous length
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.PageUp | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal (24, tv.GetCurrentLine ().Count);
			Assert.Equal (new Point (23, 1), tv.CursorPosition); // gets the previous length
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.PageDown | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal (28, tv.GetCurrentLine ().Count);
			Assert.Equal (new Point (23, 2), tv.CursorPosition); // gets the previous length
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.Home | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (Point.Empty, tv.CursorPosition);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.N | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (new Point (0, 1), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.P | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.CursorDown, new KeyModifiers ())));
			Assert.Equal (new Point (0, 1), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.CursorUp, new KeyModifiers ())));
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.CursorDown | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal (new Point (0, 1), tv.CursorPosition);
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.CursorUp | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.F | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (new Point (1, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.B | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.CursorRight, new KeyModifiers ())));
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.CursorRight | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal (new Point (1, 0), tv.CursorPosition);
			Assert.Equal (1, tv.SelectedLength);
			Assert.Equal ("T", tv.SelectedText);
			Assert.True (tv.Selecting);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.CursorLeft | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.Selecting);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.DeleteChar, new KeyModifiers ())));
			Assert.Equal ($"This is the first line.{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			...
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.D | Key.CtrlMask, new KeyModifiers ())));
			...
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.End, new KeyModifiers ())));
			Assert.Equal ($"is is the first line.{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (21, 0), tv.CursorPosition);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.Delete, new KeyModifiers ())));
			Assert.Equal ($"is is the first line{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.Home, new KeyModifiers ())));
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.End | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal (new Point (19, 0), tv.CursorPosition);
			Assert.Equal (19, tv.SelectedLength);
			Assert.Equal ("is is the first lin", tv.SelectedText);
			Assert.True (tv.Selecting);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.Home | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.Selecting);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.E | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (new Point (19, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.A | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.K | Key.CtrlMask, new KeyModifiers ())));
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.Y | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ($"is is the first lin{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (19, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.DeleteChar | Key.CtrlMask | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			...
			Assert.False (tv.Selecting);
			Assert.Equal ("is is the first lin", Clipboard.Contents);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.K | Key.AltMask, new KeyModifiers ())));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.Space | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ($"is is the first lin{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (19, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			...
			Assert.Equal (0, tv.SelectionStartRow);
			tv.SelectionStartColumn = 0;
			Assert.True (tv.ProcessKey (new OldKeyEvent (((int)'C' + Key.AltMask), new KeyModifiers ())));
			Assert.Equal ($"is is the first lin{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (19, 0), tv.CursorPosition);
			Assert.Equal (19, tv.SelectedLength);
			Assert.Equal ("is is the first lin", tv.SelectedText);
			...
			Assert.Equal (0, tv.SelectionStartColumn);
			Assert.Equal (0, tv.SelectionStartRow);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.C | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ($"is is the first lin{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (19, 0), tv.CursorPosition);
			Assert.Equal (19, tv.SelectedLength);
			Assert.Equal ("is is the first lin", tv.SelectedText);
			...
			Assert.Equal (0, tv.SelectionStartColumn);
			Assert.Equal (0, tv.SelectionStartRow);
			Assert.True (tv.ProcessKey (new OldKeyEvent (((int)'W' + Key.AltMask), new KeyModifiers ())));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			...
			Assert.Equal (0, tv.SelectionStartRow);
			Assert.Equal ("is is the first lin", Clipboard.Contents);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.W | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.X | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			...
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.CtrlMask | Key.CursorLeft, new KeyModifiers ())));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (18, 2), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.CtrlMask | Key.CursorLeft | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (12, 2), tv.CursorPosition);
			Assert.Equal (6, tv.SelectedLength);
			Assert.Equal ("third ", tv.SelectedText);
			Assert.True (tv.Selecting);
			Assert.True (tv.ProcessKey (new OldKeyEvent ((Key)((int)'B' + Key.AltMask), new KeyModifiers ())));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (8, 2), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.CtrlMask | Key.CursorRight, new KeyModifiers ())));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (12, 2), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.CtrlMask | Key.CursorRight | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (18, 2), tv.CursorPosition);
			Assert.Equal (6, tv.SelectedLength);
			Assert.Equal ("third ", tv.SelectedText);
			Assert.True (tv.Selecting);
			Assert.True (tv.ProcessKey (new OldKeyEvent ((Key)((int)'F' + Key.AltMask), new KeyModifiers ())));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			...
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.DeleteChar | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ($"This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting); Assert.True (tv.ProcessKey (new OldKeyEvent (Key.End | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ($"This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (28, 1), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.False (tv.Selecting); Assert.True (tv.ProcessKey (new OldKeyEvent (Key.Backspace | Key.CtrlMask, new KeyModifiers ())));
			...
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.False (tv.Selecting);
			Assert.False (tv.ProcessKey (new OldKeyEvent (Key.Enter, new KeyModifiers ())));
			Assert.Equal ($"This is the second line.{Environment.NewLine}This is the third ", tv.Text);
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			...
			Assert.False (tv.Selecting);
			Assert.True (tv.AllowsReturn);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.CtrlMask | Key.End | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third ", tv.Text);
			Assert.Equal (new Point (18, 2), tv.CursorPosition);
			...
			Assert.True (tv.Selecting);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.CtrlMask | Key.Home | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third ", tv.Text);
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (Environment.NewLine.Length, tv.SelectedLength);
			Assert.Equal ($"{Environment.NewLine}", tv.SelectedText);
			Assert.True (tv.Selecting);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.T | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third ", tv.Text);
			Assert.Equal (new Point (18, 2), tv.CursorPosition);
			...
			Assert.True (tv.Selecting);
			Assert.True (tv.Used);
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.InsertChar, new KeyModifiers ())));
			Assert.False (tv.Used);
			Assert.True (tv.AllowsTab);
			Assert.Equal (new Point (18, 2), tv.CursorPosition);
			tv.AllowsTab = false;
			Assert.False (tv.ProcessKey (new OldKeyEvent (Key.Tab, new KeyModifiers ())));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third ", tv.Text);
			...
			Assert.True (tv.Selecting);
			tv.Selecting = false;
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.Tab, new KeyModifiers ())));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third \t", tv.Text);
			Assert.True (tv.AllowsTab);
			tv.AllowsTab = false;
			Assert.False (tv.ProcessKey (new OldKeyEvent (Key.BackTab | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third \t", tv.Text);
			Assert.False (tv.AllowsTab);
			tv.AllowsTab = true;
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.BackTab | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third ", tv.Text);
			Assert.True (tv.AllowsTab);
			Assert.False (tv.ProcessKey (new OldKeyEvent (Key.Tab | Key.CtrlMask, new KeyModifiers ())));
			Assert.False (tv.ProcessKey (new OldKeyEvent (Application.AlternateForwardKey, new KeyModifiers ())));
			Assert.False (tv.ProcessKey (new OldKeyEvent (Key.Tab | Key.CtrlMask | Key.ShiftMask, new KeyModifiers ())));
			Assert.False (tv.ProcessKey (new OldKeyEvent (Application.AlternateBackwardKey, new KeyModifiers ())));
		}
		...
		public void HistoryText_Undo_Redo_Multi_Line_With_Empty_Text ()
		{
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.O, new KeyModifiers ())));
			Assert.Equal ("O", tv.Text);
			Assert.Equal (1, tv.Lines);
			Assert.Equal (new Point (1, 0), tv.CursorPosition);
			Assert.True (tv.IsDirty);

			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.n, new KeyModifiers ())));
			Assert.Equal ("On", tv.Text);
			Assert.Equal (1, tv.Lines);
			Assert.Equal (new Point (2, 0), tv.CursorPosition);
			Assert.True (tv.IsDirty);

			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.e, new KeyModifiers ())));
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.T, new KeyModifiers ())));
			Assert.Equal ($"One{Environment.NewLine}T", tv.Text);
			Assert.Equal (2, tv.Lines);
			Assert.Equal (new Point (1, 1), tv.CursorPosition);
			Assert.True (tv.IsDirty);

			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.w, new KeyModifiers ())));
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.h, new KeyModifiers ())));
			Assert.Equal ($"One{Environment.NewLine}Two{Environment.NewLine}Th", tv.Text);
			Assert.Equal (3, tv.Lines);
			Assert.Equal (new Point (2, 2), tv.CursorPosition);
			Assert.True (tv.IsDirty);

			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.r, new KeyModifiers ())));
		...
		public void HistoryText_Undo_Redo_Multi_Line_Selected_With_Empty_Text ()
		{
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.D1, new KeyModifiers ())));
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.D2, new KeyModifiers ())));
		...
		public void HistoryText_Undo_Redo_Multi_Line_Selected_DeleteCharLeft_All ()
		{
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.End | Key.CtrlMask | Key.ShiftMask, new KeyModifiers ())));
			Assert.Equal ($"This is the first line.{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.", tv.Text);
		...
		public void HistoryText_Undo_Redo_KillWordBackward ()
		{
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.Backspace | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal ($"First line.{Environment.NewLine}Second line", tv.Text);
			Assert.Equal ("", tv.SelectedText);
			Assert.Equal (2, tv.Lines);
		...
		public void HistoryText_Undo_Redo_Changing_On_Middle_Clear_History_Forwards ()
		{
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.D3, new KeyModifiers ())));
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.D4, new KeyModifiers ())));
		...
		public void HistoryText_Undo_Redo_First_Line_Selected_Return_And_InsertText ()
		{
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.a, new KeyModifiers ())));
		...
		public void HistoryText_Undo_Redo_Multiline_Simples_Tab_BackTab ()
		{
			...
			Assert.True (tv.ProcessKey (new OldKeyEvent (Key.BackTab, new KeyModifiers ())));
			Assert.Equal ($"First line.{Environment.NewLine}Second line.{Environment.NewLine}Third line.", tv.Text);
			Assert.Equal (3, tv.Lines);
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
		...
		public void ScrollDownTillCaretOffscreen_ThenType ()
		{
			...
			tv.ProcessKey (new OldKeyEvent (Key.p, new KeyModifiers ()));
		}
...
└─────────────┘", output);
		...
		public void ContentsChanged_Event_Fires_On_Typing ()
		{
			...
			expectedCol = 1;
			tv.ProcessKey (new OldKeyEvent (Key.Y, new KeyModifiers ()));
		...
		public void ContentsChanged_Event_Fires_On_Undo_Redo ()
		{
			...
			expectedEventCount++;
			Assert.True (_textView.ProcessKey (new OldKeyEvent (Key.Enter, new KeyModifiers ())));
			Assert.Equal (expectedEventCount, eventcount);

			// Undo
			expectedEventCount++;
			Assert.True (_textView.ProcessKey (new OldKeyEvent (Key.Z | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (expectedEventCount, eventcount);

			// Redo
			expectedEventCount++;
			Assert.True (_textView.ProcessKey (new OldKeyEvent (Key.R | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (expectedEventCount, eventcount);
