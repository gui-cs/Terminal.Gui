	...
	public class TextViewTests {
		...
		public void WordForward_With_No_Selection ()
		{
			...
			while (_textView.CursorPosition.X < _textView.Text.Length) {
				_textView.ProcessKeyPressed (new (Key.CursorRight | Key.CtrlMask));
				switch (iteration) {
				case 0:
		...
		public void WordBackward_With_No_Selection ()
		{
			...
			while (_textView.CursorPosition.X > 0) {
				_textView.ProcessKeyPressed (new (Key.CursorLeft | Key.CtrlMask));
				switch (iteration) {
				case 0:
		...
		public void Selected_Text_Shows ()
		{
			...
			_textView.ProcessKeyPressed (new (Key.CursorRight | Key.CtrlMask | Key.ShiftMask));
		...
		public void WordBackward_With_Selection ()
		{
			...
			while (_textView.CursorPosition.X > 0) {
				_textView.ProcessKeyPressed (new (Key.CursorLeft | Key.CtrlMask | Key.ShiftMask));
				switch (iteration) {
				case 0:
					Assert.Equal (31, _textView.CursorPosition.X);
					Assert.Equal (0, _textView.CursorPosition.Y);
		...
		public void Kill_To_End_Delete_Forwards_Copy_To_The_Clipboard_And_Paste ()
		{
			...
			while (!iterationsFinished) {
				switch (iteration) {
				case 0:
					_textView.ProcessKeyPressed (new (Key.K | Key.CtrlMask));
					Assert.Equal (0, _textView.CursorPosition.X);
					Assert.Equal (0, _textView.CursorPosition.Y);
					...
					break;
				case 1:
					_textView.ProcessKeyPressed (new (Key.DeleteChar | Key.CtrlMask | Key.ShiftMask));
					Assert.Equal (0, _textView.CursorPosition.X);
					Assert.Equal (0, _textView.CursorPosition.Y);
				...
				case 2:
					...
					// Paste
					_textView.ProcessKeyPressed (new (Key.Y | Key.CtrlMask));
		...
		public void Kill_To_Start_Delete_Backwards_Copy_To_The_Clipboard_And_Paste ()
		{
			...
			while (!iterationsFinished) {
				switch (iteration) {
				case 0:
					_textView.ProcessKeyPressed (new (Key.K | Key.AltMask));
					Assert.Equal (0, _textView.CursorPosition.X);
					Assert.Equal (1, _textView.CursorPosition.Y);
					...
					break;
				case 1:
					_textView.ProcessKeyPressed (new (Key.Backspace | Key.CtrlMask | Key.ShiftMask));
					Assert.Equal (23, _textView.CursorPosition.X);
					Assert.Equal (0, _textView.CursorPosition.Y);
		...
		public void Kill_Delete_WordForward ()
		{
			...
			while (!iterationsFinished) {
				_textView.ProcessKeyPressed (new (Key.DeleteChar | Key.CtrlMask));
				switch (iteration) {
				case 0:
					Assert.Equal (0, _textView.CursorPosition.X);
					Assert.Equal (0, _textView.CursorPosition.Y);
		...
		public void Kill_Delete_WordBackward ()
		{
			...
			while (!iterationsFinished) {
				_textView.ProcessKeyPressed (new (Key.Backspace | Key.CtrlMask));
				switch (iteration) {
				case 0:
					Assert.Equal (22, _textView.CursorPosition.X);
					Assert.Equal (0, _textView.CursorPosition.Y);
		...
		public void Copy_Or_Cut_Null_If_No_Selection ()
		{
			_textView.SelectionStartColumn = 0;
			_textView.SelectionStartRow = 0;
			_textView.ProcessKeyPressed (new (Key.C | Key.CtrlMask)); // Copy
			Assert.Equal ("", _textView.SelectedText);
			_textView.ProcessKeyPressed (new (Key.W | Key.CtrlMask)); // Cut
			Assert.Equal ("", _textView.SelectedText);
		}
		...
		public void Copy_Or_Cut_And_Paste_With_Selection ()
		{
			...
			_textView.ProcessKeyPressed (new (Key.Y | Key.CtrlMask)); // Paste
			Assert.Equal ("TAB to jump between text fields.", _textView.Text);
			_textView.SelectionStartColumn = 20;
			_textView.SelectionStartRow = 0;
			_textView.ProcessKeyPressed (new (Key.W | Key.CtrlMask)); // Cut
		...
		public void Cut_Not_Allowed_If_ReadOnly_Is_True ()
		{
			...
			_textView.ProcessKeyPressed (new (Key.C | Key.CtrlMask)); // Copy
			Assert.Equal ("text", _textView.SelectedText);
			_textView.ProcessKeyPressed (new (Key.W | Key.CtrlMask)); // Selecting is set to false after Cut.
			Assert.Equal ("", _textView.SelectedText);
		...
		public void TabWidth_Setting_To_Zero_Keeps_AllowsTab ()
		{
			...
			Assert.True (_textView.AllowsReturn);
			Assert.True (_textView.Multiline);
			_textView.ProcessKeyPressed (new (Key.Tab));
			Assert.Equal ("\tTAB to jump between text fields.", _textView.Text);
			_textView.ProcessKeyPressed (new (Key.Tab | Key.ShiftMask));
			Assert.Equal ("TAB to jump between text fields.", _textView.Text);
		}
		...
		public void AllowsReturn_Setting_To_True_Changes_Multiline_To_True_If_It_Is_False ()
		{
			...
			Assert.Equal (4, _textView.TabWidth);
			Assert.True (_textView.AllowsTab);
			_textView.ProcessKeyPressed (new (Key.Enter));
			Assert.Equal (Environment.NewLine +
		...
		public void Tab_Test_Follow_By_CursorLeft_And_Then_Follow_By_CursorRight ()
		{
			...
			Application.Iteration += (s, a) => {
				...
				while (col > 0) {
					col--;
					_textView.ProcessKeyPressed (new (Key.CursorLeft));
					Assert.Equal (new Point (col, 0), _textView.CursorPosition);
					leftCol = GetLeftCol (leftCol);
					Assert.Equal (leftCol, _textView.LeftColumn);
				}
				while (col < 100) {
					col++;
					_textView.ProcessKeyPressed (new (Key.CursorRight));
					Assert.Equal (new Point (col, 0), _textView.CursorPosition);
					leftCol = GetLeftCol (leftCol);
					Assert.Equal (leftCol, _textView.LeftColumn);
				}
		...
		public void Tab_Test_Follow_By_Home_And_Then_Follow_By_End_And_Then_Follow_By_BackTab_With_Text ()
		{
			...
			Application.Iteration += (s, a) => {
				...
				_textView.ProcessKeyPressed (new (Key.Home));
				col = 0;
				Assert.Equal (new Point (col, 0), _textView.CursorPosition);
				leftCol = 0;
				Assert.Equal (leftCol, _textView.LeftColumn);

				_textView.ProcessKeyPressed (new (Key.End));
...
", output);
			...
			Assert.True (tv.ProcessKeyPressed (new (Key.Backspace)));
			Application.Refresh ();
			Assert.Equal (0, tv.LeftColumn);
			TestHelpers.AssertDriverContentsAre (@"
aaa
...
", output);
		...
		public void BottomOffset_Sets_To_Zero_Adjust_TopRow ()
		{
			...
			tv.ProcessKeyPressed (new (Key.CtrlMask | Key.End));
		...
		public void RightOffset_Sets_To_Zero_Adjust_leftColumn ()
		{
			...
			tv.ProcessKeyPressed (new (Key.End));
		...
		public void LeftColumn_Add_One_If_Text_Length_Is_Equal_To_Width ()
		{
			...
			Assert.True (tv.ProcessKeyPressed (new (Key.CursorRight)));
		...
		public void KeyBindings_Command ()
		{
			...
			tv.CanFocus = false;
			Assert.True (tv.ProcessKeyPressed (new (Key.CursorLeft)));
			tv.CanFocus = true;
			Assert.False (tv.ProcessKeyPressed (new (Key.CursorLeft)));
			Assert.True (tv.ProcessKeyPressed (new (Key.CursorRight)));
			Assert.Equal (new Point (1, 0), tv.CursorPosition);
			Assert.True (tv.ProcessKeyPressed (new (Key.End | Key.CtrlMask)));
			...
			Assert.True (tv.ProcessKeyPressed (new (Key.Z | Key.CtrlMask)));
			tv.Draw ();
			Assert.Equal ($"This is the first line.{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.", tv.Text);
			Assert.Equal (new Point (23, 2), tv.CursorPosition);
			Assert.Empty (tv.Autocomplete.Suggestions);
			Assert.True (tv.ProcessKeyPressed (new (Key.R | Key.CtrlMask)));
			tv.Draw ();
			Assert.Equal ($"This is the first line.{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.F", tv.Text);
			Assert.Equal (new Point (24, 2), tv.CursorPosition);
			Assert.Empty (tv.Autocomplete.Suggestions);
			...
			Assert.True (tv.ProcessKeyPressed (new (Key.Enter)));
			...
			Assert.True (tv.ProcessKeyPressed (new (Key.PageDown | Key.ShiftMask)));
			Assert.Equal (28, tv.GetCurrentLine ().Count);
			Assert.Equal (new Point (23, 2), tv.CursorPosition); // gets the previous length
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.ProcessKeyPressed (new (Key.Home | Key.CtrlMask)));
			Assert.Equal (Point.Empty, tv.CursorPosition);
			Assert.True (tv.ProcessKeyPressed (new (Key.N | Key.CtrlMask)));
			Assert.Equal (new Point (0, 1), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.ProcessKeyPressed (new (Key.P | Key.CtrlMask)));
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.ProcessKeyPressed (new (Key.CursorDown)));
			Assert.Equal (new Point (0, 1), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.ProcessKeyPressed (new (Key.CursorUp)));
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.ProcessKeyPressed (new (Key.CursorDown | Key.ShiftMask)));
			Assert.Equal (new Point (0, 1), tv.CursorPosition);
			...
			Assert.True (tv.ProcessKeyPressed (new (Key.CursorUp | Key.ShiftMask)));
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.ProcessKeyPressed (new (Key.F | Key.CtrlMask)));
			Assert.Equal (new Point (1, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.ProcessKeyPressed (new (Key.B | Key.CtrlMask)));
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.ProcessKeyPressed (new (Key.CursorRight)));
			...
			Assert.True (tv.ProcessKeyPressed (new (Key.CursorRight | Key.ShiftMask)));
			Assert.Equal (new Point (1, 0), tv.CursorPosition);
			Assert.Equal (1, tv.SelectedLength);
			Assert.Equal ("T", tv.SelectedText);
			Assert.True (tv.Selecting);
			Assert.True (tv.ProcessKeyPressed (new (Key.CursorLeft | Key.ShiftMask)));
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.Selecting);
			Assert.True (tv.ProcessKeyPressed (new (Key.DeleteChar)));
			Assert.Equal ($"This is the first line.{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			...
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.True (tv.ProcessKeyPressed (new (Key.D | Key.CtrlMask)));
			...
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.True (tv.ProcessKeyPressed (new (Key.End)));
			Assert.Equal ($"is is the first line.{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (21, 0), tv.CursorPosition);
			Assert.True (tv.ProcessKeyPressed (new (Key.Delete)));
			Assert.Equal ($"is is the first line{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			...
			Assert.True (tv.ProcessKeyPressed (new (Key.Home)));
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.True (tv.ProcessKeyPressed (new (Key.End | Key.ShiftMask)));
			Assert.Equal (new Point (19, 0), tv.CursorPosition);
			Assert.Equal (19, tv.SelectedLength);
			Assert.Equal ("is is the first lin", tv.SelectedText);
			Assert.True (tv.Selecting);
			Assert.True (tv.ProcessKeyPressed (new (Key.Home | Key.ShiftMask)));
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.True (tv.Selecting);
			Assert.True (tv.ProcessKeyPressed (new (Key.E | Key.CtrlMask)));
			Assert.Equal (new Point (19, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.True (tv.ProcessKeyPressed (new (Key.A | Key.CtrlMask)));
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.True (tv.ProcessKeyPressed (new (Key.K | Key.CtrlMask)));
			...
			Assert.True (tv.ProcessKeyPressed (new (Key.Y | Key.CtrlMask)));
			Assert.Equal ($"is is the first lin{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (19, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			...
			Assert.True (tv.ProcessKeyPressed (new (Key.DeleteChar | Key.CtrlMask | Key.ShiftMask)));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			...
			Assert.False (tv.Selecting);
			Assert.Equal ("is is the first lin", Clipboard.Contents);
			Assert.True (tv.ProcessKeyPressed (new (Key.K | Key.AltMask)));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			...
			Assert.True (tv.ProcessKeyPressed (new (Key.Space | Key.CtrlMask)));
			Assert.Equal ($"is is the first lin{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (19, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			...
			Assert.Equal (0, tv.SelectionStartRow);
			tv.SelectionStartColumn = 0;
			Assert.True (tv.ProcessKeyPressed (new (((int)'C' + Key.AltMask))));
			Assert.Equal ($"is is the first lin{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (19, 0), tv.CursorPosition);
			Assert.Equal (19, tv.SelectedLength);
			Assert.Equal ("is is the first lin", tv.SelectedText);
			...
			Assert.Equal (0, tv.SelectionStartColumn);
			Assert.Equal (0, tv.SelectionStartRow);
			Assert.True (tv.ProcessKeyPressed (new (Key.C | Key.CtrlMask)));
			Assert.Equal ($"is is the first lin{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (19, 0), tv.CursorPosition);
			Assert.Equal (19, tv.SelectedLength);
			Assert.Equal ("is is the first lin", tv.SelectedText);
			...
			Assert.Equal (0, tv.SelectionStartColumn);
			Assert.Equal (0, tv.SelectionStartRow);
			Assert.True (tv.ProcessKeyPressed (new (((int)'W' + Key.AltMask))));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			...
			Assert.Equal (0, tv.SelectionStartRow);
			Assert.Equal ("is is the first lin", Clipboard.Contents);
			Assert.True (tv.ProcessKeyPressed (new (Key.W | Key.CtrlMask)));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			...
			Assert.True (tv.ProcessKeyPressed (new (Key.X | Key.CtrlMask)));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			...
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.True (tv.ProcessKeyPressed (new (Key.CtrlMask | Key.CursorLeft)));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (18, 2), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.True (tv.ProcessKeyPressed (new (Key.CtrlMask | Key.CursorLeft | Key.ShiftMask)));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (12, 2), tv.CursorPosition);
			Assert.Equal (6, tv.SelectedLength);
			Assert.Equal ("third ", tv.SelectedText);
			Assert.True (tv.Selecting);
			Assert.True (tv.ProcessKeyPressed (new ((Key)((int)'B' + Key.AltMask))));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (8, 2), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.True (tv.ProcessKeyPressed (new (Key.CtrlMask | Key.CursorRight)));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (12, 2), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.True (tv.ProcessKeyPressed (new (Key.CtrlMask | Key.CursorRight | Key.ShiftMask)));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (18, 2), tv.CursorPosition);
			Assert.Equal (6, tv.SelectedLength);
			Assert.Equal ("third ", tv.SelectedText);
			Assert.True (tv.Selecting);
			Assert.True (tv.ProcessKeyPressed (new ((Key)((int)'F' + Key.AltMask))));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			...
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.True (tv.ProcessKeyPressed (new (Key.DeleteChar | Key.CtrlMask)));
			Assert.Equal ($"This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting); Assert.True (tv.ProcessKeyPressed (new (Key.End | Key.CtrlMask)));
			Assert.Equal ($"This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
			Assert.Equal (new Point (28, 1), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			Assert.False (tv.Selecting);
			Assert.False (tv.Selecting); Assert.True (tv.ProcessKeyPressed (new (Key.Backspace | Key.CtrlMask)));
			...
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.False (tv.Selecting);
			Assert.False (tv.ProcessKeyPressed (new (Key.Enter)));
			Assert.Equal ($"This is the second line.{Environment.NewLine}This is the third ", tv.Text);
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (0, tv.SelectedLength);
			Assert.Equal ("", tv.SelectedText);
			...
			Assert.False (tv.Selecting);
			Assert.True (tv.AllowsReturn);
			Assert.True (tv.ProcessKeyPressed (new (Key.CtrlMask | Key.End | Key.ShiftMask)));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third ", tv.Text);
			Assert.Equal (new Point (18, 2), tv.CursorPosition);
			...
			Assert.True (tv.Selecting);
			Assert.True (tv.ProcessKeyPressed (new (Key.CtrlMask | Key.Home | Key.ShiftMask)));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third ", tv.Text);
			Assert.Equal (new Point (0, 0), tv.CursorPosition);
			Assert.Equal (Environment.NewLine.Length, tv.SelectedLength);
			Assert.Equal ($"{Environment.NewLine}", tv.SelectedText);
			Assert.True (tv.Selecting);
			Assert.True (tv.ProcessKeyPressed (new (Key.T | Key.CtrlMask)));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third ", tv.Text);
			Assert.Equal (new Point (18, 2), tv.CursorPosition);
			...
			Assert.True (tv.Selecting);
			Assert.True (tv.Used);
			Assert.True (tv.ProcessKeyPressed (new (Key.InsertChar)));
			Assert.False (tv.Used);
			Assert.True (tv.AllowsTab);
			Assert.Equal (new Point (18, 2), tv.CursorPosition);
			tv.AllowsTab = false;
			Assert.False (tv.ProcessKeyPressed (new (Key.Tab)));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third ", tv.Text);
			...
			Assert.True (tv.Selecting);
			tv.Selecting = false;
			Assert.True (tv.ProcessKeyPressed (new (Key.Tab)));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third \t", tv.Text);
			Assert.True (tv.AllowsTab);
			tv.AllowsTab = false;
			Assert.False (tv.ProcessKeyPressed (new (Key.Tab | Key.ShiftMask)));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third \t", tv.Text);
			Assert.False (tv.AllowsTab);
			tv.AllowsTab = true;
			Assert.True (tv.ProcessKeyPressed (new (Key.Tab | Key.ShiftMask)));
			Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third ", tv.Text);
			Assert.True (tv.AllowsTab);
			Assert.False (tv.ProcessKeyPressed (new (Key.Tab | Key.CtrlMask)));
			Assert.False (tv.ProcessKeyPressed (new (Application.AlternateForwardKey)));
			Assert.False (tv.ProcessKeyPressed (new (Key.Tab | Key.CtrlMask | Key.ShiftMask)));
		...
		public void HistoryText_Undo_Redo_Multi_Line_With_Empty_Text ()
		{
			...
			Assert.True (tv.ProcessKeyPressed (new ((Key)'n')));
			Assert.Equal ("On", tv.Text);
			Assert.Equal (1, tv.Lines);
			Assert.Equal (new Point (2, 0), tv.CursorPosition);
			Assert.True (tv.IsDirty);

			Assert.True (tv.ProcessKeyPressed (new ((Key)'e')));
			...
			Assert.True (tv.ProcessKeyPressed (new ((Key)'w')));
		...
		public void HistoryText_Undo_Redo_Multi_Line_Selected_With_Empty_Text ()
		{
			...
			Assert.True (tv.ProcessKeyPressed (new (Key.D1)));
			...
			Assert.True (tv.ProcessKeyPressed (new (Key.D2)));
		...
		public void HistoryText_Undo_Redo_Multi_Line_Selected_DeleteCharLeft_All ()
		{
			...
			Assert.True (tv.ProcessKeyPressed (new (Key.End | Key.CtrlMask | Key.ShiftMask)));
			Assert.Equal ($"This is the first line.{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.", tv.Text);
		...
		public void HistoryText_Undo_Redo_KillWordBackward ()
		{
			...
			Assert.True (tv.ProcessKeyPressed (new (Key.Backspace | Key.CtrlMask)));
			Assert.Equal ($"First line.{Environment.NewLine}Second line", tv.Text);
			Assert.Equal ("", tv.SelectedText);
			Assert.Equal (2, tv.Lines);
		...
		public void HistoryText_Undo_Redo_Changing_On_Middle_Clear_History_Forwards ()
		{
			...
			Assert.True (tv.ProcessKeyPressed (new (Key.D3)));
			...
			Assert.True (tv.ProcessKeyPressed (new (Key.D4)));
		...
		public void HistoryText_Undo_Redo_First_Line_Selected_Return_And_InsertText ()
		{
			...
			Assert.True (tv.ProcessKeyPressed (new ((Key)'a')));
		...
		public void ScrollDownTillCaretOffscreen_ThenType ()
		{
			...
			tv.ProcessKeyPressed (new (Key.P));
		}
...
└─────────────┘", output);
		...
		public void ContentsChanged_Event_Fires_On_Undo_Redo ()
		{
			...
			expectedEventCount++;
			Assert.True (_textView.ProcessKeyPressed (new (Key.Enter)));
			Assert.Equal (expectedEventCount, eventcount);

			// Undo
			expectedEventCount++;
			Assert.True (_textView.ProcessKeyPressed (new (Key.Z | Key.CtrlMask)));
			Assert.Equal (expectedEventCount, eventcount);

			// Redo
			expectedEventCount++;
			Assert.True (_textView.ProcessKeyPressed (new (Key.R | Key.CtrlMask)));
			Assert.Equal (expectedEventCount, eventcount);
