...
",
        ...
        while (!iterationsFinished) {
            ...
            switch (iteration) {
                case 0:
                    ...
                    Assert.Equal (0, _textView.CursorPosition.Y);
                    Assert.Equal (
                                  "is the first line." + Environment.NewLine
                                                       + "This is the second line.",
                                  _textView.Text);
        ...
        while (!iterationsFinished) {
            switch (iteration) {
                ...
                case 1:
                    _textView.NewKeyDownEvent (
                                               new Key (
                                                        KeyCode.Delete | KeyCode.CtrlMask |
                                                        KeyCode.ShiftMask));
                    Assert.Equal (0, _textView.CursorPosition.X);
                    Assert.Equal (0, _textView.CursorPosition.Y);
                    Assert.Equal ("This is the second line.", _textView.Text);
    ...
    public void Mouse_Button_Shift_Preserves_Selection () {
        ...
        Assert.True (
                     _textView.MouseEvent (
                                           new MouseEvent {
                                                              X = 12, Y = 0,
                                                              Flags = MouseFlags.Button1Pressed | MouseFlags.ButtonShift
                                                          }));
    ...
    public void Selected_Text_Shows () {
        ...
        Attribute[] attributes = new[] {
                                           ...
                                           new (
                                                _textView.ColorScheme.Focus.Background,
                                                _textView.ColorScheme.Focus.Foreground)
                                       };
...
",
    ...
    public void WordForward_Multiline_With_Selection () {
        ...
        while (!iterationsFinished) {
            _textView.NewKeyDownEvent (
                                       new Key (
                                                KeyCode.CursorRight | KeyCode.CtrlMask |
                                                KeyCode.ShiftMask));
            switch (iteration) {
                case 0:
                    Assert.Equal (5, _textView.CursorPosition.X);
        ...
        Exception exception = Record.Exception (
                                                () => {
                                                    var tv = new TextView {
                                                                              ...
                                                                              WordWrap = true,
                                                                              Text =
                                                                                  "これは、左右のクリップ境界をテストするための非常に長いテキストです。"
                                                                          };
                                                });
        Assert.Null (exception);
    }
