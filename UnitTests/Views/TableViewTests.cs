...
namespace Terminal.Gui.ViewsTests {
	...
	public class TableViewTests {
		...
		public void PageDown_ExcludesHeaders ()
		{
			...
			tableView.ProcessKey (new (Key.PageDown, new KeyModifiers ()));
...
";
		...
		public void TableView_Activate ()
		{
			...
			tv.ProcessKey (new (Key.Enter, new KeyModifiers ()));
			...
			tv.ProcessKey (new (Key.z, new KeyModifiers ()));
			Assert.Equal ("R0C0", activatedValue);
		...
		public void TableViewMultiSelect_CannotFallOffLeft ()
		{
			...
			tv.SelectedColumn = 1;
			tv.SelectedRow = 1;
			tv.ProcessKey (new (Key.CursorLeft | Key.ShiftMask, new KeyModifiers { Shift = true }));
		...
		public void TableViewMultiSelect_CannotFallOffRight ()
		{
			...
			tv.SelectedColumn = 0;
			tv.SelectedRow = 1;
			tv.ProcessKey (new (Key.CursorRight | Key.ShiftMask, new KeyModifiers { Shift = true }));
		...
		public void TableViewMultiSelect_CannotFallOffBottom ()
		{
			...
			tv.SelectedRow = 0;
			tv.ProcessKey (new (Key.CursorRight | Key.ShiftMask, new KeyModifiers { Shift = true }));
			tv.ProcessKey (new (Key.CursorDown | Key.ShiftMask, new KeyModifiers { Shift = true }));
		...
		public void TableViewMultiSelect_CannotFallOffTop ()
		{
			...
			tv.SelectedRow = 1;
			tv.ProcessKey (new (Key.CursorLeft | Key.ShiftMask, new KeyModifiers { Shift = true }));
			tv.ProcessKey (new (Key.CursorUp | Key.ShiftMask, new KeyModifiers { Shift = true }));
...
";
		...
		public void ScrollDown_OneLineAtATime ()
		{
			...
			// Scroll down
			tableView.ProcessKey (new KeyEventArgs () { Key = Key.CursorDown });
...
│1│2│3│";
			...
			// Scroll right
			tableView.ProcessKey (new KeyEventArgs () { Key = Key.CursorRight });
...
│2│3│4│";
		...
		public void TestColumnStyle_VisibleFalse_CursorStepsOverInvisibleColumns ()
		{
			...
			tableView.ProcessKey (new KeyEventArgs () { Key = Key.CursorLeft });
		...
		public void TestColumnStyle_FirstColumnVisibleFalse_CursorStaysAt1 (bool useHome)
		{
			...
			tableView.ProcessKey (new KeyEventArgs () {
		...
		public void TestColumnStyle_VisibleFalse_MultiSelectingStepsOverInvisibleColumns ()
		{
			...
			tableView.ProcessKey (new KeyEventArgs () { Key = Key.CursorRight | Key.ShiftMask });
		...
		public void TestToggleCells_MultiSelectOn ()
		{
			...
			// Toggle Select
			tableView.ProcessKey (new KeyEventArgs () { Key = Key.Space });
			...
			tableView.ProcessKey (new KeyEventArgs () { Key = Key.CursorRight });
			tableView.ProcessKey (new KeyEventArgs () { Key = Key.CursorUp });
		...
		public void TestToggleCells_MultiSelectOn_SquareSelectToggled ()
		{
			...
			tableView.ProcessKey (new KeyEventArgs () { Key = Key.ShiftMask | Key.CursorDown });
			tableView.ProcessKey (new KeyEventArgs () { Key = Key.ShiftMask | Key.CursorRight });
...
│☐│1│2│";
			...
			//toggle the top cell
			tv.ProcessKey (new (Key.Space, new KeyModifiers ()));
...
│☐│1│2│";
			...
			tv.ProcessKey (new (Key.CursorDown, new KeyModifiers ()));
			tv.ProcessKey (new (Key.Space, new KeyModifiers ()));
...
│☐│1│2│";
			...
			// untoggle top one
			tv.ProcessKey (new (Key.CursorUp, new KeyModifiers ()));
			tv.ProcessKey (new (Key.Space, new KeyModifiers ()));
...
│☐│1│2│";
		...
		public void TestTableViewCheckboxes_SelectAllToggle ()
		{
			...
			//toggle all cells
			tv.ProcessKey (new (Key.A | Key.CtrlMask, new KeyModifiers { Ctrl = true }));
			tv.ProcessKey (new (Key.Space, new KeyModifiers ()));
...
│zoo  │o                                         │";
			...
			// already on fish
			tv.ProcessKey (new KeyEventArgs () { Key = Key.f });
			Assert.Equal (0, tv.SelectedRow);

			// not focused
			tv.ProcessKey (new KeyEventArgs () { Key = Key.z });
			Assert.Equal (0, tv.SelectedRow);
			...
			// move to troll
			tv.ProcessKey (new KeyEventArgs () { Key = Key.t });
			Assert.Equal (1, tv.SelectedRow);
			...
			Assert.Equal (0, tv.SelectedColumn);
			Assert.Equal (2, tv.SelectedRow);
			tv.ProcessKey (new KeyEventArgs () { Key = Key.CursorRight });
			Assert.Equal (1, tv.SelectedColumn);
			Assert.Equal (2, tv.SelectedRow);
			...
			tv.ProcessKey (new KeyEventArgs () { Key = Key.h });
			Assert.Equal (0, tv.SelectedRow);

			// jump to zoo which ends in o
			tv.ProcessKey (new KeyEventArgs () { Key = Key.o });
			Assert.Equal (3, tv.SelectedRow);
