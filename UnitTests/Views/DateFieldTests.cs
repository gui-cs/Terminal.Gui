...
namespace Terminal.Gui.ViewsTests {
	public class DateFieldTests {
		...
		public void KeyBindings_Command ()
		{
			...
			df.ReadOnly = true;
			Assert.True (df.ProcessKey (new OldKeyEvent (Key.DeleteChar, new KeyModifiers ())));
			Assert.Equal (" 12/12/1971", df.Text);
			df.ReadOnly = false;
			Assert.True (df.ProcessKey (new OldKeyEvent (Key.D | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (" 02/12/1971", df.Text);
			df.CursorPosition = 4;
			df.ReadOnly = true;
			Assert.True (df.ProcessKey (new OldKeyEvent (Key.Delete, new KeyModifiers ())));
			Assert.Equal (" 02/12/1971", df.Text);
			df.ReadOnly = false;
			Assert.True (df.ProcessKey (new OldKeyEvent (Key.Backspace, new KeyModifiers ())));
			Assert.Equal (" 02/02/1971", df.Text);
			Assert.True (df.ProcessKey (new OldKeyEvent (Key.Home, new KeyModifiers ())));
			Assert.Equal (1, df.CursorPosition);
			Assert.True (df.ProcessKey (new OldKeyEvent (Key.End, new KeyModifiers ())));
			Assert.Equal (10, df.CursorPosition);
			Assert.True (df.ProcessKey (new OldKeyEvent (Key.A | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (1, df.CursorPosition);
			Assert.True (df.ProcessKey (new OldKeyEvent (Key.E | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (10, df.CursorPosition);
			Assert.True (df.ProcessKey (new OldKeyEvent (Key.CursorLeft, new KeyModifiers ())));
			Assert.Equal (9, df.CursorPosition);
			Assert.True (df.ProcessKey (new OldKeyEvent (Key.CursorRight, new KeyModifiers ())));
			Assert.Equal (10, df.CursorPosition);
			Assert.False (df.ProcessKey (new OldKeyEvent (Key.A, new KeyModifiers ())));
			df.ReadOnly = true;
			df.CursorPosition = 1;
			Assert.True (df.ProcessKey (new OldKeyEvent (Key.D1, new KeyModifiers ())));
			Assert.Equal (" 02/02/1971", df.Text);
			df.ReadOnly = false;
			Assert.True (df.ProcessKey (new OldKeyEvent (Key.D1, new KeyModifiers ())));
