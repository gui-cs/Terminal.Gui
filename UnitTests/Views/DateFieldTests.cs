...
namespace Terminal.Gui.ViewsTests {
	public class DateFieldTests {
		...
		public void KeyBindings_Command ()
		{
			...
			df.ReadOnly = true;
			Assert.True (df.ProcessKeyPressed (new (Key.DeleteChar, new KeyModifiers ())));
			Assert.Equal (" 12/12/1971", df.Text);
			df.ReadOnly = false;
			Assert.True (df.ProcessKeyPressed (new (Key.D | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (" 02/12/1971", df.Text);
			df.CursorPosition = 4;
			df.ReadOnly = true;
			Assert.True (df.ProcessKeyPressed (new (Key.Delete, new KeyModifiers ())));
			Assert.Equal (" 02/12/1971", df.Text);
			df.ReadOnly = false;
			Assert.True (df.ProcessKeyPressed (new (Key.Backspace, new KeyModifiers ())));
			Assert.Equal (" 02/02/1971", df.Text);
			Assert.True (df.ProcessKeyPressed (new (Key.Home, new KeyModifiers ())));
			Assert.Equal (1, df.CursorPosition);
			Assert.True (df.ProcessKeyPressed (new (Key.End, new KeyModifiers ())));
			Assert.Equal (10, df.CursorPosition);
			Assert.True (df.ProcessKeyPressed (new (Key.A | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (1, df.CursorPosition);
			Assert.True (df.ProcessKeyPressed (new (Key.E | Key.CtrlMask, new KeyModifiers ())));
			Assert.Equal (10, df.CursorPosition);
			Assert.True (df.ProcessKeyPressed (new (Key.CursorLeft, new KeyModifiers ())));
			Assert.Equal (9, df.CursorPosition);
			Assert.True (df.ProcessKeyPressed (new (Key.CursorRight, new KeyModifiers ())));
			Assert.Equal (10, df.CursorPosition);
			// Non-numerics are ignored
			Assert.True (df.ProcessKeyPressed (new (Key.A, new KeyModifiers ())));
			df.ReadOnly = true;
			df.CursorPosition = 1;
			Assert.True (df.ProcessKeyPressed (new (Key.D1, new KeyModifiers ())));
			Assert.Equal (" 02/02/1971", df.Text);
			df.ReadOnly = false;
			Assert.True (df.ProcessKeyPressed (new (Key.D1, new KeyModifiers ())));
