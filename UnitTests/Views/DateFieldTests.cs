...
namespace Terminal.Gui.ViewsTests {
	public class DateFieldTests {
		...
		public void KeyBindings_Command ()
		{
			...
			df.ReadOnly = true;
			Assert.True (df.ProcessKeyPressed (new (Key.DeleteChar)));
			Assert.Equal (" 12/12/1971", df.Text);
			df.ReadOnly = false;
			Assert.True (df.ProcessKeyPressed (new (Key.D | Key.CtrlMask)));
			Assert.Equal (" 02/12/1971", df.Text);
			df.CursorPosition = 4;
			df.ReadOnly = true;
			Assert.True (df.ProcessKeyPressed (new (Key.Delete)));
			Assert.Equal (" 02/12/1971", df.Text);
			df.ReadOnly = false;
			Assert.True (df.ProcessKeyPressed (new (Key.Backspace)));
			Assert.Equal (" 02/02/1971", df.Text);
			Assert.True (df.ProcessKeyPressed (new (Key.Home)));
			Assert.Equal (1, df.CursorPosition);
			Assert.True (df.ProcessKeyPressed (new (Key.End)));
			Assert.Equal (10, df.CursorPosition);
			Assert.True (df.ProcessKeyPressed (new (Key.A | Key.CtrlMask)));
			Assert.Equal (1, df.CursorPosition);
			Assert.True (df.ProcessKeyPressed (new (Key.E | Key.CtrlMask)));
			Assert.Equal (10, df.CursorPosition);
			Assert.True (df.ProcessKeyPressed (new (Key.CursorLeft)));
			Assert.Equal (9, df.CursorPosition);
			Assert.True (df.ProcessKeyPressed (new (Key.CursorRight)));
			Assert.Equal (10, df.CursorPosition);
			// Non-numerics are ignored
			Assert.True (df.ProcessKeyPressed (new (Key.A)));
			df.ReadOnly = true;
			df.CursorPosition = 1;
			Assert.True (df.ProcessKeyPressed (new (Key.D1)));
			Assert.Equal (" 02/02/1971", df.Text);
			df.ReadOnly = false;
			Assert.True (df.ProcessKeyPressed (new (Key.D1)));
