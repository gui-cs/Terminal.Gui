...
namespace Terminal.Gui.ViewsTests {
	public class DateFieldTests {
		...
		public void KeyBindings_Command ()
		{
			...
			df.ReadOnly = true;
			Assert.True (df.ProcessKeyDown (new (KeyCode.DeleteChar)));
			Assert.Equal (" 12/12/1971", df.Text);
			df.ReadOnly = false;
			Assert.True (df.ProcessKeyDown (new (KeyCode.D | KeyCode.CtrlMask)));
			Assert.Equal (" 02/12/1971", df.Text);
			df.CursorPosition = 4;
			df.ReadOnly = true;
			Assert.True (df.ProcessKeyDown (new (KeyCode.Delete)));
			Assert.Equal (" 02/12/1971", df.Text);
			df.ReadOnly = false;
			Assert.True (df.ProcessKeyDown (new (KeyCode.Backspace)));
			Assert.Equal (" 02/02/1971", df.Text);
			Assert.True (df.ProcessKeyDown (new (KeyCode.Home)));
			Assert.Equal (1, df.CursorPosition);
			Assert.True (df.ProcessKeyDown (new (KeyCode.End)));
			Assert.Equal (10, df.CursorPosition);
			Assert.True (df.ProcessKeyDown (new (KeyCode.A | KeyCode.CtrlMask)));
			Assert.Equal (1, df.CursorPosition);
			Assert.True (df.ProcessKeyDown (new (KeyCode.E | KeyCode.CtrlMask)));
			Assert.Equal (10, df.CursorPosition);
			Assert.True (df.ProcessKeyDown (new (KeyCode.CursorLeft)));
			Assert.Equal (9, df.CursorPosition);
			Assert.True (df.ProcessKeyDown (new (KeyCode.CursorRight)));
			Assert.Equal (10, df.CursorPosition);
			// Non-numerics are ignored
			Assert.True (df.ProcessKeyDown (new (KeyCode.A)));
			df.ReadOnly = true;
			df.CursorPosition = 1;
			Assert.True (df.ProcessKeyDown (new (KeyCode.D1)));
			Assert.Equal (" 02/02/1971", df.Text);
			df.ReadOnly = false;
			Assert.True (df.ProcessKeyDown (new (KeyCode.D1)));
