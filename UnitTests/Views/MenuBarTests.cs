...
";
	...
	public void MenuOpened_On_Disabled_MenuItem ()
	{
		...
		Assert.True (mCurrent.ProcessKeyDown (new Key (KeyCode.CursorDown)));
		Assert.True (menu.IsMenuOpen);
		Assert.Equal ("_File", miCurrent.Parent.Title);
		Assert.Equal ("_Save", miCurrent.Title);

		Assert.True (mCurrent.ProcessKeyDown (new Key (KeyCode.CursorUp)));
		Assert.True (menu.IsMenuOpen);
		Assert.Equal ("_File", miCurrent.Parent.Title);
		Assert.Equal ("_New", miCurrent.Title);
...
";
		...
		Assert.True (Application.Top.Subviews [1].ProcessKeyDown (new Key (KeyCode.CursorDown)));
		Application.Top.Draw ();
		expected = @"
 Numbers                 
┌────────┐               
...
";
		...
		Assert.True (Application.Top.Subviews [2].ProcessKeyDown (new Key (KeyCode.CursorLeft)));
		Application.Top.Draw ();
		expected = @"
 Numbers  
┌────────┐
...
";
		...
		Assert.True (Application.Top.Subviews [1].ProcessKeyDown (new Key (KeyCode.Esc)));
		Application.Top.Draw ();
		expected = @"
 Numbers
";
...
";
		...
		Assert.True (Application.Top.Subviews [1].ProcessKeyDown (new Key (KeyCode.CursorDown)));
		Assert.True (Application.Top.Subviews [1].ProcessKeyDown (new Key (KeyCode.Enter)));
		Application.Top.Draw ();
		expected = @"
 Numbers       
┌─────────────┐
...
";
		...
		Assert.True (Application.Top.Subviews [2].ProcessKeyDown (new Key (KeyCode.Enter)));
		Application.Top.Draw ();
		expected = @"
 Numbers  
┌────────┐
...
";
	...
	public void HotKey_MenuBar_ProcessKeyPressed_Menu_ProcessKey ()
	{
		...
		Assert.True (menu.ProcessKeyDown (new Key (KeyCode.AltMask | KeyCode.F)));
		Assert.True (menu.IsMenuOpen);
		Application.Top.Draw ();
		TestHelpers.AssertDriverContentsAre (expectedMenu.expectedSubMenuOpen (0), _output);

		Assert.True (Application.Top.Subviews [1].ProcessKeyDown (new Key (KeyCode.N)));
		Application.MainLoop.RunIteration ();
		Assert.True (newAction);

		Assert.True (menu.ProcessKeyDown (new Key (KeyCode.AltMask | KeyCode.E)));
		Assert.True (menu.IsMenuOpen);
		Application.Top.Draw ();
		TestHelpers.AssertDriverContentsAre (expectedMenu.expectedSubMenuOpen (1), _output);

		Assert.True (Application.Top.Subviews [1].ProcessKeyDown (new Key (KeyCode.C)));
		Application.MainLoop.RunIteration ();
		Assert.True (copyAction);
	}
	...
	public void MenuBar_Position_And_Size_With_HotKeys_Is_The_Same_As_Without_HotKeys ()
	{
		...
		// Open second
		Assert.True (Application.Top.Subviews [1].ProcessKeyDown (new Key (KeyCode.CursorRight)));
		Assert.True (menu.IsMenuOpen);
		Application.Top.Draw ();
		TestHelpers.AssertDriverContentsAre (expectedMenu.expectedSubMenuOpen (1), _output);
	...
	public void Parent_MenuItem_Stay_Focused_If_Child_MenuItem_Is_Empty_By_Keyboard ()
	{
		...
		Assert.True (menu._openMenu.ProcessKeyDown (new Key (KeyCode.CursorRight)));
		Assert.True (menu.IsMenuOpen);
		Assert.False (tf.HasFocus);
		Application.Top.Draw ();
		TestHelpers.AssertDriverContentsAre (expectedMenu.expectedSubMenuOpen (1), _output);
		...
		// Left - Edit
		Assert.True (menu._openMenu.ProcessKeyDown (new Key (KeyCode.CursorLeft)));
		Assert.True (menu.IsMenuOpen);
		Assert.False (tf.HasFocus);
		Application.Top.Draw ();
		TestHelpers.AssertDriverContentsAre (expectedMenu.expectedSubMenuOpen (1), _output);
	...
	public void Key_Open_And_Close_The_MenuBar ()
	{
		...
		Assert.True (Application.Top.ProcessKeyDown (menu.Key));
		Assert.True (menu.IsMenuOpen);
		Assert.True (Application.Top.ProcessKeyDown (menu.Key));
		Assert.False (menu.IsMenuOpen);

		menu.Key = KeyCode.F10 | KeyCode.ShiftMask;
		Assert.False (Application.Top.ProcessKeyDown (new Key (KeyCode.F9)));
		Assert.False (menu.IsMenuOpen);

		Assert.True (Application.Top.ProcessKeyDown (new Key (KeyCode.F10 | KeyCode.ShiftMask)));
		Assert.True (menu.IsMenuOpen);
		Assert.True (Application.Top.ProcessKeyDown (new Key (KeyCode.F10 | KeyCode.ShiftMask)));
		Assert.False (menu.IsMenuOpen);
	}
...
#endif
		Assert.True (menu.ProcessKeyDown (new Key (KeyCode.CursorRight)));
		Assert.True (menu.ProcessKeyDown (new Key (KeyCode.CursorRight)));
	}
...
└──────────────────────────────────────┘", _output);
			...
			Assert.True (win.ProcessKeyDown (menu.Key));
			top.Draw ();
			TestHelpers.AssertDriverContentsWithFrameAre (@"
┌──────────────────────────────────────┐
│ File  Edit                           │
...
└──────────────────────────────────────┘", _output);
			...
			Assert.True (top.ProcessKeyDown (new Key (KeyCode.F9)));
			top.Draw ();
			TestHelpers.AssertDriverContentsWithFrameAre (@"
┌──────────────────────────────────────┐
│ File  Edit                           │
...
└──────────────────────────────────────┘", _output);
			...
			Assert.True (top.Subviews [0].ProcessKeyDown (new Key (KeyCode.CursorRight)));
			Application.Refresh ();
			TestHelpers.AssertDriverContentsWithFrameAre (@"
┌──────────────────────────────────────┐
│ File  Edit                           │
...
└──────────────────────────────────────┘", _output);
			...
			Assert.True (((MenuBar)top.Subviews [0])._openMenu.ProcessKeyDown (new Key (KeyCode.CursorRight)));
			top.Draw ();
			TestHelpers.AssertDriverContentsWithFrameAre (@"
┌──────────────────────────────────────┐
│ File  Edit                           │
...
└──────────────────────────────────────┘", _output);
	...
	public void AllowNullChecked_Get_Set ()
	{
		...
		Assert.True (menu.ProcessKeyDown (menu.Key));
		Assert.True (menu._openMenu.ProcessKeyDown (new Key (KeyCode.Enter)));
		Application.MainLoop.RunIteration ();
...
";
	...
	public void MenuBarItem_Children_Null_Does_Not_Throw ()
	{
		...
		var exception = Record.Exception (() => menu.ProcessKeyDown (new Key (KeyCode.Space)));
	...
	public void CanExecute_HotKey ()
	{
		...
		Assert.True (top.ProcessKeyDown (new Key (KeyCode.F | KeyCode.AltMask)));
		Assert.True (top.ProcessKeyDown (new Key (KeyCode.N | KeyCode.AltMask)));
		Application.MainLoop.RunIteration ();
		Assert.NotNull (win);
	...
	public void Separators_Does_Not_Throws_Pressing_Menu_Shortcut ()
	{
		...
		var exception = Record.Exception (() => Assert.True (menu.ProcessKeyDown (new Key (KeyCode.AltMask | KeyCode.Q))));
		Assert.Null (exception);
	}
}