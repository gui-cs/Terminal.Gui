...
namespace Terminal.Gui.ViewsTests {
	public class CheckboxTests {
		...
		public void KeyBindings_Command ()
		{
			...
			Assert.False (ckb.ProcessHotKey (new KeyEventArgs (Key.T, new KeyModifiers ())));
			Assert.False (isChecked);
			ckb.Text = "T_est";
			Assert.Equal (Key.E, ckb.HotKey);
			Assert.True (ckb.ProcessHotKey (new KeyEventArgs (Key.E | Key.AltMask, new KeyModifiers () { Alt = true })));
			Assert.True (isChecked);
			isChecked = false;
			Assert.True (ckb.OnKeyPressed (new KeyEventArgs ((Key)' ', new KeyModifiers ())));
			Assert.True (isChecked);
			isChecked = false;
			Assert.True (ckb.OnKeyPressed (new KeyEventArgs (Key.Space, new KeyModifiers ())));
			Assert.True (isChecked);
			Assert.True (ckb.AutoSize);
...
";
		...
		public void AllowNullChecked_Get_Set ()
		{
			...
			Assert.True (checkBox.OnKeyPressed (new KeyEventArgs (Key.Space, new KeyModifiers ())));
