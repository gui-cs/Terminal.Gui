...
public class EscSeqUtilsTests {
	...
	{
		// ESC
		_cki = new ConsoleKeyInfo [] { new ConsoleKeyInfo ('\u001b', 0, false, false, false) };
		var expectedCki = new ConsoleKeyInfo ('\u001b', ConsoleKey.Escape, false, false, false);
		EscSeqUtils.DecodeEscSeq (_escSeqReqProc, ref _newConsoleKeyInfo, ref _key, _cki, ref _mod, out _c1Control, out _code, out _values, out _terminating, out _isKeyMouse, out _mouseFlags, out _pos, out _isReq, ProcessContinuousButtonPressed);
		...
		Assert.Equal (new List<MouseFlags> () { MouseFlags.Button1Released, MouseFlags.Button1Clicked }, _mouseFlags);
		Assert.Equal (new Point (1, 2), _pos);
		Assert.False (_isReq);
		Assert.Equal (0, (int)_arg1);
		Assert.Equal (Point.Empty, _arg2);
