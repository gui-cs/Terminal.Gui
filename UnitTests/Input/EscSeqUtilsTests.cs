...
public class EscSeqUtilsTests {
	...
	MouseFlags _arg1;
	Point _arg2;
	string _c1Control, _code, _terminating;
	ConsoleKeyInfo [] _cki;

	EscSeqRequests _escSeqReqProc;
	bool _isKeyMouse;
	bool _isReq;
	ConsoleKey _key;
	ConsoleModifiers _mod;
	List<MouseFlags> _mouseFlags;
	ConsoleKeyInfo _newConsoleKeyInfo;
	Point _pos;
	string [] _values;
	...
	public void DecodeEscSeq_Tests ()
	{
		...
		EscSeqUtils.DecodeEscSeq (_escSeqReqProc, ref _newConsoleKeyInfo, ref _key, _cki, ref _mod,
			out _c1Control, out _code, out _values, out _terminating, out _isKeyMouse, out _mouseFlags,
			out _pos, out _isReq, ProcessContinuousButtonPressed);
		...
		Assert.Equal (new List<MouseFlags> { MouseFlags.Button1Released, MouseFlags.Button1Clicked },
			_mouseFlags);
		Assert.Equal (new Point (1, 2), _pos);
		Assert.False (_isReq);
		Assert.Equal (0, (int)_arg1);
	...
	void ClearAll ()
	{
		_escSeqReqProc = default;
		_newConsoleKeyInfo = default;
		_key = default;
		_cki = default;
		_mod = default;
		_c1Control = default;
		_code = default;
		_terminating = default;
		_values = default;
		_isKeyMouse = default;
		_isReq = default;
		_mouseFlags = default;
	...
	public void GetConsoleModifiers_Tests ()
	{
		...
		Assert.Equal (ConsoleModifiers.Shift | ConsoleModifiers.Alt | ConsoleModifiers.Control,
