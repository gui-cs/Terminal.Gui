...
namespace Terminal.Gui.TextTests {
	public class AutocompleteTests {
		...
		public void KeyBindings_Command ()
		{
			...
			Assert.True (tv.OnKeyPressed (new (Key.F)));
			top.Draw ();
			...
			Assert.True (tv.OnKeyPressed (new (Key.CursorDown)));
			top.Draw ();
			Assert.Equal ($"F Fortunately super feature.", tv.Text);
			Assert.Equal (new Point (1, 0), tv.CursorPosition);
			Assert.Equal (2, tv.Autocomplete.Suggestions.Count);
			...
			Assert.Equal (0, tv.Autocomplete.SelectedIdx);
			Assert.Equal ("Fortunately", tv.Autocomplete.Suggestions [tv.Autocomplete.SelectedIdx].Replacement);
			Assert.True (tv.OnKeyPressed (new (Key.CursorUp)));
			top.Draw ();
			Assert.Equal ($"F Fortunately super feature.", tv.Text);
			Assert.Equal (new Point (1, 0), tv.CursorPosition);
			Assert.Equal (2, tv.Autocomplete.Suggestions.Count);
			...
			top.Draw ();
			Assert.True (tv.OnKeyPressed (new (tv.Autocomplete.CloseKey)));
			Assert.Equal ($"F Fortunately super feature.", tv.Text);
			Assert.Equal (new Point (1, 0), tv.CursorPosition);
			Assert.Empty (tv.Autocomplete.Suggestions);
			Assert.Equal (3, g.AllSuggestions.Count);
			...
			tv.PositionCursor ();
			Assert.True (tv.OnKeyPressed (new (tv.Autocomplete.Reopen)));
			Assert.Equal ($"F Fortunately super feature.", tv.Text);
			Assert.Equal (new Point (1, 0), tv.CursorPosition);
			Assert.Equal (2, tv.Autocomplete.Suggestions.Count);
			Assert.Equal (3, g.AllSuggestions.Count);
			Assert.True (tv.OnKeyPressed (new (tv.Autocomplete.SelectionKey)));
			tv.PositionCursor ();
		...
		public void CursorLeft_CursorRight_Mouse_Button_Pressed_Does_Not_Show_Popup ()
		{
			...
			for (int i = 0; i < 7; i++) {
				Assert.True (tv.OnKeyPressed (new (Key.CursorRight)));
				Application.Refresh ();
				if (i < 4 || i > 5) {
...
This ag long line and against TextView.
     against                           ", output);
			...
			Assert.True (tv.OnKeyPressed (new (Key.CursorLeft)));
			Application.Refresh ();
			TestHelpers.AssertDriverContentsWithFrameAre (@"
This ag long line and against TextView.
     against                           ", output);
...
This ag long line and against TextView.
     against                           ", output);
			...
			Assert.True (tv.OnKeyPressed (new (Key.Backspace)));
			Application.Refresh ();
			TestHelpers.AssertDriverContentsWithFrameAre (@"
This a long line and against TextView.
     and                              
