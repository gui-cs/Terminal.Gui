...
namespace Terminal.Gui.TextTests {
	public class AutocompleteTests {
		...
		public void KeyBindings_Command ()
		{
			...
			top.Draw ();
			Assert.True (tv.OnKeyPressed (new KeyEventArgs (tv.Autocomplete.CloseKey, new KeyModifiers ())));
			Assert.Equal ($"F Fortunately super feature.", tv.Text);
			Assert.Equal (new Point (1, 0), tv.CursorPosition);
			Assert.Empty (tv.Autocomplete.Suggestions);
			Assert.Equal (3, g.AllSuggestions.Count);
			...
			tv.PositionCursor ();
			Assert.True (tv.OnKeyPressed (new KeyEventArgs (tv.Autocomplete.Reopen, new KeyModifiers ())));
			Assert.Equal ($"F Fortunately super feature.", tv.Text);
			Assert.Equal (new Point (1, 0), tv.CursorPosition);
			Assert.Equal (2, tv.Autocomplete.Suggestions.Count);
			Assert.Equal (3, g.AllSuggestions.Count);
			Assert.True (tv.OnKeyPressed (new KeyEventArgs (tv.Autocomplete.SelectionKey, new KeyModifiers ())));
			tv.PositionCursor ();
...
This a long line and against TextView.
     ...
     against                          ", output);
			...
			Assert.True (tv.OnKeyPressed (new KeyEventArgs (Key.g, new KeyModifiers ())));
			Application.Refresh ();
			TestHelpers.AssertDriverContentsWithFrameAre (@"
