...
└──────────────────┘",
    ...
    public void Draw_A_Menu_Over_A_Top_Dialog ()
    {
        ...
        Assert.Equal (new (0, 0, 40, 15), Application.Running!.GetClip ()!.GetBounds ());
...
";
    ...
    public void HotKey_MenuBar_OnKeyDown_OnKeyUp_ProcessKeyPressed ()
    {
        ...
        Assert.False (Application.Running.ProcessKeyDown (new KeyEventArgs (Key.AltMask)));
        Assert.False (Application.Running.ProcessKeyDown (new KeyEventArgs (Key.AltMask)));
        Assert.True (Application.Running.ProcessKeyUp (new KeyEventArgs (Key.AltMask)));
        Assert.True (menu.IsMenuOpen);
        Application.Running.Draw ();
...
";
        ...
        Assert.True (Application.Running.ProcessKeyDown (new KeyEventArgs (Key.N)));
        AutoInitShutdownAttribute.RunIteration ();
        Assert.False (newAction); // not yet, hot keys don't work if the item is not visible

        Assert.True (Application.Running.ProcessKeyDown (new KeyEventArgs (Key.F)));
        AutoInitShutdownAttribute.RunIteration ();
        Assert.True (Application.Running.ProcessKeyDown (new KeyEventArgs (Key.N)));
        AutoInitShutdownAttribute.RunIteration ();
...
";
        ...
        Assert.True (Application.Running.ProcessKeyDown (new KeyEventArgs (Key.CursorRight)));
        Assert.True (Application.Running.ProcessKeyDown (new KeyEventArgs (Key.C)));
        AutoInitShutdownAttribute.RunIteration ();
    ...
    public void HotKey_MenuBar_ProcessKeyPressed_Menu_ProcessKey ()
    {
        ...
        Assert.True (Application.Running.SubViews.ElementAt (1).NewKeyDownEvent (Key.N));
        AutoInitShutdownAttribute.RunIteration ();
        Assert.True (newAction);
        ...
        Assert.True (Application.Running.SubViews.ElementAt (1).NewKeyDownEvent (Key.C));
        AutoInitShutdownAttribute.RunIteration ();
        Assert.True (copyAction);
        top.Dispose ();
    }
...
└──────────────────────────────────────┘",
    ...
    public void MenuBar_In_Window_Without_Other_Views_Without_Top_Init_With_Run_T ()
    {
        ...
        Application.AddTimeout (TimeSpan.Zero, () =>
                                                       {
                                                           Toplevel top = Application.Running;
...
└──────────────────────────────────────┘",
    ...
    public void MenuBar_Position_And_Size_With_HotKeys_Is_The_Same_As_Without_HotKeys ()
    {
        ...
        // Open second
        Assert.True (Application.Running.SubViews.ElementAt (1).NewKeyDownEvent (Key.CursorRight));
        Assert.True (menu.IsMenuOpen);
        top.SetClipToScreen ();
        top.Draw ();
        DriverAssert.AssertDriverContentsAre (expectedMenu.ExpectedSubMenuOpen (1), output);
...
";
        ...
        Assert.True (Application.Running.SubViews.ElementAt (1).NewKeyDownEvent (Key.CursorDown));
        top.Draw ();
...
";
        ...
        Assert.True (Application.Running.SubViews.ElementAt (2).NewKeyDownEvent (Key.CursorLeft));
        top.Draw ();
...
";
        ...
        Assert.True (Application.Running.SubViews.ElementAt (1).NewKeyDownEvent (Key.Esc));
        top.Draw ();
...
";
        ...
        menu.NewMouseEvent (
                            new ()
                            {
                                Position = new (1, 2), Flags = MouseFlags.ReportMousePosition, View = Application.Running.SubViews.ElementAt (1)
                            }
                           );
        top.Draw ();
...
";
        ...
        Assert.False (
                      menu.NewMouseEvent (
                                          new ()
                                          {
                                              Position = new (1, 1), Flags = MouseFlags.ReportMousePosition, View = Application.Running.SubViews.ElementAt (1)
                                          }
                                         )
                     );
        top.Draw ();
...
";
        ...
        menu.NewMouseEvent (
                            new () { Position = new (70, 2), Flags = MouseFlags.Button1Clicked, View = Application.Running }
                           );
        top.Draw ();
