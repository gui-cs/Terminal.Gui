...
public class TreeViewTests (ITestOutputHelper output)
{
    ...
    public void CursorVisibility_MultiSelect ()
    {
        ...
        tv.SelectAll ();
        tv.SetCursor (tv.Cursor with { Position = new Point (0, 0), Shape = CursorShape.BlinkingBlock });
    ...
    public void ObjectActivationButton_RightClick ()
    {
        ...
        tree.NewMouseEvent (new Mouse { Position = new (0, 1), Flags = MouseFlags.LeftButtonDoubleClicked });

        Assert.Null (activated);
        Assert.False (called);

        tree.NewMouseEvent (new Mouse { Position = new (0, 1), Flags = MouseFlags.MiddleButtonClicked });
