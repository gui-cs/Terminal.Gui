...
public class ApplicationTests
{
    ...
    public void Begin_Sets_Application_Top_To_Console_Size ()
    {
        ...
        Assert.Equal (new (0, 0, 80, 25), Application.Running!.Frame);
        Application.Driver!.SetScreenSize (5, 5);
        Assert.Equal (new (0, 0, 5, 5), Application.Running!.Frame);
        top.Dispose ();
    }
    ...
    public void End_And_Shutdown_Should_Not_Dispose_ApplicationTop ()
    {
        ...
        Application.Running!.Title = "End_And_Shutdown_Should_Not_Dispose_ApplicationTop";
        Assert.Equal (rs.Toplevel, Application.Running);
        Application.End (rs);

#if DEBUG_IDISPOSABLE
        Assert.True (rs.WasDisposed);
        Assert.False (Application.Running!.WasDisposed); // Is true because the rs.Toplevel is the same as Application.Running
#endif
    ...
    public void Init_Begin_End_Cleans_Up ()
    {
        ...
        Assert.Equal (topLevel, Application.Running);
    ...
    public void Internal_Properties_Correct ()
    {
        ...
        Assert.Null (Application.Running);
        SessionToken rs = Application.Begin (new ());
        Assert.Equal (Application.Running, rs.Toplevel);
    ...
    public void Run_T_After_InitWithDriver_with_TopLevel_Does_Not_Throws ()
    {
        ...
        Assert.True (Application.Running is Window);
    ...
    public void Run_T_After_InitWithDriver_with_TopLevel_and_Driver_Does_Not_Throws ()
    {
        ...
        Assert.True (Application.Running is Dialog);
    ...
    {
        // Init doesn't create a Toplevel and assigned it to Application.Running
        // but Begin does
        ...
        {
            Assert.NotEqual (initTop, Application.Running);
#if DEBUG_IDISPOSABLE
            Assert.False (initTop.WasDisposed);
#endif
            Application.RequestStop ();
    ...
    public void End_Does_Not_Dispose ()
    {
        ...
        Assert.NotNull (Application.Running);
        Assert.Equal (w, Application.Running);
    ...
    public void Run_Creates_Top_Without_Init ()
    {
        ...
        Application.Iteration -= OnApplicationOnIteration;
#if DEBUG_IDISPOSABLE
        Assert.Equal (top, Application.Running);
        Assert.False (top.WasDisposed);
        Exception exception = Record.Exception (Application.Shutdown);
        Assert.NotNull (exception);
        Assert.False (top.WasDisposed);
        ...
        void OnApplicationOnIteration (object s, IterationEventArgs e) { Assert.NotNull (Application.Running); }
    }
    ...
    public void Run_T_With_V2_Driver_Does_Not_Call_ResetState_After_Init ()
    {
        ...
        Assert.NotNull (Application.Driver);
        Assert.NotNull (Application.Running);
        Assert.False (Application.Running!.IsRunning);
        Application.Running!.Dispose ();
        Application.Shutdown ();
    }
