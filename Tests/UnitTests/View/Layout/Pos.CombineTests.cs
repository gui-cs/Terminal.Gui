...
public class PosCombineTests (ITestOutputHelper output)
{
    ...
    {
        Application.Running = new Toplevel () { Width = 80, Height = 25 };
        ...
        win2.Add (view2);
        win1.Add (view1, win2);
        Application.Running.Add (win1);
        Application.Running.Layout ();

        Assert.Equal (new Rectangle (0, 0, 80, 25), Application.Running.Frame);
        ...
        var foundView = Application.Running.GetViewsUnderLocation (new Point(9, 4), ViewportSettingsFlags.None).LastOrDefault ();
    ...
    public void PosCombine_Refs_SuperView_Throws ()
    {
        ...
        f.X = Pos.X (Application.Running) + Pos.X (v2) - Pos.X (v1);
        f.Y = Pos.Y (Application.Running) + Pos.Y (v2) - Pos.Y (v1);

        Application.Running.SubViewsLaidOut += (s, e) =>
        {
            Assert.Equal (0, Application.Running.Frame.X);
            Assert.Equal (0, Application.Running.Frame.Y);
