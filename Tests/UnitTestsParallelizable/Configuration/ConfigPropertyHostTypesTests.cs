...
public class ConfigPropertyHostTypesTests
{
    ...
    public void ConfigPropertyHostTypes_DynamicDependencies_Match_Returned_Types ()
    {
        // Arrange
        MethodInfo getTypes =
            typeof (ConfigPropertyHostTypes).GetMethod (nameof (ConfigPropertyHostTypes.GetTypes),
                                                        BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static)!;

        HashSet<Type> rooted = getTypes.GetCustomAttributes<DynamicDependencyAttribute> ().Select (a => a.Type!).ToHashSet ();
        ...
        Assert.True (missingRoots.Length == 0 && extraRoots.Length == 0,
                     $"ConfigPropertyHostTypes [DynamicDependency] drift detected.\nTypes in array but not rooted by attribute: {
                         string.Join (", ", missingRoots.Select (t => t.FullName))
                     }\nTypes rooted by attribute but not in array: {
                         string.Join (", ", extraRoots.Select (t => t.FullName))
                     }");
    }
    ...
    public void ConfigPropertyHostTypes_GetTypes_Matches_Reflected_Hosts_In_TerminalGui_Assembly ()
    {
        ...
        HashSet<Type> reflected = terminalGuiAssembly.GetTypes ().Where (HasConfigurationProperty).ToHashSet ();
        ...
        Assert.True (missing.Length == 0 && extra.Length == 0,
                     ...
                     }\nExtra (remove from list): {
                         string.Join (", ", extra.Select (t => t.FullName))
                     }");
    }
