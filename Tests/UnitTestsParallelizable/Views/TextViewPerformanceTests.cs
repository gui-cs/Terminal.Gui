...
public class TextViewPerformanceTests
{
    ...
    public void ShouldInvalidateMaxWidthCache_Returns_False_When_Line_Modified_HasMoreLinesWithSame_CachedMaxWidth_Line_On_Delete ()
    {
        ...
        bool shouldInvalidate = model.ShouldInvalidateMaxWidthCache (2, false);
        Assert.False (shouldInvalidate, "Cache should NOT be invalidated when deleting from a line that is not the only cached max width line because there are multiple lines with the same max width");
    }
