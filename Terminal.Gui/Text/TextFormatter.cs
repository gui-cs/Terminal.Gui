...
public class TextFormatter {
	...
	/// <remarks>
	///         <para>
	///                 Used by <see cref="View.AutoSize" /> to resize the view's <see cref="View.Bounds" /> to fit
	///                 <see cref="Size" />.
	///         </para>
	///         <para>
	...
	/// <summary>
	///         Gets the cursor position from <see cref="HotKey" />. If the <see cref="HotKey" /> is defined, the cursor will
	...
	/// <remarks>
	///         <para>
	///                 Upon a 'get' of this property, if the text needs to be formatted (if <see cref="NeedsFormat" /> is
	///                 <c>true</c>)
	///                 <see cref="Format(string, int, bool, bool, bool, int, TextDirection, bool)" /> will be called
	///                 internally.
	///         </para>
	/// </remarks>
	public List<string> Lines {
		get {
			...
			if (string.IsNullOrEmpty (Text) || Size.IsEmpty || Size.Width == 0 || Size.Height == 0) {
			...
			if (NeedsFormat) {
				...
				if (IsVerticalDirection (Direction)) {
					...
					_lines = Format (shown_text, Size.Height,
						VerticalAlignment == VerticalTextAlignment.Justified,
					...
					_lines = Format (shown_text, Size.Width, Alignment == TextAlignment.Justified,
		...
		bool fillRemaining = true, ConsoleDriver driver = null)
	{
		...
		for (var line = lineOffset; line < linesFormated.Count; line++) {
			...
			if (_textAlignment == TextAlignment.Right ||
			    (_textAlignment == TextAlignment.Justified && !IsLeftToRight (Direction))) {
				if (isVertical) {
			...
			} else if (_textVerticalAlignment == VerticalTextAlignment.Top ||
				   _textVerticalAlignment == VerticalTextAlignment.Justified) {
				if (isVertical) {
					y = bounds.Top;
				} else {
					y = bounds.Top + line;
				...
				if (HotKeyPos > -1 && idx == HotKeyPos) {
					...
					if (isVertical) {
						if (runeWidth == 0) {
							...
							var foundIdx =
								lastZeroWidthPos.IndexOf (p => p.Value.Y == current);
							if (foundIdx == -1) {
								current--;
	...
	///         </para>
	///         <para>
	///                 If <paramref name="preserveTrailingSpaces" /> is <see langword="false" /> at most one space will be
	///                 preserved at the end of the last line.
	///         </para>
	/// </remarks>
		...
		TextDirection textDirection = TextDirection.LeftRight_TopBottom)
	{
		...
		if (preserveTrailingSpaces) {
			...
			if (IsHorizontalDirection (textDirection)) {
				...
				while ((end = start + GetLengthThatFits (runes.GetRange (start, runes.Count - start),
					       width, tabWidth)) < runes.Count) {
		...
		if (start < text.GetRuneCount ()) {
			...
			if (IsVerticalDirection (textDirection) || preserveTrailingSpaces ||
			    (!preserveTrailingSpaces && str.GetColumns () <= width)) {
				lines.Add (str);
			}
		}
	...
	/// <summary>
	///         Justifies the text to fill the width provided. Space will be added between words (demarked by spaces and tabs)
	///         to
	...
	///         If <see langword="false" />, forces text to fit a single line. Line breaks are converted to spaces. The text
	///         will be clipped to <paramref name="width" />.
	/// </param>
	/// <param name="preserveTrailingSpaces">
	///         If <see langword="true" /> trailing spaces at the end of wrapped lines will be preserved.
	///         If <see langword="false" />, trailing spaces at the end of wrapped lines will be trimmed.
