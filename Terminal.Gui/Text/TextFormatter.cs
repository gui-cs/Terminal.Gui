...
public class TextFormatter {
    ...
    /// <summary>
    ///     Determines if the bounds width will be used or only the text width will be used, If <see langword="true"/> all
    ///     the bounds area will be filled with whitespaces and the same background color showing a perfect rectangle.
    /// </summary>
    ...
    ///     to make the text just fit <c>width</c>. Spaces will not be added to the ends.
    /// </summary>
    /// <param name="text"></param>
    /// <param name="width"></param>
    ...
    /// <summary>
    ///     Computes the maximum width needed to render the text (single line or multiple lines, word wrapped) given a
    ///     number of columns to constrain the text to.
    /// </summary>
    ...
    public static int GetSumMaxCharWidth (
        List<string> lines,
        int startIndex = -1,
        int length = -1,
        int tabWidth = 0
    ...
    public static int GetLengthThatFits (string text, int columns, int tabWidth = 0) {
        return GetLengthThatFits (text?.ToRuneList (), columns, tabWidth);
    }
