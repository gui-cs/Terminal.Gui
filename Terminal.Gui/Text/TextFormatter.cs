	...
	public class TextFormatter {
		...
		public virtual ustring Text {
			...
			set {
				...
				if (text != null && text.RuneCount > 0 && (Size.Width == 0 || Size.Height == 0 || Size.Width != text.ConsoleWidth)) 					// Provide a default size (width = length of longest line, height = 1)
				...
				if (NeedsFormat) {
					...
					if (IsVerticalDirection (textDirection)) {
						...
						if (!AutoSize) {
							...
							if (lines.Count > colsWidth) 								lines.RemoveRange (colsWidth, lines.Count - colsWidth);
						}
					} else {
						lines = Format (shown_text, Size.Width, textAlignment == TextAlignment.Justified, Size.Height > 1,
							PreserveTrailingSpaces, 0, textDirection);
						if (!AutoSize && lines.Count > Size.Height) 							lines.RemoveRange (Size.Height, lines.Count - Size.Height);
					}
		...
		static ustring StripCRLF (ustring str, bool keepNewLine = false)
		{
			...
			for (int i = 0; i < runes.Count; i++) 				switch (runes [i]) {
				case '\n':
					if (!keepNewLine) 						runes.RemoveAt (i);
					break;
				...
				case '\r':
					if (i + 1 < runes.Count && runes [i + 1] == '\n') {
						...
						if (!keepNewLine) 							runes.RemoveAt (i);
						i++;
					} else 						if (!keepNewLine) 							runes.RemoveAt (i);
					break;
				}
		...
		public static List<ustring> SplitNewLine (ustring text)
		{
			...
			if (runes.Count > 0 && lines.Count == 0) 				lines.Add (ustring.Make (runes));
else if (runes.Count > 0 && start < runes.Count) 				lines.Add (ustring.Make (runes.GetRange (start, runes.Count - start)));
else 				lines.Add (ustring.Make (""));
			return lines;
		}
			...
			TextDirection textDirection = TextDirection.LeftRight_TopBottom)
		{
			...
			if (!preserveTrailingSpaces) 				if (IsHorizontalDirection (textDirection)) 					while ((end = start + Math.Max (GetMaxLengthForWidth (runes.GetRange (start, runes.Count - start), width), 1)) < runes.Count) {
...
else 					while ((end = start + width) < runes.Count) {
						while (runes [end] != ' ' && end > start)
							end--;
						if (end == start)
							end = start + width;
...
else 				while ((end = start) < runes.Count) {
			...
			int GetNextWhiteSpace (int from, int cWidth, out bool incomplete, int cLength = 0)
			{
				...
				while (length < cWidth && to < runes.Count) {
					var rune = runes [to];
					if (IsHorizontalDirection (textDirection)) 						length += Rune.ColumnWidth (rune);
else 						length++;
					if (length > cWidth) {
						if (to >= runes.Count || length > 1 && cWidth <= 1) 							incomplete = true;
						return to;
					}
					if (rune == ' ') 						if (length == cWidth) 							return to + 1;
else if (length > cWidth) 							return to;
else 							return GetNextWhiteSpace (to + 1, cWidth, out incomplete, length);
else if (rune == '\t') {
						length += tabWidth + 1;
						if (length == tabWidth && tabWidth > cWidth) 							return to + 1;
else if (length > cWidth && tabWidth > cWidth) 							return to;
else 							return GetNextWhiteSpace (to + 1, cWidth, out incomplete, length);
					}
					to++;
				}
				if (cLength > 0 && to < runes.Count && runes [to] != ' ' && runes [to] != '\t') 					return from;
else if (cLength > 0 && to < runes.Count && (runes [to] == ' ' || runes [to] == '\t')) 					return lastFrom;
else 					return to;
			}
		...
		public static ustring ClipAndJustify (ustring text, int width, bool justify, TextDirection textDirection = TextDirection.LeftRight_TopBottom)
		{
			...
			if (slen > width) 				if (IsHorizontalDirection (textDirection)) 					return ustring.Make (runes.GetRange (0, GetMaxLengthForWidth (text, width)));
else 					return ustring.Make (runes.GetRange (0, width));
else {
				if (justify) 					return Justify (text, width, ' ', textDirection);
else if (IsHorizontalDirection (textDirection) && GetTextWidth (text) > width) 					return ustring.Make (runes.GetRange (0, GetMaxLengthForWidth (text, width)));
				return text;
			}
		}
		...
		public static ustring Justify (ustring text, int width, char spaceChar = ' ', TextDirection textDirection = TextDirection.LeftRight_TopBottom)
		{
			...
			int textCount;
			if (IsHorizontalDirection (textDirection)) 				textCount = words.Sum (arg => GetTextWidth (arg));
else 				textCount = words.Sum (arg => arg.RuneCount);
			...
			for (int w = 0; w < words.Length; w++) {
				...
				if (w + 1 == words.Length - 1) 					for (int i = 0; i < extras; i++)
						s.Append (spaceChar);
			}
			...
			bool preserveTrailingSpaces = false, int tabWidth = 0, TextDirection textDirection = TextDirection.LeftRight_TopBottom)
		{
			...
			for (int i = 0; i < runeCount; i++) {
				...
				if (c == '\n') {
					...
					foreach (var line in wrappedLines) 						lineResult.Add (ClipAndJustify (line, width, justify, textDirection));
					if (wrappedLines.Count == 0) 						lineResult.Add (ustring.Empty);
					lp = i + 1;
				}
			}
			foreach (var line in WordWrap (ustring.Make (runes.GetRange (lp, runeCount - lp)), width, preserveTrailingSpaces, tabWidth, textDirection)) 				lineResult.Add (ClipAndJustify (line, width, justify, textDirection));
		...
		public static int GetSumMaxCharWidth (ustring text, int startIndex = -1, int length = -1)
		{
			...
			for (int i = startIndex == -1 ? 0 : startIndex; i < (length == -1 ? runes.Length : startIndex + length); i++) 				max += Math.Max (Rune.ColumnWidth (runes [i]), 1);
			return max;
		}
		...
		public static int GetMaxLengthForWidth (ustring text, int width)
		{
			...
			for (; runeIdx < runes.Count; runeIdx++) {
				...
				if (runesLength + runeWidth > width) 					break;
		...
		public static int GetMaxColsForWidth (List<ustring> lines, int width)
		{
			...
			for (; lineIdx < lines.Count; lineIdx++) {
				...
				if (runesLength + maxRruneWidth > width) 					break;
		...
		{
			if (ustring.IsNullOrEmpty (text)) 				return new Rect (new Point (x, y), Size.Empty);
			...
			if (IsHorizontalDirection (direction)) {
				...
				int cols = 0;
				foreach (var rune in text) 					if (rune == '\n') {
						ml++;
						if (cols > mw) 							mw = cols;
						cols = 0;
					} else if (rune != '\r') {
		...
		public static bool FindHotKey (ustring text, Rune hotKeySpecifier, bool firstUpperCase, out int hotPos, out Key hotKey)
		{
			...
			foreach (Rune c in text) {
				if ((char)c != 0xFFFD) 					if (c == hotKeySpecifier) 						hot_pos = i;
else if (hot_pos > -1) {
						hot_key = c;
						break;
					}
			...
			if (hot_pos == -1 && firstUpperCase) {
				i = 0;
				foreach (Rune c in text) {
					if ((char)c != 0xFFFD) 						if (Rune.IsUpper (c)) {
							hot_key = c;
							hot_pos = i;
							break;
						}
		...
		public ustring ReplaceHotKeyWithTag (ustring text, int hotPos)
		{
			// Set the high bit
			var runes = text.ToRuneList ();
			if (Rune.IsLetterOrNumber (runes [hotPos])) 				runes [hotPos] = new Rune ((uint)runes [hotPos]);
			return ustring.Make (runes);
		}
		...
		public void Draw (Rect bounds, Attribute normalColor, Attribute hotColor, Rect containerBounds = default, bool fillRemaining = true)
		{
			...
			if (Application.Driver != null) 				maxBounds = containerBounds == default
					? bounds
			...
			if (maxBounds.Width == 0 || maxBounds.Height == 0) 				return;
			var savedClip = Application.Driver?.Clip;
			if (Application.Driver != null) 				Application.Driver.Clip = maxBounds;
			...
			for (int line = lineOffset; line < linesFormated.Count; line++) {
				...
				if (textAlignment == TextAlignment.Right || textAlignment == TextAlignment.Justified && !IsLeftToRight (textDirection)) 					if (isVertical) {
				...
				} else if (textAlignment == TextAlignment.Centered) 					if (isVertical) {
						var runesWidth = GetSumMaxCharWidth (Lines, line);
				...
				// Vertical Alignment
				if (textVerticalAlignment == VerticalTextAlignment.Bottom || textVerticalAlignment == VerticalTextAlignment.Justified && !IsTopToBottom (textDirection)) 					if (isVertical) 						y = bounds.Bottom - runes.Length;
else 						y = bounds.Bottom - Lines.Count + line;
else if (textVerticalAlignment == VerticalTextAlignment.Top || textVerticalAlignment == VerticalTextAlignment.Justified) 					if (isVertical) 						y = bounds.Top;
else 						y = bounds.Top + line;
else if (textVerticalAlignment == VerticalTextAlignment.Middle) 					if (isVertical) {
				...
				for (var idx = (isVertical ? start - y : start - x) + colOffset; current < start + size; idx++) {
					if (idx < 0 || x + current + colOffset < 0) {
						current++;
						continue;
					} else if (!fillRemaining && idx > runes.Length - 1) 						break;
					...
					if (isVertical) {
						...
						if (idx >= 0 && idx < runes.Length) 							rune = runes [idx];
					} else {
					...
					if (HotKeyPos > -1 && idx == HotKeyPos) {
						...
						!isVertical && textAlignment == TextAlignment.Justified) 							CursorPosition = idx - start;
						Application.Driver?.SetAttribute (hotColor);
						Application.Driver?.AddRune (rune);
						Application.Driver?.SetAttribute (normalColor);
					} else 						Application.Driver?.AddRune (rune);
					var runeWidth = Math.Max (Rune.ColumnWidth (rune), 1);
					if (isVertical) 						current++;
else 						current += runeWidth;
					var nextRuneWidth = idx + 1 > -1 && idx + 1 < runes.Length ? Rune.ColumnWidth (runes [idx + 1]) : 0;
					if (!isVertical && idx + 1 < runes.Length && current + nextRuneWidth > start + size) 						break;
				}
			}
			if (Application.Driver != null) 				Application.Driver.Clip = (Rect)savedClip;
		}
	}
}
