	...
	public partial class CollectionNavigator {
		...
		public int GetNextMatchingItem (int currentIndex, char keyStruck)
		{
			...
			if (!char.IsControl (keyStruck)) {
				...
				if (SearchString.Length > 0 && DateTime.Now - lastKeystroke < TimeSpan.FromMilliseconds (TypingDelay)) 					// "dd" is a candidate
					candidateState = SearchString + keyStruck;
else 					// its a fresh keystroke after some time
				...
				// instead of "can" + 'd').
				if (SearchString.Length > 1 && idxCandidate == -1) 					// ignore it since we're still within the typing delay
		...
		internal int GetNextMatchingItem (int currentIndex, string search, bool minimizeMovement = false)
		{
			...
			if (matchingIndexes.Length > 0) {
				...
				if (currentlySelected == -1) 					// we are not currently selecting any item beginning with the search
					// so jump to first item in list that begins with the letter
					return matchingIndexes [0];
else {

					// the current index is part of the matching collection
					if (minimizeMovement) 						// if we would rather not jump around (e.g. user is typing lots of text to get this match)
		...
		{
			if (Collection == null) 				throw new InvalidOperationException ("Collection is null");
		}
