...
///         <see cref="View.Height"/> objects are all absolute values and are not relative. The position and size of the view is described by
///         <see cref="View.Frame"/>.
///     </para>
///     <para>
...
public enum LayoutStyle
{
    ...
    ///     <see cref="View.Height"/> objects are relative to the <see cref="View.SuperView"/> and are computed at layout time. The position and size of the view will be computed based on these objects at layout time.
    ///     <see cref="View.Frame"/> will provide the absolute computed values.
    /// </summary>
    Computed
}
...
public partial class View
{
    ...
    ///         <see langword="true"/> then <see cref="Width"/> and <see cref="Height"/> will be used if <see cref="Text"/> can fit; if
    ///         <see cref="Text"/> won't fit the view will be resized as needed.
    ///     </para>
    ///     <para>
    ...
    ///     from the <see cref="Margin"/>. The Border provides the space for a visual border (drawn using line-drawing glyphs) and the Title. The Border expands inward; in other words if `Border.Thickness.Top == 2` the border and title will take up the first row and the second row will be filled with spaces.
    /// </summary>
    /// <remarks>
    ///     <para><see cref="BorderStyle"/> provides a simple helper for turning a simple border frame on or off.</para>
    ///     <para>
    ///         The adornments (<see cref="Margin"/>, <see cref="Border"/>, and <see cref="Padding"/>) are not part of the View's content and are not clipped by the View's Clip Area.
    ///     </para>
    ///     <para>
    ///         Changing the size of a frame (<see cref="Margin"/>, <see cref="Border"/>, or <see cref="Padding"/>) will change the size of the
    ///         <see cref="Frame"/> and trigger <see cref="LayoutSubviews"/> to update the layout of the
    ...
    /// <remarks>
    ///     <para>
    ///         If <see cref="LayoutStyle"/> is <see cref="LayoutStyle.Computed"/> the value of Bounds is indeterminate until the view has been initialized (
    ///         <see cref="IsInitialized"/> is true) and <see cref="LayoutSubviews"/> has been called.
    ///     </para>
    ///     <para>
    ...
    ///     </para>
    ///     <para>
    ///         Because <see cref="Bounds"/> coordinates are relative to the upper-left corner of the <see cref="View"/>, the coordinates of the upper-left corner of the rectangle returned by this property are (0,0). Use this property to obtain the size of the area of the view for tasks such as drawing the view's contents.
    ///     </para>
    /// </remarks>
    ...
    ///     <para>
    ///         Setting Frame will set <see cref="X"/>, <see cref="Y"/>, <see cref="Width"/>, and <see cref="Height"/> to the values of the corresponding properties of the
    ///         <paramref name="value"/> parameter.
    ///     </para>
    ///     <para>This causes <see cref="LayoutStyle"/> to be <see cref="LayoutStyle.Absolute"/>.</para>
    ///     <para>
    ///         Altering the Frame will eventually (when the view hierarchy is next laid out via  see cref="LayoutSubviews"/>) cause
    ///         <see cref="LayoutSubview(View, Rect)"/> and <see cref="OnDrawContent(Rect)"/> methods to be called.
    ///     </para>
    /// </remarks>
    public Rect Frame
    ...
    /// <remarks>
    ///     <para>
    ///         If set to a relative value (e.g. <see cref="Dim.Fill(int)"/>) the value is indeterminate until the view has been initialized (
    ///         <see cref="IsInitialized"/> is true) and <see cref="SetRelativeLayout(Rect)"/> has been called.
    ///     </para>
    ///     <para>
    ...
    /// <remarks>
    ///     <para>
    ///         Setting this property to <see cref="LayoutStyle.Absolute"/> will cause <see cref="Frame"/> to determine the size and position of the view.
    ///         <see cref="X"/> and <see cref="Y"/> will be set to <see cref="Dim.DimAbsolute"/> using <see cref="Frame"/>.
    ///     </para>
    ///     <para>
    ...
    ///         <see cref="Y"/> properties are `null` they will be set to <see cref="Pos.PosAbsolute"/> using the current value of
    ///         <see cref="Frame"/>. If either of the <see cref="Width"/> and <see cref="Height"/> properties are `null` they will be set to
    ...
    /// <summary>
    ///     The frame (specified as a <see cref="Thickness"/>) that separates a View from other SubViews of the same SuperView. The margin offsets the
    ///     <see cref="Bounds"/> from the <see cref="Frame"/>.
    /// </summary>
    /// <remarks>
    ///     <para>
    ///         The adornments (<see cref="Margin"/>, <see cref="Border"/>, and <see cref="Padding"/>) are not part of the View's content and are not clipped by the View's Clip Area.
    ///     </para>
    ///     <para>
    ///         Changing the size of an adornment (<see cref="Margin"/>, <see cref="Border"/>, or <see cref="Padding"/>) will change the size of
    ///         <see cref="Frame"/> and trigger <see cref="LayoutSubviews"/> to update the layout of the
    ///         <see cref="SuperView"/> and its <see cref="Subviews"/>.
    ///     </para>
    /// </remarks>
    ...
    ///     <see cref="Width"/>, and <see cref="Height"/> during set operations and in <see cref="LayoutSubviews"/>. If invalid settings are discovered exceptions will be thrown indicating the error. This will impose a performance penalty and thus should only be used for debugging.
    /// </remarks>
    ...
    /// <remarks>
    ///     <para>
    ///         If set to a relative value (e.g. <see cref="Pos.Center"/>) the value is indeterminate until the view has been initialized (
    ///         <see cref="IsInitialized"/> is true) and <see cref="SetRelativeLayout(Rect)"/> has been called.
    ///     </para>
    ///     <para>
    ///         Changing this property will eventually (when the view is next drawn) cause the
    ...
    /// <summary>
    ///     Event called only once when the <see cref="View"/> is being initialized for the first time. Allows configurations and assignments to be performed before the
    ///     <see cref="View"/> being shown. This derived from <see cref="ISupportInitializeNotification"/> to allow notify all the views that are being initialized.
    /// </summary>
    ...
    /// <param name="clamped">
    ///     If <see langword="true"/>, <paramref name="rx"/> and <paramref name="ry"/> will be clamped to the screen dimensions (will never be negative and will always be less than
    ///     <see cref="ConsoleDriver.Cols"/> and <see cref="ConsoleDriver.Rows"/>, respectively.
    /// </param>
    ...
    /// <remarks>
    ///     <para>
    ///         The position and dimensions of the view are indeterminate until the view has been initialized. Therefore, the behavior of this method is indeterminate if
    ...
    /// <summary>
    ///     This internal method is overridden by Adornment to do nothing to prevent recursion during View construction. And, because Adornments don't have Adornments. It's internal to support unit tests.
    /// </summary>
    ...
    /// <summary>
    ///     Sets the internal <see cref="LayoutNeeded"/> flag for this View and all of it's subviews and it's SuperView. The main loop will call SetRelativeLayout and LayoutSubviews for any view with
    ///     <see cref="LayoutNeeded"/> set.
    /// </summary>
    ...
    ///     <see cref="Height"/>) to <see cref="Frame"/>, given a rectangle describing the SuperView's Bounds (nominally the same as
