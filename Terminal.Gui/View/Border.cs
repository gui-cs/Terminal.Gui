...
namespace Terminal.Gui {
	...
	public struct Thickness {
		...
		{
			if (left < 0 || top < 0 || right < 0 || bottom < 0) throw new ArgumentException ("Invalid value for this property.");
	...
	public class Border {
		...
		public sealed class ToplevelContainer : Toplevel {
			...
			public override Border Border {
				get => base.Border;
				set {
					if (base.Border != null && base.Border.Child != null && value.Child == null) value.Child = base.Border.Child;
					base.Border = value;
					if (value == null) return;
					Rect frame;
					if (Border.Child != null && (Border.Child.Width is Dim || Border.Child.Height is Dim)) frame = Rect.Empty;
					else frame = Frame;
			...
			void AdjustContentView (Rect frame)
			{
				...
				if (frame == Rect.Empty) {
					...
					if (Border.Child == null) Border.Child = new ChildContentView (this) {
						X = wp.X,
						Y = wp.Y,
					...
					if (Border.Child == null) Border.Child = new ChildContentView (cFrame, this);
					else Border.Child.Frame = cFrame;
				}
				if (Subviews?.Count == 0)
			...
			public override void Redraw (Rect bounds)
			{
				...
				if (Border.DrawMarginFrame) if (!ustring.IsNullOrEmpty (Border.Title))
			...
			{
				if (Border.Child != null) Border.Child.CanFocus = CanFocus;
		...
		/// <summary>
		/// Specifies the <see cref="BorderStyle"/> for a view.
		/// </summary>
		...
		public int ActualWidth {
			get {
				...
				if (Parent?.Border == null) return Math.Min (Child?.Frame.Width + 2 * marginFrame + Padding.Right
		...
		public int ActualHeight {
			get {
				var driver = Application.Driver;
				if (Parent?.Border == null) return Math.Min (Child?.Frame.Height + 2 * marginFrame + Padding.Bottom
		...
		private void SetMarginFrameTitleBrush ()
		{
			if (child != null) {
				...
				if (view.ColorScheme != null) {
					if (borderBrush == default) BorderBrush = view.GetNormalColor ().Foreground;
					if (background == default) Background = view.GetNormalColor ().Background;
					return;
				}
			}
			BorderBrush = default;
		...
		/// </summary>
		[JsonInclude, JsonConverter (typeof (AttributeJsonConverter))]
		public Attribute? Effect3DBrush {
			get {
				if (effect3DBrush == null && effect3D) return effect3DBrush = new Attribute (Color.Gray, Color.DarkGray);
				else return effect3DBrush;
			}
		...
		///  more the <see cref="BorderStyle"/> and the <see cref="Effect3D"/>.
		/// </summary>
		...
		public void DrawContent (View view = null, bool fill = true)
		{
			...
			if (Parent?.Border != null) DrawParentBorder (Parent.ViewToScreen (Parent.Bounds), fill);
			else DrawChildBorder (Child.ViewToScreen (Child.Bounds), fill);
		}
		...
		public void DrawFullContent ()
		{
			...
			var driver = Application.Driver;
			Rect scrRect;
			if (Parent?.Border != null) scrRect = Parent.ViewToScreen (Parent.Bounds);
			else scrRect = Child.ViewToScreen (Child.Bounds);
			Rect borderRect;
			if (Parent?.Border != null) borderRect = scrRect;
			else borderRect = new Rect () {
			...
			if (Effect3D) {
				...
				for (int r = effectBorder.Y; r < Math.Min (effectBorder.Bottom, driver.Rows); r++) for (int c = effectBorder.X; c < Math.Min (effectBorder.Right, driver.Cols); c++)
			...
			if (DrawMarginFrame) {
				...
				if (borderRect.Width > 0 && borderRect.Height > 0) driver.DrawWindowFrame (borderRect, 1, 1, 1, 1, BorderStyle != BorderStyle.None, fill: true, this);
			}
		...
		private void DrawChildBorder (Rect frame, bool fill = true)
		{
			...
			for (int r = frame.Bottom + drawMarginFrame + padding.Bottom;
				r < Math.Min (frame.Bottom + drawMarginFrame + sumThickness.Bottom, driver.Rows); r++) for (int c = frame.X - drawMarginFrame - sumThickness.Left;
					c < Math.Min (frame.Right + drawMarginFrame + sumThickness.Right, driver.Cols); c++)
					AddRuneAt (driver, c, r, ' ');
			...
			for (int r = frame.Y - drawMarginFrame;
				r < Math.Min (frame.Bottom + drawMarginFrame, driver.Rows); r++) for (int c = frame.X - drawMarginFrame - padding.Left;
					c < frame.X - drawMarginFrame; c++)
					AddRuneAt (driver, c, r, ' ');

			// Draw the right Padding
			for (int r = frame.Y - drawMarginFrame;
				r < Math.Min (frame.Bottom + drawMarginFrame, driver.Rows); r++) for (int c = frame.Right + drawMarginFrame;
					c < Math.Min (frame.Right + drawMarginFrame + padding.Right, driver.Cols); c++)
					AddRuneAt (driver, c, r, ' ');

			// Draw the lower Padding
			for (int r = frame.Bottom + drawMarginFrame;
				r < Math.Min (frame.Bottom + drawMarginFrame + padding.Bottom, driver.Rows); r++) for (int c = frame.X - drawMarginFrame - padding.Left;
					c < Math.Min (frame.Right + drawMarginFrame + padding.Right, driver.Cols); c++)
					AddRuneAt (driver, c, r, ' ');
			...
			if (Effect3D) {
				...
				for (int r = frame.Y - drawMarginFrame - sumThickness.Top + effect3DOffset.Y;
					r >= 0 && r < frame.Y - drawMarginFrame - sumThickness.Top; r++) for (int c = frame.X - drawMarginFrame - sumThickness.Left + effect3DOffset.X;
						c >= 0 && c < Math.Min (frame.Right + drawMarginFrame + sumThickness.Right + effect3DOffset.X, driver.Cols); c++)
						AddRuneAt (driver, c, r, (Rune)driver.Contents [r, c, 0]);

				// Draw the left Effect3D
				for (int r = frame.Y - drawMarginFrame - sumThickness.Top + effect3DOffset.Y;
					r >= 0 && r < Math.Min (frame.Bottom + drawMarginFrame + sumThickness.Bottom + effect3DOffset.Y, driver.Rows); r++) for (int c = frame.X - drawMarginFrame - sumThickness.Left + effect3DOffset.X;
						c >= 0 && c < frame.X - drawMarginFrame - sumThickness.Left; c++)
						AddRuneAt (driver, c, r, (Rune)driver.Contents [r, c, 0]);

				// Draw the right Effect3D
				for (int r = frame.Y - drawMarginFrame - sumThickness.Top + effect3DOffset.Y;
					r >= 0 && r < Math.Min (frame.Bottom + drawMarginFrame + sumThickness.Bottom + effect3DOffset.Y, driver.Rows); r++) for (int c = frame.Right + drawMarginFrame + sumThickness.Right;
						c >= 0 && c < Math.Min (frame.Right + drawMarginFrame + sumThickness.Right + effect3DOffset.X, driver.Cols); c++)
						AddRuneAt (driver, c, r, (Rune)driver.Contents [r, c, 0]);
		...
		private void DrawParentBorder (Rect frame, bool fill = true)
		{
			...
			for (int r = Math.Max (frame.Bottom - borderThickness.Bottom, frame.Y);
				r < Math.Min (frame.Bottom, driver.Rows); r++) for (int c = frame.X;
					c < Math.Min (frame.Right, driver.Cols); c++)
					AddRuneAt (driver, c, r, ' ');
			...
			for (int r = Math.Max (frame.Bottom - sumThickness.Bottom, frame.Y + borderThickness.Top);
				r < Math.Min (frame.Bottom - borderThickness.Bottom, driver.Rows); r++) for (int c = frame.X + borderThickness.Left;
					c < Math.Min (frame.Right - borderThickness.Right, driver.Cols); c++)
					AddRuneAt (driver, c, r, ' ');
			...
			if (Effect3D) {
				...
				for (int r = Math.Max (frame.Y + effect3DOffset.Y, 0);
					r < frame.Y; r++) for (int c = Math.Max (frame.X + effect3DOffset.X, 0);
						c < Math.Min (frame.Right + effect3DOffset.X, driver.Cols); c++)
						AddRuneAt (driver, c, r, (Rune)driver.Contents [r, c, 0]);

				// Draw the left Effect3D
				for (int r = Math.Max (frame.Y + effect3DOffset.Y, 0);
					r < Math.Min (frame.Bottom + effect3DOffset.Y, driver.Rows); r++) for (int c = Math.Max (frame.X + effect3DOffset.X, 0);
						c < frame.X; c++)
						AddRuneAt (driver, c, r, (Rune)driver.Contents [r, c, 0]);

				// Draw the right Effect3D
				for (int r = Math.Max (frame.Y + effect3DOffset.Y, 0);
					r < Math.Min (frame.Bottom + effect3DOffset.Y, driver.Rows); r++) for (int c = frame.Right;
						c < Math.Min (frame.Right + effect3DOffset.X, driver.Cols); c++)
						AddRuneAt (driver, c, r, (Rune)driver.Contents [r, c, 0]);
		...
		public void DrawTitle (View view)
		{
			...
			if (DrawMarginFrame) {
				driver.SetAttribute (new Attribute (BorderBrush, Background));
				if (view.HasFocus) driver.SetAttribute (new Attribute (Child.ColorScheme.HotNormal.Foreground, Background));
		...
		public void DrawTitle (View view, Rect rect)
		{
			...
			if (DrawMarginFrame) {
				driver.SetAttribute (new Attribute (BorderBrush, Background));
				if (view.HasFocus) driver.SetAttribute (new Attribute (view.ColorScheme.HotNormal.Foreground, Background));
