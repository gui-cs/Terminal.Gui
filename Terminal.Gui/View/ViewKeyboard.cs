...
public partial class View
{
    ...
    /// <summary>
    ///     Gets or sets the hot key defined for this view. Pressing the hot key on the keyboard while this view has focus will invoke the
    ///     <see cref="Command.Default"/> and <see cref="Command.Accept"/> commands. <see cref="Command.Default"/> causes the view to be focused and
    ///     <see cref="Command.Accept"/> does nothing. By default, the HotKey is automatically set to the first character of
    ///     <see cref="Text"/> that is prefixed with with <see cref="HotKeySpecifier"/>.
    ///     <para>
    ...
    ///         modifier. For selecting items within a View that are not Views themselves, the keypress can be key without the Alt modifier. For example, in a Dialog, a Button with the text of "_Text" can be selected with Alt-T. Or, in a
    ///         <see cref="Menu"/> with "_File _Edit", Alt-F will select (show) the "_File" menu. If the "_File" menu has a sub-menu of "_New" `Alt-N` or `N` will ONLY select the "_New" sub-menu if the "_File" menu is already opened.
    ///     </para>
    /// </summary>
    /// <remarks>
    ...
    ///     </para>
    ///     <para>
    ///         By default, when the HotKey is set to <see cref="Key.A"/> through <see cref="KeyCode.Z"/> key bindings will be added for both the un-shifted and shifted versions. This means if the HotKey is
    ///         <see cref="Key.A"/>, key bindings for <c>Key.A</c> and <c>Key.A.WithShift</c> will be added. This behavior can be overriden by overriding
    ///         <see cref="AddKeyBindingsForHotKey"/>.
    ///     </para>
    ...
    /// <summary>
    ///     Adds key bindings for the specified HotKey. Useful for views that contain multiple items that each have their own HotKey such as
    ///     <see cref="RadioGroup"/>.
    /// </summary>
    /// <remarks>
    ///     <para>
    ///         By default key bindings are added for both the base key (e.g. <see cref="Key.D3"/>) and the Alt-shifted key (e.g.
    ///         <c>Key.D3.WithAlt</c> This behavior can be overriden by overriding <see cref="AddKeyBindingsForHotKey"/>.
    ///     </para>
    ///     <para>
    ///         By default, when <paramref name="hotKey"/> is <see cref="Key.A"/> through <see cref="Key.Z"/> key bindings will be added for both the un-shifted and shifted versions. This means if the HotKey is
    ///         <see cref="Key.A"/>, key bindings for <c>Key.A</c> and <c>Key.A.WithShift</c> will be added. This behavior can be overriden by overriding
    ///         <see cref="AddKeyBindingsForHotKey"/>.
    ///     </para>
    ///     <para>
    ...
    /// <summary>
    ///     Gets or sets whether the view is a stop-point for keyboard navigation of focus. Will be <see langword="true"/> only if the
    ///     <see cref="CanFocus"/> is also <see langword="true"/>. Set to <see langword="false"/> to prevent the view from being a stop-point for keyboard navigation.
    /// </summary>
    /// <remarks>
    ///     The default keyboard navigation keys are <c>Key.Tab</c> and <c>Key>Tab.WithShift</c>. These can be changed by modifying the key bindings (see
    ///     <see cref="KeyBindings.Add(Key, Command[])"/>) of the SuperView.
    /// </remarks>
    public bool TabStop
    {
        get => _tabStop;
    ...
    /// <summary>If the view is enabled, processes a new key down event and returns <see langword="true"/> if the event was handled.</summary>
    /// <remarks>
    ///     <para>If the view has a sub view that is focused, <see cref="NewKeyDownEvent"/> will be called on the focused view first.</para>
    ///     <para>
    ///         If the focused sub view does not handle the key press, this method calls <see cref="OnKeyDown"/> to allow the view to pre-process the key press. If
    ///         <see cref="OnKeyDown"/> returns <see langword="false"/>, this method then calls
    ...
    /// <summary>
    ///     Low-level API called when the user presses a key, allowing a view to pre-process the key down event. This is called from
    ///     <see cref="NewKeyDownEvent"/> before <see cref="OnInvokingKeyBindings"/>.
    /// </summary>
    /// <param name="keyEvent">Contains the details about the key that produced the event.</param>
    /// <returns>
    ///     <see langword="false"/> if the key press was not handled. <see langword="true"/> if the keypress was handled and no other view should see it.
    /// </returns>
    /// <remarks>
    ///     <para>
    ...
    ///     <see cref="OnKeyDown"/> before <see cref="OnInvokingKeyBindings"/>. Set <see cref="Key.Handled"/> to true to stop the key from being processed by other views.
    /// </summary>
    /// <remarks>
    ///     <para>Not all terminals support key distinct up notifications, Applications should avoid depending on distinct KeyUp events.</para>
    ///     <para>See <see href="../docs/keyboard.md">for an overview of Terminal.Gui keyboard APIs.</see></para>
    /// </remarks>
    public event EventHandler<Key> KeyDown;

    /// <summary>
    ///     Low-level API called when the user presses a key, allowing views do things during key down events. This is called from
    ///     <see cref="NewKeyDownEvent"/> after <see cref="OnInvokingKeyBindings"/>.
    /// </summary>
    /// <param name="keyEvent">Contains the details about the key that produced the event.</param>
    /// <returns>
    ///     <see langword="false"/> if the key press was not handled. <see langword="true"/> if the keypress was handled and no other view should see it.
    /// </returns>
    /// <remarks>
    ///     <para>Override <see cref="OnProcessKeyDown"/> to override the behavior of how the base class processes key down events.</para>
    ///     <para>
    ///         For processing <see cref="HotKey"/>s and commands, use <see cref="Command"/> and
    ///         <see cref="KeyBindings.Add(Key, Command[])"/>instead.
    ///     </para>
    ///     <para>Fires the <see cref="ProcessKeyDown"/> event.</para>
    ///     <para>Not all terminals support distinct key up notifications; applications should avoid depending on distinct KeyUp events.</para>
    /// </remarks>
    ...
    /// <remarks>
    ///     <para>
    ///         SubViews can use the <see cref="ProcessKeyDown"/> of their super view override the default behavior of when key bindings are invoked.
    ///     </para>
    ///     <para>Not all terminals support distinct key up notifications; applications should avoid depending on distinct KeyUp events.</para>
    ///     <para>See <see href="../docs/keyboard.md">for an overview of Terminal.Gui keyboard APIs.</see></para>
    /// </remarks>
    ...
    /// <summary>
    ///     If the view is enabled, processes a new key up event and returns <see langword="true"/> if the event was handled. Called before
    ///     <see cref="NewKeyDownEvent"/>.
    /// </summary>
    /// <remarks>
    ///     <para>
    ...
    ///     </para>
    ///     <para>If the view has a sub view that is focused, <see cref="NewKeyUpEvent"/> will be called on the focused view first.</para>
    ///     <para>If the focused sub view does not handle the key press, this method calls <see cref="OnKeyUp"/>, which is cancellable.</para>
    ///     <para>See <see href="../docs/keyboard.md">for an overview of Terminal.Gui keyboard APIs.</see></para>
    /// </remarks>
    /// <param name="keyEvent"></param>
    /// <returns><see langword="true"/> if the event was handled.</returns>
    ...
    /// <param name="keyEvent">Contains the details about the key that produced the event.</param>
    /// <returns><see langword="false"/> if the key stroke was not handled. <see langword="true"/> if no other view should see it.</returns>
    /// <remarks>
    ///     Not all terminals support key distinct down/up notifications, Applications should avoid depending on distinct KeyUp events.
    ///     <para>
    ...
    ///     <remarks>
    ///         Not all terminals support key distinct down/up notifications, Applications should avoid depending on distinct KeyDown and KeyUp events and instead should use
    ///         <see cref="KeyDown"/>.
    ///         <para>See <see href="../docs/keyboard.md">for an overview of Terminal.Gui keyboard APIs.</see></para>
    ///     </remarks>
    /// </summary>
    ...
    /// <summary>
    ///     Invoked when a key is pressed that may be mapped to a key binding. Set <see cref="Key.Handled"/> to true to stop the key from being processed by other views.
    /// </summary>
    ...
    /// <returns>
    ///     <see langword="null"/> if no command was bound the <paramref name="keyEvent"/>. <see langword="true"/> if commands were invoked and at least one handled the command.
    ...
    /// <returns>
    ///     <see langword="null"/> if no command was found. <see langword="true"/> if the command was invoked and it handled the command.
    ...
    ///     </para>
    ///     <para>
    ///         If <see cref="AddCommand"/> has already been called for <paramref name="command"/> <paramref name="f"/> will replace the old one.
    ///     </para>
    /// </summary>
