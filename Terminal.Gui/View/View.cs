...
namespace Terminal.Gui {
	...
	///    Rect parameter to specify the absolute position and size (the View.<see cref="Frame "/>). To create a View 
	...
	public partial class View : Responder, ISupportInitializeNotification {
		...
		public virtual Key HotKey {
			get => hotKey;
			set {
				if (hotKey != value) hotKey = TextFormatter.HotKey = value == Key.Unknown ? Key.Null : value;
			}
		}
		...
		public Key Shortcut {
			...
			set {
				if (shortcutHelper.Shortcut != value && (ShortcutHelper.PostShortcutValidation (value) || value == Key.Null)) shortcutHelper.Shortcut = value;
			}
		}
		...
		public override bool CanFocus {
			get => base.CanFocus;
			set {
				if (!addingView && IsInitialized && SuperView?.CanFocus == false && value) throw new InvalidOperationException ("Cannot set CanFocus to true if the SuperView CanFocus is false!");
				if (base.CanFocus != value) {
					...
					if (!value && SuperView?.Focused == this) SuperView.focused = null;
					if (!value && HasFocus) {
						SetHasFocus (false, this);
						SuperView?.EnsureFocus ();
						if (SuperView != null && SuperView.Focused == null) {
							SuperView.FocusNext ();
							if (SuperView.Focused == null) Application.Current.FocusNext ();
							Application.EnsuresTopOnFront ();
						}
					}
					if (subviews != null && IsInitialized) foreach (var view in subviews) if (view.CanFocus != value) if (!value) {
		...
		/// <summary>
		/// Gets or sets the X position for the view (the column). Only used if the <see cref="LayoutStyle"/> is <see cref="LayoutStyle.Computed"/>.
		/// </summary>
		...
		public Pos X {
			get => x;
			set {
				if (ForceValidatePosDim && !ValidatePosDim (x, value)) throw new ArgumentException ();
		...
		/// <summary>
		/// Gets or sets the Y position for the view (the row). Only used if the <see cref="LayoutStyle"/> is <see cref="LayoutStyle.Computed"/>.
		/// </summary>
		...
		public Pos Y {
			get => y;
			set {
				if (ForceValidatePosDim && !ValidatePosDim (y, value)) throw new ArgumentException ();
		...
		public Dim Width {
			get => width;
			set {
				if (ForceValidatePosDim && !ValidatePosDim (width, value)) throw new ArgumentException ("ForceValidatePosDim is enabled", nameof (Width));
				...
				if (ForceValidatePosDim) {
					...
					if (IsAdded && autoSize && !isValidNewAutSize) throw new InvalidOperationException ("Must set AutoSize to false before set the Width.");
				}
				OnResizeNeeded ();
			}
		}
		...
		public Dim Height {
			get => height;
			set {
				if (ForceValidatePosDim && !ValidatePosDim (height, value)) throw new ArgumentException ("ForceValidatePosDim is enabled", nameof (Height));
				...
				if (ForceValidatePosDim) {
					...
					if (IsAdded && autoSize && !isValidNewAutSize) throw new InvalidOperationException ("Must set AutoSize to false before set the Height.");
				}
				OnResizeNeeded ();
			}
		}
		...
		{
			if (!IsInitialized || layoutStyle == LayoutStyle.Absolute || oldValue == null || oldValue.GetType () == newValue.GetType () || this is Toplevel) return true;
			if (layoutStyle == LayoutStyle.Computed) if (oldValue.GetType () != newValue.GetType () && !(newValue is Pos.PosAbsolute || newValue is Dim.DimAbsolute)) return true;
			return false;
		}
		...
		public bool GetMinimumBounds (out Size size)
		{
			...
			if (!AutoSize && !ustring.IsNullOrEmpty (TextFormatter.Text)) switch (TextFormatter.IsVerticalDirection (TextDirection)) {
				case true:
		...
		/// <summary>
		/// Gets or sets the <see cref="TextFormatter"/> which can be handled differently by any derived class.
		/// </summary>
		...
		/// Use <see cref="View"/> to initialize a View with  <see cref="LayoutStyle"/> of <see cref="LayoutStyle.Computed"/> 
		/// </remarks>
		...
		///   coordinates. The initial size (<see cref="Frame"/>) will be 
		...
		///   coordinates with the given string. The size (<see cref="Frame"/>) will be 
		///   adjusted to fit the contents of <see cref="Text"/>, including newlines ('\n') for multiple lines. 
		/// </para>
		/// <para>
		...
		/// <para>
		///   The <see cref="View"/> will be created at the given
		///   coordinates with the given string. The initial size (<see cref="Frame"/>) will be 
		///   adjusted to fit the contents of <see cref="Text"/>, including newlines ('\n') for multiple lines. 
		/// </para>
		/// <para>
		...
		internal void SetNeedsLayout ()
		{
			...
			foreach (var view in Subviews) view.SetNeedsLayout ();
		...
		public virtual void Remove (View view)
		{
			...
			foreach (var v in subviews) if (v.Frame.IntersectsWith (touched))
		...
		public virtual void PositionCursor ()
		{
			...
			if (focused == null && SuperView != null) SuperView.EnsureFocus ();
			else if (focused?.Visible == true && focused?.Enabled == true && focused?.Frame.Width > 0 && focused.Frame.Height > 0) focused.PositionCursor ();
			else if (focused?.Visible == true && focused?.Enabled == false) focused = null;
			else if (CanFocus && HasFocus && Visible && Frame.Width > 0 && Frame.Height > 0) Move (TextFormatter.HotKeyPos == -1 ? 0 : TextFormatter.CursorPosition, 0);
			else Move (frame.X, frame.Y);
		}
		...
		public virtual ColorScheme ColorScheme {
			get {
				if (colorScheme == null) return SuperView?.ColorScheme;
				return colorScheme;
			}
			set {
		...
		public virtual void Redraw (Rect bounds)
		{
			...
			if (ColorScheme != null) Driver.SetAttribute (HasFocus ? GetFocusColor () : GetNormalColor ());

			var boundsAdjustedForBorder = Bounds;
			if (!IgnoreBorderPropertyOnRedraw && Border != null) Border.DrawContent (this);
			...
			if (subviews != null) foreach (var view in subviews) if (!view.NeedDisplay.IsEmpty || view.ChildNeedsDisplay || view.LayoutNeeded) {
						if (view.Frame.IntersectsWith (clipRect) && (view.Frame.IntersectsWith (bounds) || bounds.X < 0 || bounds.Y < 0)) {
							if (view.LayoutNeeded) view.LayoutSubviews ();
		...
		Rect GetContainerBounds ()
		{
			...
			if (containerBounds.X + containerBounds.Width > driverClip.X + driverClip.Width) containerBounds.Width = Math.Max (containerBounds.Width + lenOffset, 0);
			else containerBounds.Width = Math.Min (containerBounds.Width, driverClip.Width);
			lenOffset = driverClip.Y + driverClip.Height - (containerBounds.Y + containerBounds.Height);
			if (containerBounds.Y + containerBounds.Height > driverClip.Y + driverClip.Height) containerBounds.Height = Math.Max (containerBounds.Height + lenOffset, 0);
			else containerBounds.Height = Math.Min (containerBounds.Height, driverClip.Height);
		...
		protected bool? InvokeKeybindings (KeyEvent keyEvent)
		{
			...
			if (KeyBindings.ContainsKey (keyEvent.Key))
				foreach (var command in KeyBindings [keyEvent.Key]) {
					...
					if (!CommandImplementations.ContainsKey (command)) throw new NotSupportedException ($"A KeyBinding was set up for the command {command} ({keyEvent.Key}) but that command is not supported by this View ({GetType ().Name})");

					// each command has its own return value
					var thisReturn = CommandImplementations [command] ();

					// if we haven't got anything yet, the current command result should be used
					if (toReturn == null) toReturn = thisReturn;

					// if ever see a true then that's what we will return
					if (thisReturn ?? false) toReturn = true;
				}
		...
		{
			if (command.Length == 0) throw new ArgumentException ("At least one command must be specified", nameof (command));

			if (KeyBindings.ContainsKey (key)) KeyBindings [key] = command;
			else KeyBindings.Add (key, command);
		}
		...
		{
			foreach (var kvp in KeyBindings.Where (kvp => kvp.Value.SequenceEqual (command)).ToArray ()) KeyBindings.Remove (kvp.Key);
		}
		...
		protected void AddCommand (Command command, Func<bool?> f)
		{
			...
			if (CommandImplementations.ContainsKey (command))                               // replace that implementation
				CommandImplementations [command] = f;
			else                            // else record how to perform the action (this should be the normal case)
		...
		{
			if (focused == null && subviews?.Count > 0) if (FocusDirection == Direction.Forward) FocusFirst ();
				else FocusLast ();
		}
		...
		public void FocusFirst ()
		{
			...
			foreach (var view in tabIndexes) if (view.CanFocus && view.tabStop && view.Visible && view.Enabled) {
					SetFocus (view);
					return;
				}
		}
		...
		internal void SetRelativeLayout (Rect superviewFrame)
		{
			...
			if (AutoSize)                           // Note this is global to this function and used as such within the local functions defined
			...
			(int newLocation, int newDimension) GetNewLocationAndDimension (int superviewLocation, int superviewDimension, Pos pos, Dim dim, int autosizeDimension)
			{
				...
				switch (pos) {
				case Pos.PosCenter:
					if (dim == null) newDimension = AutoSize ? autosizeDimension : superviewDimension;
					else {
				...
				case Pos.PosCombine combine:
					...
					if (combine.add) newLocation = left + right;
					else newLocation = left - right;
			...
			int CalculateNewDimension (Dim d, int location, int dimension, int autosize)
			{
				...
				case Dim.DimCombine combine:
					...
					if (combine.add) newDimension = leftNewDim + rightNewDim;
					else newDimension = leftNewDim - rightNewDim;
			...
			if (Frame != r) {
				...
				if (!SetMinWidthHeight ()) TextFormatter.Size = GetSizeNeededForTextAndHotKey ();
			}
		}
		...
		internal static List<View> TopologicalSort (View superView, IEnumerable<View> nodes, ICollection<(View From, View To)> edges)
		{
			...
			while (noEdgeNodes.Any ()) {
				...
				foreach (var e in edges.Where (e => e.From.Equals (n)).ToArray ()) {
					...
					if (edges.All (me => !me.To.Equals (m)) && m != superView)                                              // insert m into S
						noEdgeNodes.Add (m);
				}
			}
			...
			if (edges.Any ()) {
				...
				if (from != superView?.GetTopSuperView (to, from)) if (!ReferenceEquals (from, to)) if (ReferenceEquals (from.SuperView, to)) throw new InvalidOperationException ($"ComputedLayout for \"{superView}\": \"{to}\" references a SubView (\"{from}\").");
						else throw new InvalidOperationException ($"ComputedLayout for \"{superView}\": \"{from}\" linked with \"{to}\" was not found. Did you forget to add it to {superView}?");
					else throw new InvalidOperationException ($"ComputedLayout for \"{superView}\": A recursive cycle was found in the relative Pos/Dim of the SubViews.");
			}
		...
		public virtual void LayoutSubviews ()
		{
			...
			foreach (var v in ordered) LayoutSubview (v, Frame);
		...
		{
			if (v.LayoutStyle == LayoutStyle.Computed) v.SetRelativeLayout (hostFrame);
		...
		/// or not when <see cref="TextFormatter.WordWrap"/> is enabled. If <see langword="true"/> 
		/// any trailing spaces will be trimmed when either the <see cref="Text"/> property is changed or 
		/// when <see cref="TextFormatter.WordWrap"/> is set to <see langword="true"/>.
			...
			set {
				if (TextFormatter.Direction != value) {
					...
					if (!ForceValidatePosDim && directionChanged && AutoSize
					    || ForceValidatePosDim && directionChanged && AutoSize && isValidOldAutSize) OnResizeNeeded ();
					else if (directionChanged && IsAdded) {
						SetWidthHeight (Bounds.Size);
						SetMinWidthHeight ();
					} else SetMinWidthHeight ();
					TextFormatter.Size = GetSizeNeededForTextAndHotKey ();
					SetNeedsDisplay ();
				}
			}
		...
		/// <summary>
		/// Get or sets whether the view will use <see cref="Gui.View.Border"/> (if <see cref="Border"/> is set) to draw 
		/// a border. If <see langword="false"/> (the default),
		/// <see cref="Redraw(Rect)"/> will call <see cref="Border.DrawContent(View, bool)"/>
		...
		bool ResizeView (bool autoSize)
		{
			...
			if (nBoundsSize != Bounds.Size) if (ForceValidatePosDim) aSize = SetWidthHeight (nBoundsSize);
				else Bounds = new Rect (Bounds.X, Bounds.Y, nBoundsSize.Width, nBoundsSize.Height);
		...
		bool SetWidthHeight (Size nBounds)
		{
			...
			if (aSize) Bounds = new Rect (Bounds.X, Bounds.Y, canSizeW ? rW : Bounds.Width, canSizeH ? rH : Bounds.Height);
		...
		/// <returns>The number of characters required for the <see cref="TextFormatter.HotKeySpecifier"/>. If the text direction specified
		/// by <see cref="TextDirection"/> does not match the <paramref name="isWidth"/> parameter, <c>0</c> is returned.</returns>
		public int GetHotKeySpecifierLength (bool isWidth = true)
		{
			if (isWidth) return TextFormatter.IsHorizontalDirection (TextDirection) &&
				    TextFormatter.Text?.Contains (HotKeySpecifier) == true
				    ? Math.Max (Rune.ColumnWidth (HotKeySpecifier), 0) : 0;
			else return TextFormatter.IsVerticalDirection (TextDirection) &&
							    TextFormatter.Text?.Contains (HotKeySpecifier) == true
							    ? Math.Max (Rune.ColumnWidth (HotKeySpecifier), 0) : 0;
		}
		...
		/// <summary>
		/// Gets the dimensions required for <see cref="Text"/> accounting for a <see cref="TextFormatter.HotKeySpecifier"/> .
		/// </summary>
		/// <returns></returns>
		public Size GetSizeNeededForTextAndHotKey ()
		{
			if (ustring.IsNullOrEmpty (TextFormatter.Text)) return Bounds.Size;
		...
		public virtual void BeginInit ()
		{
			...
			if (subviews?.Count > 0) foreach (var view in subviews) if (!view.IsInitialized) view.BeginInit ();
		}
		...
		{
			IsInitialized = true;
			if (subviews != null) foreach (var view in subviews) if (!view.IsInitialized) view.EndInit ();
		...
		bool CanBeVisible (View view)
		{
			...
			for (var c = view.SuperView; c != null; c = c.SuperView) if (!c.Visible) return false;
		...
		/// Determines the current <see cref="ColorScheme"/> based on the <see cref="Enabled"/> value.
		/// </summary>
		/// <returns><see cref="ColorScheme.Focus"/> if <see cref="Enabled"/> is <see langword="true"/>
		/// or <see cref="ColorScheme.Disabled"/> if <see cref="Enabled"/> is <see langword="false"/>.
		/// If it's overridden can return other values.</returns>
		...
		/// Determines the current <see cref="ColorScheme"/> based on the <see cref="Enabled"/> value.
		/// </summary>
		/// <returns><see cref="ColorScheme.HotNormal"/> if <see cref="Enabled"/> is <see langword="true"/>
		/// or <see cref="ColorScheme.Disabled"/> if <see cref="Enabled"/> is <see langword="false"/>.
		/// If it's overridden can return other values.</returns>
