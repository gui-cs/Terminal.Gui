...
///             <term>SubView</term>
///             <description>
///                 A View that is contained in another view and will be rendered as part of the containing view's ContentArea. SubViews are added to another view via the
///                 <see cref="View.Add(View)"/>` method. A View may only be a SubView of a single View.
///             </description>
///         </item>
///         <item>
...
///     </para>
///     <para>
///         Views that are focusable should implement the <see cref="PositionCursor"/> to make sure that the cursor is placed in a location that makes sense. Unix terminals do not have a way of hiding the cursor, so it can be distracting to have the cursor left at the last focused view. So views should make sure that they place the cursor in a visually sensible place.
///     </para>
///     <para>
...
///     </para>
///     <para>
///         View supports two layout styles: <see cref="LayoutStyle.Absolute"/> or <see cref="LayoutStyle.Computed"/>. The style is determined by the values of
///         <see cref="X"/>, <see cref="Y"/>, <see cref="Width"/>, and <see cref="Height"/>. If any of these is set to non-absolute
///         <see cref="Pos"/> or <see cref="Dim"/> object, then the layout style is <see cref="LayoutStyle.Computed"/>. Otherwise it is
///         <see cref="LayoutStyle.Absolute"/> .
///     </para>
///     <para>
///         To create a View using Absolute layout, call a constructor that takes a Rect parameter to specify the absolute position and size or simply set
///         <see cref="View.Frame "/>). To create a View using Computed layout use a constructor that does not take a Rect parameter and set the X, Y, Width and Height properties on the view to non-absolute values. Both approaches use coordinates that are relative to the
///         <see cref="Bounds"/> of the <see cref="SuperView"/> the View is added to.
///     </para>
///     <para>
///         Computed layout is more flexible and supports dynamic console apps where controls adjust layout as the terminal resizes or other Views change size or position. The
///         <see cref="X"/>, <see cref="Y"/>, <see cref="Width"/>, and <see cref="Height"/> properties are
///         <see cref="Dim"/> and <see cref="Pos"/> objects that dynamically update the position of a view. The X and Y properties are of type
///         <see cref="Pos"/> and you can use either absolute positions, percentages, or anchor points. The Width and Height properties are of type
///         <see cref="Dim"/> and can use absolute position, percentages, and anchors. These are useful as they will take care of repositioning views when view's adornments are resized or if the terminal size changes.
///     </para>
///     <para>
///         Absolute layout requires specifying coordinates and sizes of Views explicitly, and the View will typically stay in a fixed position and size. To change the position and size use the
///         <see cref="Frame"/> property.
///     </para>
///     <para>
///         Subviews (child views) can be added to a View by calling the <see cref="Add(View)"/> method. The container of a View can be accessed with the
///         <see cref="SuperView"/> property.
///     </para>
///     <para>
///         To flag a region of the View's <see cref="Bounds"/> to be redrawn call <see cref="SetNeedsDisplay(Rect)"/>. To flag the entire view for redraw call
///         <see cref="SetNeedsDisplay()"/>.
///     </para>
///     <para>
///         The <see cref="LayoutSubviews"/> method is invoked when the size or layout of a view has changed. The default processing system will keep the size and dimensions for views that use the
///         <see cref="LayoutStyle.Absolute"/>, and will recompute the Adornments for the views that use
///         <see cref="LayoutStyle.Computed"/>.
///     </para>
///     <para>
///         Views have a <see cref="ColorScheme"/> property that defines the default colors that subviews should use for rendering. This ensures that the views fit in the context where they are being used, and allows for themes to be plugged in. For example, the default colors for windows and Toplevels uses a blue background, while it uses a white background for dialog boxes and a red background for errors.
///     </para>
///     <para>
...
///         <see cref="ISupportInitialize.BeginInit"/> and <see cref="ISupportInitialize.EndInit"/> which will be called when the view is added to a
///         <see cref="SuperView"/>.
///     </para>
///     <para>
///         If first-run-only initialization is preferred, overrides to <see cref="ISupportInitializeNotification"/> can be implemented, in which case the
...
///         <see cref="View"/> inheritance hierarchies to override base class layout code optimally by doing so only on first run, instead of on every run.
///     </para>
...
public partial class View : Responder, ISupportInitializeNotification
{
    ...
    /// <summary>
    ///     Event fired when the <see cref="View.Title"/> is changing. Set <see cref="TitleEventArgs.Cancel"/> to `true` to cancel the Title change.
    /// </summary>
    ...
    /// <remarks>
    ///     <para>
    ///         Use <see cref="X"/>, <see cref="Y"/>, <see cref="Width"/>, and <see cref="Height"/> properties to dynamically control the size and location of the view. The
    ///         <see cref="View"/> will be created using <see cref="LayoutStyle.Absolute"/> coordinates. The initial size (
    ...
    ///         <see cref="X"/>, <see cref="Y"/>, <see cref="Width"/>, and <see cref="Height"/> properties to dynamically control the size and location of the view, changing it to
    ///         <see cref="LayoutStyle.Computed"/>.
    ///     </para>
    /// </remarks>
    public View ()
    ...
    ///         <see cref="ISupportInitialize.BeginInit"/> and <see cref="ISupportInitialize.EndInit"/> which will be called when the
    ///         <see cref="SuperView"/> is initialized.
    ///     </para>
    ///     <para>
    ///         If first-run-only initialization is preferred, overrides to <see cref="ISupportInitializeNotification"/> can be implemented too, in which case the
    ///         <see cref="ISupportInitialize"/> methods will only be called if
    ///         <see cref="ISupportInitializeNotification.IsInitialized"/> is <see langword="false"/>. This allows proper
    ///         <see cref="View"/> inheritance hierarchies to override base class layout code optimally by doing so only on first run, instead of on every run.
    ///     </para>
