...
/// <remarks>
/// <para>
/// This interface enables storing heterogeneous runnables in collections (e.g., <see cref="IApplication.RunnableSessionStack"/>)
...
public interface IRunnable
{
    ...
    /// <summary>
    /// Gets whether this runnable session is currently running (i.e., on the <see cref="IApplication.RunnableSessionStack"/>).
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// Runnables are added to the stack during <see cref="IApplication.Begin"/> and removed in <see cref="IApplication.End"/>.
    /// </para>
    /// </remarks>
    bool IsRunning { get; }
    ...
    /// <summary>
    /// Raised when <see cref="IsRunning"/> is changing (e.g., when <see cref="IApplication.Begin"/> or <see cref="IApplication.End"/> is called).
    ...
    /// <summary>
    /// Raised after <see cref="IsRunning"/> has changed (after the runnable has been added to or removed from the <see cref="IApplication.RunnableSessionStack"/>).
    /// </summary>
    /// <remarks>
    /// <para>
    /// Subscribe to this event to perform post-state-change logic. When <see cref="EventArgs{T}.CurrentValue"/> is <see langword="true"/>,
    /// the runnable has started and is on the stack. When <see langword="false"/>, the runnable has stopped and been removed from the stack.
    /// </para>
    /// <para>
    /// This event follows the Terminal.Gui Cancellable Work Pattern (CWP).
    /// </para>
    ...
    /// When <see cref="CancelEventArgs{T}.NewValue"/> is <see langword="true"/>, the runnable is becoming modal (top of stack).
    ...
    /// When <see cref="EventArgs{T}.CurrentValue"/> is <see langword="true"/>, the runnable became modal (top of stack).
