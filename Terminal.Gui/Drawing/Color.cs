...
public readonly partial record struct Color : ISpanParsable<Color>, IUtf8SpanParsable<Color>, ISpanFormattable,
    ...
    public static Dictionary<ColorName, string> Colors {
        ...
        set {
            ...
            static ColorName GetColorToNameMapValue (KeyValuePair<ColorName, string> kvp) =>
                Enum.TryParse<ColorName> (kvp.Key.ToString (), true, out var colorName)
                    ? colorName
    ...
    /// <summary>
    /// Gets the <see cref="Color"/> using a legacy 16-color <see cref="ColorName"/> value. <see langword="get"/> will return
    /// the closest 16
    /// color match to the true color when no exact value is found.
    /// </summary>
    /// <remarks>
    /// Get returns the <see cref="GetClosestNamedColor"/> of the closest 24-bit color value. Set sets the RGB value using a
    /// hard-coded map.
    /// </remarks>
    ...
    /// The <see cref="GetClosestNamedColor"/> to check if this <see cref="Color"/> is closer to than any other configured
    /// named color.
    /// </param>
    /// <returns>
    /// <see langword="true"/> if the closest named color is the provided value.
    /// <br/>
    /// <see langword="false"/> if any other named color is closer to this <see cref="Color"/> than
    /// <paramref name="namedColor"/>.
    /// </returns>
    /// <remarks>
    ...
    internal static ColorName GetClosestNamedColor (Color inputColor) =>
        ColorExtensions.ColorToNameMap.MinBy (pair => CalculateColorDistance (inputColor, pair.Key)).Value;
