...
public readonly partial record struct Color {
    ...
    /// <br/>
    /// If not null, must implement <see cref="ICustomColorFormatter"/> or will be ignored and
    /// <see cref="CultureInfo.InvariantCulture"/> will
    /// be used.
    /// </param>
    /// <returns>
    /// A <see cref="Color"/> value equivalent to <paramref name="text"/>, if parsing was successful.
    ...
    /// </param>
    /// <param name="formatProvider">
    /// An optional <see cref="IFormatProvider"/> to use when formatting the <see cref="Color"/> using custom format strings
    /// not specified for
    /// this method. Provides this instance as <see cref="Argb"/>.
    /// <br/>
    /// If this parameter is not null, the specified <see cref="IFormatProvider"/> will be used instead of the custom
    /// formatting provided by the
    /// <see cref="Color"/> type.
    /// <para/>
    ...
    ///         <description>
    ///         General/default format - Returns a named <see cref="Color"/> if there is a match, or a 24-bit/3-byte/6-hex
    ///         digit string in
    ///         "#RRGGBB" format.
    ///         </description>
    ///     </item>
    ...
    ///         <term>G</term>
    ///         <description>
    ///         Extended general format - Returns a named <see cref="Color"/> if there is a match, or a 32-bit/4-byte/8-hex
    ///         digit string in
    ///         "#AARRGGBB" format.
    ///         </description>
    ///     </item>
    ...
    ///         <term>d</term>
    ///         <description>
    ///         Decimal format - Returns a 3-component decimal representation of the <see cref="Color"/> in "rgb(R,G,B)"
    ///         format.
    ///         </description>
    ///     </item>
    ///     <item>
    ...
    /// </list>
    /// <para>
    /// If <paramref name="formatProvider"/> is provided and is a non-null <see cref="ICustomColorFormatter"/>, the following
    /// behaviors are
    /// available, for the specified values of <paramref name="formatString"/>:
    /// <list type="bullet">
    ///     <listheader>
    ///         <term>Value</term> <description>Result</description>
    ///     </listheader>
    ...
    ///         with the null string, and <see cref="R"/>, <see cref="G"/>, <see cref="B"/>, and <see cref="A"/> as typed
    ///         arguments of type
    ///         <see cref="Byte"/>.
    ///         </description>
    ///     </item>
    ...
    public string ToString (
        ...
        return (formatString, formatProvider) switch {
                   ...
                   (null or { Length: 0 }, { }) when !Equals (formatProvider, CultureInfo.InvariantCulture) => string
                       .Format (
                                formatProvider,
    ...
    ///     <see cref="ToString(string?,System.IFormatProvider?)"/>, as this method results in an intermediate allocation of
    ///     one or more instances
    ///     of <see langword="string"/> and a copy of that string to <paramref name="destination"/> if formatting was
    ///     successful.
    ///     <br/>
    ///     When possible, use <see cref="ToString(string?,System.IFormatProvider?)"/>, which attempts to avoid intermediate
    ///     allocations.
    ///     </para>
    ///     <para>
    ///     This method only returns <see langword="true"/> and with its output written to <paramref name="destination"/> if
    ///     the formatted
    ///     string, <i>in its entirety</i>, will fit in <paramref name="destination"/>. If the resulting formatted string is
    ///     too large to fit in
