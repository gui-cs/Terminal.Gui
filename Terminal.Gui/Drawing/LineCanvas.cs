...
namespace Terminal.Gui {
	...
	public class LineCanvas {
		...
		public Dictionary<Point, Rune> GenerateImage (Rect inArea)
		{
			...
			for (int y = inArea.Y; y < inArea.Y + inArea.Height; y++) for (int x = inArea.X; x < inArea.X + inArea.Width; x++) {
					...
					if (rune != null) map.Add (new Point (x, y), rune.Value);
				}
		...
		private abstract class IntersectionRuneResolver {
			...
			public Rune? GetRuneForIntersects (ConsoleDriver driver, IntersectionDefinition [] intersects)
			{
				...
				if (doubleHorizontal) return doubleVertical ? doubleBoth : doubleH;

				if (doubleVertical) return doubleV;
		...
		private Rune? GetRuneForIntersects (ConsoleDriver driver, IntersectionDefinition [] intersects)
		{
			...
			if (runeResolvers.ContainsKey (runeType)) return runeResolvers [runeType].GetRuneForIntersects (driver, intersects);
		...
		private IntersectionRuneType GetRuneTypeForIntersects (IntersectionDefinition [] intersects)
		{
			...
			if (Has (set,
				...
				IntersectionType.StartRight,
				IntersectionType.StartUp,
				IntersectionType.StartDown)) return IntersectionRuneType.Crosshair;
			#endregion
			...
			if (Exactly (set,
				IntersectionType.StartRight,
				IntersectionType.StartDown)) return IntersectionRuneType.ULCorner;

			if (Exactly (set,
				IntersectionType.StartLeft,
				IntersectionType.StartDown)) return IntersectionRuneType.URCorner;

			if (Exactly (set,
				IntersectionType.StartUp,
				IntersectionType.StartLeft)) return IntersectionRuneType.LRCorner;

			if (Exactly (set,
				IntersectionType.StartUp,
				IntersectionType.StartRight)) return IntersectionRuneType.LLCorner;
			#endregion Corner Conditions

			#region T Conditions
			if (Has (set,
				IntersectionType.PassOverHorizontal,
				IntersectionType.StartDown)) return IntersectionRuneType.TopTee;
			if (Has (set,
				IntersectionType.StartRight,
				IntersectionType.StartLeft,
				IntersectionType.StartDown)) return IntersectionRuneType.TopTee;
			...
			if (Has (set,
				IntersectionType.PassOverHorizontal,
				IntersectionType.StartUp)) return IntersectionRuneType.BottomTee;
			if (Has (set,
				IntersectionType.StartRight,
				IntersectionType.StartLeft,
				IntersectionType.StartUp)) return IntersectionRuneType.BottomTee;
			...
			if (Has (set,
				IntersectionType.PassOverVertical,
				IntersectionType.StartRight)) return IntersectionRuneType.LeftTee;
			if (Has (set,
				IntersectionType.StartRight,
				IntersectionType.StartDown,
				IntersectionType.StartUp)) return IntersectionRuneType.LeftTee;


			if (Has (set,
				IntersectionType.PassOverVertical,
				IntersectionType.StartLeft)) return IntersectionRuneType.RightTee;
			if (Has (set,
				IntersectionType.StartLeft,
				IntersectionType.StartDown,
				IntersectionType.StartUp)) return IntersectionRuneType.RightTee;
			#endregion

			if (All (intersects, Orientation.Horizontal)) return IntersectionRuneType.HLine;

			if (All (intersects, Orientation.Vertical)) return IntersectionRuneType.VLine;
		...
		class StraightLine {
			...
			private IntersectionDefinition IntersectsHorizontally (int x, int y)
			{
				if (Start.Y != y) return null;
					...
					if (EndsAt (x, y))
						...
						if (xmin < x && xmax > x) return new IntersectionDefinition (
							new Point (x, y),
							IntersectionType.PassOverHorizontal,
							this
							);
			...
			private IntersectionDefinition IntersectsVertically (int x, int y)
			{
				if (Start.X != x) return null;
					...
					if (EndsAt (x, y))
						...
						if (ymin < y && ymax > y) return new IntersectionDefinition (
							new Point (x, y),
							IntersectionType.PassOverVertical,
							this
							);
			...
			{
				if (Length == 0) return typeWhenZero;
			...
			{
				if (Orientation == Orientation.Horizontal) return Start.X + Length == x && Start.Y == y;
