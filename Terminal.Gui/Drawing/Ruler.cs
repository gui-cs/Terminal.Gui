    ...
    public class Ruler {
        ...
        public void Draw (Point location, int start = 0) {
            ...
            if (Orientation == Orientation.Horizontal) {
                var hrule =
                    _hTemplate.Repeat ((int)Math.Ceiling ((double)Length + 2 / (double)_hTemplate.Length))
                        [start..(Length + start)];
                ...
                var vrule =
                    _vTemplate.Repeat ((int)Math.Ceiling ((double)(Length + 2) / (double)_vTemplate.Length))
                        [start..(Length + start)];
