...
namespace Terminal.Gui{
	...
	public static class EscSeqUtils {
		...
		public static ConsoleKeyInfo GetConsoleInputKey (ConsoleKeyInfo consoleKeyInfo)
		{
			...
			switch ((uint)keyChar) {
			case 0:
				if (consoleKeyInfo.Key == (ConsoleKey)64) 					newConsoleKeyInfo = new ConsoleKeyInfo (' ', ConsoleKey.Spacebar,
		...
		public static void DecodeEscSeq (EscSeqReqProc escSeqReqProc, ref ConsoleKeyInfo newConsoleKeyInfo, ref ConsoleKey key, ConsoleKeyInfo [] cki, ref ConsoleModifiers mod, out string c1Control, out string code, out string [] values, out string terminating, out bool isKeyMouse, out List<MouseFlags> buttonState, out Point pos, out bool isReq, Action<MouseFlags, Point> continuousButtonPressedHandler)
		{
			...
			case "ESC":
				if (values == null && string.IsNullOrEmpty (terminating)) {
					...
					if (cki [1].KeyChar >= 97 && cki [1].KeyChar <= 122) 						key = (ConsoleKey)cki [1].KeyChar.ToString ().ToUpper () [0];
else 						key = (ConsoleKey)cki [1].KeyChar;
					newConsoleKeyInfo = new ConsoleKeyInfo ((char)key,
			...
			case "CSI":
				...
				key = GetConsoleKey (terminating [0], values [0], ref mod);
				if (key != 0 && values.Length > 1) 					mod |= GetConsoleModifiers (values [1]);
				newConsoleKeyInfo = new ConsoleKeyInfo ('\0',
					key,
					(mod & ConsoleModifiers.Shift) != 0,
					(mod & ConsoleModifiers.Alt) != 0,
		...
		{
			if (kChar == null || kChar.Length == 0) 				return (null, null, null, null);
			if (kChar [0] != '\x1b') 				throw new InvalidOperationException ("Invalid escape character!");
			if (kChar.Length == 1) 				return ("ESC", null, null, null);
			if (kChar.Length == 2) 				return ("ESC", null, null, kChar [1].ToString ());
			...
			for (int i = 2; i < kChar.Length; i++) {
				var c = kChar [i];
				if (char.IsDigit (c)) 					values [valueIdx] += c.ToString ();
else if (c == ';') 					valueIdx++;
else if (valueIdx == nSep - 1 || i == kChar.Length - 1) 					terminating += c.ToString ();
		...
		public static void GetMouse (ConsoleKeyInfo [] cki, out List<MouseFlags> mouseFlags, out Point pos, Action<MouseFlags, Point> continuousButtonPressedHandler)
		{
			...
			for (int i = 0; i < kChar.Length; i++) {
				var c = kChar [i];
				if (c == '<') 					foundButtonCode = true;
else if (foundButtonCode && c != ';') 					value += c.ToString ();
else if (c == ';') {
					if (foundButtonCode) {
						foundButtonCode = false;
						buttonCode = int.Parse (value);
					}
					if (foundPoint == 1) 						pos.X = int.Parse (value) - 1;
					value = "";
					foundPoint++;
				} else if (foundPoint > 0 && c != 'm' && c != 'M') 					value += c.ToString ();
else if (c == 'm' || c == 'M') {
			...
			if (!isButtonClicked && !isButtonDoubleClicked && (buttonState == MouseFlags.Button1Pressed || buttonState == MouseFlags.Button2Pressed ||
				...
				} else if (mouseFlags [0] == MouseFlags.ReportMousePosition) 					isButtonPressed = false;
			...
			//} 
				...
				if (isButtonTripleClicked) 					isButtonTripleClicked = false;
...
else if (buttonState == MouseFlags.ReportMousePosition) 				mouseFlags [0] = MouseFlags.ReportMousePosition;
else 				mouseFlags [0] = buttonState;
		...
		private static async Task ProcessContinuousButtonPressedAsync (MouseFlags mouseFlag, Action<MouseFlags, Point> continuousButtonPressedHandler)
		{
			while (isButtonPressed) {
				...
				if (view == null)
					break;
				if (isButtonPressed && lastMouseButtonPressed != null && (mouseFlag & MouseFlags.ReportMousePosition) == 0) 					Application.MainLoop.Invoke (() => continuousButtonPressedHandler (mouseFlag, point));
			}
		}
		...
		public static Process GetParentProcess (Process process)
		{
			...
			using (var mos = new ManagementObjectSearcher (query)) 				foreach (ManagementObject mo in mos.Get ()) 					if (mo ["ParentProcessId"] != null) 						try {
