	...
	public class EscSeqReqProc {
		...
		public void Add (string terminating, int numOfReq = 1)
		{
			lock (EscSeqReqStats) {
				...
				if (found == null) 					EscSeqReqStats.Add (new EscSeqReqStatus (terminating, numOfReq));
else if (found != null && found.NumOutstanding < found.NumRequests) 					found.NumOutstanding = Math.Min (found.NumOutstanding + numOfReq, found.NumRequests);
			}
		}
		...
		public void Remove (string terminating)
		{
			lock (EscSeqReqStats) {
				...
				if (found != null && found.NumOutstanding == 0) 					EscSeqReqStats.Remove (found);
else if (found != null && found.NumOutstanding > 0) {
					found.NumOutstanding--;
					if (found.NumOutstanding == 0) 						EscSeqReqStats.Remove (found);
				}
			}
		}
		...
		public bool Requested (string terminating)
		{
			lock (EscSeqReqStats) {
				...
				if (found != null && found.NumOutstanding > 0) 					return true;
else 					EscSeqReqStats.Remove (found);
				return false;
			}
		}
	}
