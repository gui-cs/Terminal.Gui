...
/// <remarks>
///     <para>
///     This class provides a high-level abstraction with helper methods and properties for common keyboard operations. Use
///     this class
...
///                 return a copy of the Key with the shift modifier applied. This
...
///                 copy of the Key with the shift modifier removed. This
...
///                 corresponding to
///                 the un-shifted A to Z keys on a keyboard. Properties are provided for these (e.g. <see cref="Key.A"/>,
///                 <see cref="Key.B"/>, etc.).
///                 Even though the encoded values are the same as the ASCII values for uppercase characters, these enum
///                 values represent *lowercase*, un-shifted characters.
///                 </description>
///             </item>
///             <item>
///                 <term>Persistence as strings</term>
///                 <description>
///                 Keys are persisted as <c>"[Modifiers]+[Key]</c>. For example <c>new Key(Key.Delete).WithAlt.WithDel</c>
///                 is persisted as <c>"Ctrl+Alt+Delete"</c>. See <see cref="ToString()"/>
...
public class Key : EventArgs, IEquatable<Key> {
    ...
    public Key (string str) {
        var result = TryParse (str, out Key key);
        if (!result) {
    ...
    static readonly Dictionary<string, KeyCode> _modifierDict =
