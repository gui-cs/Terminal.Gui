...
namespace Terminal.Gui {
	...
	/// <summary>
	/// Defines the event arguments for keyboard input.
	/// </summary>
	public class KeyEventArgs : EventArgs {
		...
		/// <summary>
		/// Indicates if the current event has already been processed and the driver should stop notifying any other event subscriber.
		...
		/// <summary>
		/// Pretty prints the KeyEventArgs
		/// </summary>
		/// <returns></returns>
		public override string ToString ()
		{
			...
			msg += $"{((Key)KeyValue != Key.Unknown && ((uint)this.KeyValue & (uint)Key.CharMask) > 27 ? $"{(char)KeyValue}" : $"{Key}")}";
