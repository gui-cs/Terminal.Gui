	...
	public class ShortcutHelper {
		...
		public static ustring GetShortcutTag (Key shortcut, ustring delimiter = null)
		{
			...
			for (int i = 0; i < keys.Length; i++) {
				...
				if (key == Key.AltMask.ToString () || key == Key.ShiftMask.ToString () || key == Key.CtrlMask.ToString ()) 					continue;
				if (!tag.IsEmpty) 					tag += delimiter;
				if (!key.Contains ("F") && key.Length > 2 && keys.Length == 1) {
					k = (uint)Key.AltMask + k;
					tag += ((char)k).ToString ();
				} else if (key.Length == 2 && key.StartsWith ("D")) 					tag += ((char)key.ElementAt (1)).ToString ();
else 					tag += key;
			}
		...
		public static ustring GetKeyToString (Key key, out Key knm)
		{
			...
			knm &= ~mK;
			for (uint i = (uint)Key.F1; i < (uint)Key.F12; i++) 				if (knm == (Key)i) 					mK |= (Key)i;
			knm &= ~mK;
		...
		public static Key GetShortcutFromTag (ustring tag, ustring delimiter = null)
		{
			...
			for (int i = 0; i < keys.Length; i++) {
				...
				if (k == "Ctrl") 					//hasCtrl = true;
					key |= Key.CtrlMask;
else if (k == "Shift") 					key |= Key.ShiftMask;
else if (k == "Alt") 					key |= Key.AltMask;
				...
				} else 					key |= (Key)Enum.Parse (typeof (Key), k.ToString ());
			}
		...
		{
			for (uint i = (uint)first; i < (uint)last; i++) 				if ((key | (Key)i) == key) 					return true;
			return false;
		}
		...
		{
			if ((key & (Key.CtrlMask | Key.ShiftMask | Key.AltMask)) == 0 && !CheckKeysFlagRange (key, Key.F1, Key.F12)) 				return false;
		...
		public static bool PostShortcutValidation (Key key)
		{
			...
			if (CheckKeysFlagRange (key, Key.F1, Key.F12) ||
				(key & (Key.CtrlMask | Key.ShiftMask | Key.AltMask)) != 0 && knm != Key.Null && knm != Key.Unknown) 				return true;
			return false;
		}
		...
		public static bool FindAndOpenByShortcut (KeyEvent kb, View view = null)
		{
			...
			foreach (var v in view.Subviews) {
				if (v.Shortcut != Key.Null && v.Shortcut == (Key)key) {
					...
					if (action != null) 						Application.MainLoop.AddIdle (() => {
							action ();
							return false;
						});
					return true;
