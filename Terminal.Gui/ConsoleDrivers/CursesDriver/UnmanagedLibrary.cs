...
namespace Unix.Terminal {
    ...
    internal class UnmanagedLibrary {
        ...
        public UnmanagedLibrary (string[] libraryPathAlternatives, bool isFullPath) {
            ...
            if (this.handle == IntPtr.Zero) {
                throw new IOException (
                                       $"Error loading native library \"{string.Join (", ", libraryPathAlternatives)}\"");
            }
        }
        ...
        public T GetNativeMethodDelegate<T> (string methodName)
            ...
            if (ptr == IntPtr.Zero) {
                throw new MissingMethodException (
                                                  string.Format (
                                                                 "The native method \"{0}\" does not exist",
                                                                 methodName));
            }
        ...
        static class Windows {
            ...
            [DllImport ("kernel32.dll")] internal static extern IntPtr GetProcAddress (IntPtr hModule, string procName);
        }

        static class Linux {
            [DllImport ("libdl.so")] internal static extern IntPtr dlopen (string filename, int flags);
            [DllImport ("libdl.so")] internal static extern IntPtr dlsym (IntPtr handle, string symbol);
        }

        static class MacOSX {
            [DllImport ("libSystem.dylib")] internal static extern IntPtr dlopen (string filename, int flags);
            [DllImport ("libSystem.dylib")] internal static extern IntPtr dlsym (IntPtr handle, string symbol);
        }
        ...
        /// </summary>
        static class Mono {
            [DllImport ("__Internal")] internal static extern IntPtr dlopen (string filename, int flags);
            [DllImport ("__Internal")] internal static extern IntPtr dlsym (IntPtr handle, string symbol);
        }
        ...
        static class CoreCLR {
            ...
            static CoreCLR () => NativeLibrary.SetDllImportResolver (
                                                                     ...
                                                                     (
                                                                         ...
                                                                         libraryName == "libcoreclr.so"
                                                                             ? (IntPtr)(-1)
                                                                             : IntPtr.Zero);

            [DllImport ("libcoreclr.so")] internal static extern IntPtr dlopen (string filename, int flags);
            [DllImport ("libcoreclr.so")] internal static extern IntPtr dlsym (IntPtr handle, string symbol);
        }
    }
}
