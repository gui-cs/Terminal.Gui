...
public abstract class ConsoleDriver
{
    ...
    public HashSet<Region> Clip
    {
        ...
        set
        {
            ...
            if (value.Count == 1)
            {
                ...
                _clip = [new (Region.Intersect (value, Screen))];
            }
            else
            {
                _clip = value;
    ...
    {
        rect = Region.Intersect (Clip, rect);
        lock (Contents)
        {
    ...
    {
        return col >= 0 && row >= 0 && col < Cols && row < Rows && Region.Contains (Clip, col, row);
    }
