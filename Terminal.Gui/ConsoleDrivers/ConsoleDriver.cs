...
namespace Terminal.Gui {
	...
	public struct Attribute {
		...
		{
			if (Application.Driver == null) 				// Create the attribute, but show it's not been initialized
		...
		{
			return $"Attribute: {Value} Foreground: {Foreground} Background: {Background}";
		}
	}
	...
	public class ColorScheme : IEquatable<ColorScheme> {
		...
		public Attribute Normal {
			get { return _normal; }
			set {
				if (!value.HasValidColors) 					return;
				_normal = value;
			}
		}
		...
		internal void Initialize ()
		{
			...
			// the attributes
			if (!_normal.Initialized) 				_normal = new Attribute (_normal.Foreground, _normal.Background);
			if (!_focus.Initialized) 				_focus = new Attribute (_focus.Foreground, _focus.Background);
			if (!_hotNormal.Initialized) 				_hotNormal = new Attribute (_hotNormal.Foreground, _hotNormal.Background);
			if (!_hotFocus.Initialized) 				_hotFocus = new Attribute (_hotFocus.Foreground, _hotFocus.Background);
			if (!_disabled.Initialized) 				_disabled = new Attribute (_disabled.Foreground, _disabled.Background);
		}
	}
	...
	public static class Colors {
		private class SchemeNameComparerIgnoreCase : IEqualityComparer<string> {
			...
			{
				if (x != null && y != null) 					return string.Equals (x, y, StringComparison.InvariantCultureIgnoreCase);
				return false;
			}
		...
		/// <remarks>
		/// <para>
		///	This API will be deprecated in the future. Use <see cref="ColorSchemes"/> instead (e.g. <c>edit.ColorScheme = Colors.ColorSchemes["TopLevel"];</c>
		/// </para>
		/// </remarks>
		...
		/// <remarks>
		/// <para>
		///	This API will be deprecated in the future. Use <see cref="ColorSchemes"/> instead (e.g. <c>edit.ColorScheme = Colors.ColorSchemes["Base"];</c>
		/// </para>
		/// </remarks>
		...
		/// <remarks>
		/// <para>
		///	This API will be deprecated in the future. Use <see cref="ColorSchemes"/> instead (e.g. <c>edit.ColorScheme = Colors.ColorSchemes["Dialog"];</c>
		/// </para>
		/// </remarks>
		...
		/// <remarks>
		/// <para>
		///	This API will be deprecated in the future. Use <see cref="ColorSchemes"/> instead (e.g. <c>edit.ColorScheme = Colors.ColorSchemes["Menu"];</c>
		/// </para>
		/// </remarks>
		...
		/// <remarks>
		/// <para>
		///	This API will be deprecated in the future. Use <see cref="ColorSchemes"/> instead (e.g. <c>edit.ColorScheme = Colors.ColorSchemes["Error"];</c>
		/// </para>
		/// </remarks>
	...
	public abstract class ConsoleDriver {
		...
		{
			if (c <= 0x1F || c >= 0X7F && c <= 0x9F) 				// ASCII (C0) control characters.
			...
			int paddingBottom = 0, bool border = true, bool fill = false, Border borderContent = null)
		{
			...
			int fwidth = region.Width - (paddingRight + paddingLeft);

			// fheight is count of vLine chars
			int fheight = region.Height - (paddingBottom + paddingTop);
...
else 				hLine = vLine = uRCorner = uLCorner = lLCorner = lRCorner = clearChar;

			// Outside top
			if (paddingTop > 1) 				for (int r = region.Y; r < ftop; r++) 					for (int c = region.X; c < region.X + region.Width; c++) 						AddRuneAt (c, r, topChar);

			// Outside top-left
			for (int c = region.X; c < fleft; c++) 				AddRuneAt (c, ftop, leftChar);

			// Frame top-left corner
			AddRuneAt (fleft, ftop, paddingTop >= 0 ? paddingLeft >= 0 ? uLCorner : hLine : leftChar);

			// Frame top
			for (int c = fleft + 1; c < fleft + 1 + fwidth; c++) 				AddRuneAt (c, ftop, paddingTop > 0 ? hLine : topChar);

			// Frame top-right corner
			if (fright > fleft) 				AddRuneAt (fright, ftop, paddingTop >= 0 ? paddingRight >= 0 ? uRCorner : hLine : rightChar);

			// Outside top-right corner
			for (int c = fright + 1; c < fright + paddingRight; c++) 				AddRuneAt (c, ftop, rightChar);

			// Left, Fill, Right
			if (fbottom > ftop) {
				for (int r = ftop + 1; r < fbottom; r++) {
					// Outside left
					for (int c = region.X; c < fleft; c++) 						AddRuneAt (c, r, leftChar);

					// Frame left
					AddRuneAt (fleft, r, paddingLeft > 0 ? vLine : leftChar);

					// Fill
					if (fill) 						for (int x = fleft + 1; x < fright; x++) 							AddRuneAt (x, r, clearChar);

					// Frame right
					if (fright > fleft) {
						var v = vLine;
						if ((Diagnostics & DiagnosticFlags.FrameRuler) == DiagnosticFlags.FrameRuler) 							v = (char)('0' + (r - ftop) % 10); // vLine;
						AddRuneAt (fright, r, paddingRight > 0 ? v : rightChar);
					}

					// Outside right
					for (int c = fright + 1; c < fright + paddingRight; c++) 						AddRuneAt (c, r, rightChar);
				}

				// Outside Bottom
				for (int c = region.X; c < region.X + region.Width; c++) 					AddRuneAt (c, fbottom, leftChar);
				...
				if (fright > fleft) {
					...
					for (int c = fleft + 1; c < fright; c++) {
						var h = hLine;
						if ((Diagnostics & DiagnosticFlags.FrameRuler) == DiagnosticFlags.FrameRuler) 							h = (char)('0' + (c - fleft) % 10); // hLine;
				...
				// Outside right
				for (int c = fright + 1; c < fright + paddingRight; c++) 					AddRuneAt (c, fbottom, rightChar);
			}

			// Out bottom - ensure top is always drawn if we overlap
			if (paddingBottom > 0) 				for (int r = fbottom + 1; r < fbottom + paddingBottom; r++) 					for (int c = region.X; c < region.X + region.Width; c++) 						AddRuneAt (c, r, bottomChar);
		}
		...
		public void InitalizeColorSchemes (bool supportsColors = true)
		{
			...
			foreach (var s in Colors.ColorSchemes) 				s.Value.Initialize ();
	...
	internal static class ClipboardProcessRunner {
		...
		public static (int exitCode, string result) Process (string cmd, string arguments, string input = null, bool waitForOutput = true)
		{
			...
			using (var process = new Process {
				...
				if (process.ExitCode > 0) 					output = $@"Process failed to run. Command line: {cmd} {arguments}.
										Output: {output}
