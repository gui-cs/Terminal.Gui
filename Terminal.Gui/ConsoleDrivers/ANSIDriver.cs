...
internal class WindowsANSIConsole
{
    ...
    {
        _inputHandle = new (PInvoke.GetStdHandle (STD_HANDLE.STD_INPUT_HANDLE), false);
        _outputHandle = new (PInvoke.GetStdHandle (STD_HANDLE.STD_OUTPUT_HANDLE), false);
    ...
    private readonly CONSOLE_MODE _originalConsoleMode;
    ...
    {
        Span<INPUT_RECORD> records = new (new INPUT_RECORD [128]);
    ...
    public bool SetCursorPosition (COORD position) { return PInvoke.SetConsoleCursorPosition (_outputHandle, position); }
    ...
    public bool SetCursorVisibility (CursorVisibility visibility)
    {
        ...
        if (_currentCursorVisibility.HasValue == false || _currentCursorVisibility.Value != visibility)
        {
            ...
            {
                stringBuilder.Append (CSI_ShowCursor);

                switch (visibility)
                {
                    case CursorVisibility.Box:
                        stringBuilder.Append (CSI_SetCursorStyle (DECSCUSR_Style.BlinkingBlock));

                        break;
                    case CursorVisibility.BoxFix:
                        stringBuilder.Append (CSI_SetCursorStyle (DECSCUSR_Style.SteadyBlock));

                        break;

                    case CursorVisibility.Vertical:
                        stringBuilder.Append (CSI_SetCursorStyle (DECSCUSR_Style.BlinkingBar));

                        break;
                    case CursorVisibility.VerticalFix:
                        stringBuilder.Append (CSI_SetCursorStyle (DECSCUSR_Style.SteadyBar));

                        break;

                    case CursorVisibility.UnderlineFix:
                        stringBuilder.Append (CSI_SetCursorStyle (DECSCUSR_Style.SteadyUnderline));

                        break;

                    case CursorVisibility.Underline:
                        stringBuilder.Append (CSI_SetCursorStyle (DECSCUSR_Style.BlinkingUnderline));

                        break;

                    case CursorVisibility.Default:
                    default:
                        stringBuilder.Append (CSI_SetCursorStyle (DECSCUSR_Style.UserShape));
            ...
            {
                stringBuilder.Append (CSI_HideCursor);
            }
    ...
    public bool Write (string text)
    {
        unsafe
        {
            nint pText = Marshal.AllocHGlobal (text.Length * sizeof (char));
    ...
    private CONSOLE_MODE ConsoleMode
    {
        get
        {
...
internal class ANSIDriver : ConsoleDriver
{
    ...
    private AnsiMainLoopDriver _mainLoopDriver;
    ...
    internal override MainLoop Init ()
    {
        ...
        if (!RunningUnitTests)
        {
            try
            {
                ...
                if (_isWindowsTerminal)
                {
                    WinConsole.Write (CSI_SaveCursorAndActivateAltBufferNoBackscroll);
                }
            }
    ...
    public override void UpdateScreen ()
    {
        ...
        stringBuilder.Append (CSI_SetCursorPosition (1, 1));
        ...
        int nextCol = -1; // The net column we can write to. -1 if we've not processed anything yet.

        for (int row = top; row < rows; row++)
        {
            stringBuilder.Clear ();
            stringBuilder.Append (CSI_SetCursorPosition (row + 1, 1));
            ...
            for (int col = left; col < cols; col++)
            {
                ...
                for (; col < cols; col++)
                {
                    ...
                    if (attr != redrawAttr)
                    {
                        ...
                        if (Force16Colors)
                        {
                            stringBuilder.Append (CSI_SetForegroundColor (attr.Foreground.GetAnsiColorCode ()));
                            stringBuilder.Append (CSI_SetBackgroundColor (attr.Background.GetAnsiColorCode ()));
                        }
                        else
                        {
                            stringBuilder.Append (CSI_SetForegroundColorRGB (attr.Foreground.R, attr.Foreground.G, attr.Foreground.B));
                            stringBuilder.Append (CSI_SetBackgroundColorRGB (attr.Background.R, attr.Background.G, attr.Background.B));
                        }
                    }
            ...
            if (stringBuilder.Length > 0)
            {
                stringBuilder.Insert (0, CSI_SetCursorPosition (row + 1, nextCol + 1));
                WinConsole.Write (stringBuilder.ToString ());
            }
        }

        stringBuilder.Append (CSI_SetCursorPosition (Row + 1, Col + 1));
    ...
    internal void ProcessInput (INPUT_RECORD inputEvent)
    {
        switch (inputEvent.EventType)
        {
            case (ushort)INPUT_RECORD_EVENT_TYPE.KEY_EVENT:
                ConsoleKeyInfoEx keyInfo = ToConsoleKeyInfoEx (inputEvent.Event.KeyEvent);
            ...
            case (ushort)INPUT_RECORD_EVENT_TYPE.MOUSE_EVENT:
                MouseEvent me = ToDriverMouse (inputEvent.Event.MouseEvent);
                OnMouseEvent (me);
            ...
            case (ushort)INPUT_RECORD_EVENT_TYPE.WINDOW_BUFFER_SIZE_EVENT:
                ...
                ResizeScreen ();
                ClearContents ();
                OnSizeChanged (new (new Size (Cols, Rows)));
    ...
    private MOUSE_BUTTON_STATE? _lastMouseButtonPressed;
    ...
    private MouseEvent ToDriverMouse (MOUSE_EVENT_RECORD mouseEvent)
    {
        ...
        if ((mouseEvent.dwButtonState != 0 && mouseEvent.dwEventFlags == 0 && _lastMouseButtonPressed == null && !_isButtonDoubleClicked)
            || (_lastMouseButtonPressed == null
                && ((MOUSE_EVENT_FLAGS)mouseEvent.dwEventFlags).HasFlag (MOUSE_EVENT_FLAGS.MOUSE_MOVED)
                && mouseEvent.dwButtonState != 0
                && !_isButtonReleased
        ...
        else if (mouseEvent.dwEventFlags == (uint)MOUSE_EVENT_FLAGS.MOUSE_WHEELED && mouseEvent.dwControlKeyState == (uint)CONTROL_KEY_STATE.SHIFT_PRESSED)
        {
            switch ((int)mouseEvent.dwButtonState)
            {
                case int v when v > 0:
        ...
        else if (mouseEvent.dwEventFlags == (uint)MOUSE_EVENT_FLAGS.MOUSE_MOVED)
        {
            ...
            {
                _pointMove = new (mouseEvent.dwMousePosition.X, mouseEvent.dwMousePosition.Y);
            }
        }
        ...
        return new ()
        {
            Position = new Point (mouseEvent.dwMousePosition.X, mouseEvent.dwMousePosition.Y),
            Flags = mouseFlag
        };
    }

    private MouseFlags ProcessButtonClick (MOUSE_EVENT_RECORD mouseEvent)
    {
    ...
    private static MouseFlags SetControlKeyStates (MOUSE_EVENT_RECORD mouseEvent, MouseFlags mouseFlag)
    {
    ...
    public ConsoleKeyInfoEx ToConsoleKeyInfoEx (KEY_EVENT_RECORD keyEvent)
    {
        ...
        bool shift = (state & CONTROL_KEY_STATE.SHIFT_PRESSED) != 0;
        bool alt = (state & (CONTROL_KEY_STATE.LEFT_ALT_PRESSED | CONTROL_KEY_STATE.RIGHT_ALT_PRESSED)) != 0;
        bool control = (state & (CONTROL_KEY_STATE.LEFT_CTRL_PRESSED | CONTROL_KEY_STATE.RIGHT_CTRL_PRESSED)) != 0;
        bool capsLock = (state & CONTROL_KEY_STATE.CAPSLOCK_ON) != 0;
        bool numLock = (state & CONTROL_KEY_STATE.NUMLOCK_ON) != 0;
        bool scrollLock = (state & CONTROL_KEY_STATE.SCROLLLOCK_ON) != 0;

        var consoleKeyInfo = new ConsoleKeyInfo (keyEvent.uChar.UnicodeChar, (ConsoleKey)keyEvent.wVirtualKeyCode, shift, alt, control);

        return new (consoleKeyInfo, capsLock, numLock, scrollLock);
    }

    private KeyCode MapKey (ConsoleKeyInfoEx keyInfoEx)
    {
    ...
    public override void UpdateCursor ()
    {
        ...
        bool result = SetCursorVisibility (_cachedCursorVisibility);
        Debug.Assert (result);
    }
    ...
    public override void SendKeys (char keyChar, ConsoleKey key, bool shift, bool alt, bool control)
    {
        ...
        var controlKey = new CONTROL_KEY_STATE ();
    ...
    internal override void End ()
    {
        ...
        if (!RunningUnitTests && _isWindowsTerminal)
        {
            ...
            WinConsole?.Write (CSI_RestoreCursorAndRestoreAltBufferWithBackscroll);
        }
...
internal class AnsiMainLoopDriver : IMainLoopDriver
{
    ...
    private readonly WindowsANSIConsole _winConsole;
    private CancellationTokenSource _eventReadyTokenSource = new ();
    private readonly CancellationTokenSource _inputHandlerTokenSource = new ();

    // The records that we keep fetching
    private readonly Queue<INPUT_RECORD []> _resultQueue = new ();
