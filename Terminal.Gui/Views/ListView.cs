...
///                 <see cref="string" /> values are converted into <see cref="string" /> values before rendering, and
///                 other values are
...
public class ListView : View {
	bool _allowsMarking;
	bool _allowsMultipleSelection = true;
	...
	/// <summary>
	///         Gets or sets the <see cref="IListDataSource" /> backing this <see cref="ListView" />, enabling custom
	///         rendering.
	/// </summary>
	...
	public override void OnDrawContent (Rect contentArea)
	{
		...
		for (var row = 0; row < f.Height; row++, item++) {
			...
			if (_source == null || item >= _source.Count) {
				...
				if (_allowsMarking) {
					Driver.AddRune (_source.IsMarked (item)
						?
