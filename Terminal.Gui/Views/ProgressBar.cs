...
public class ProgressBar : View
{
    ...
    public override void OnDrawContent (Rectangle contentArea)
    {
        ...
        if (_isActivity)
        {
            for (var i = 0; i < ContentArea.Width; i++)
            {
        ...
        {
            var mid = (int)(_fraction * ContentArea.Width);
            int i;

            for (i = 0; (i < mid) & (i < ContentArea.Width); i++)
            {
                Driver.AddRune (SegmentCharacter);
            }

            for (; i < ContentArea.Width; i++)
            {
                Driver.AddRune ((Rune)' ');
            }
        }
    ...
    public void Pulse ()
    {
        ...
        else
        {
            ...
            else if (_activityPos [0] >= ContentArea.Width)
            {
                if (_bidirectionalMarquee)
                {
                    for (var i = 0; i < _activityPos.Length; i++)
                    {
                        _activityPos [i] = ContentArea.Width + i - 2;
                    }
