...
public class RadioGroup : View {
	...
	public override bool? OnInvokingKeyBindings (Key keyEvent)
	{
		...
		if (KeyBindings.TryGet (key, out _)) {
			...
			for (var i = 0; i < _radioLabels.Count; i++) {
				if (TextFormatter.FindHotKey (_radioLabels [i], HotKeySpecifier, out _, out var hotKey,
					    true)
			...
			if (hotPos != -1 && hotKey != KeyCode.Null) {
				...
				for (var j = 0; j < rlRunes.Length; j++) {
					...
					if (j == hotPos && i == _cursor) {
						Application.Driver.SetAttribute (HasFocus
	...
	public override bool MouseEvent (MouseEvent me)
	{
		...
		var rCount = _orientation == Orientation.Horizontal
			? _horizontal.Last ().pos + _horizontal.Last ().length
			: _radioLabels.Count;

		if (pos < rCount) {
			var c = _orientation == Orientation.Horizontal
