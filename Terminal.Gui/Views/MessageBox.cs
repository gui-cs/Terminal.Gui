...
namespace Terminal.Gui {
	...
	public static class MessageBox {
		...
		{
			int defaultWidth = 50;
			if (defaultWidth > Application.Driver.Cols / 2) defaultWidth = (int)(Application.Driver.Cols * 0.60f);
			int maxWidthLine = TextFormatter.MaxWidthLine (message);
			if (wrapMessagge && maxWidthLine > Application.Driver.Cols) maxWidthLine = Application.Driver.Cols;
			if (width == 0) maxWidthLine = Math.Max (maxWidthLine, defaultWidth);
			else maxWidthLine = width;
			...
			Dialog d;
			if (width == 0 & height == 0) d = new Dialog (title, border, buttonList.ToArray ()) {
				Height = msgboxHeight
			};
			else d = new Dialog (title, width, Math.Max (height, 4), border, buttonList.ToArray ());
			...
			if (width == 0 & height == 0) {
				...
				if (wrapMessagge) d.Width = Math.Min (dWidth, Application.Driver.Cols);
				else d.Width = dWidth;
			}
