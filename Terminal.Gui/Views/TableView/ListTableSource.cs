...
public class ListTableSource : ITableSource
{
    ...
    private int CalculateColumns ()
    {
        ...
        {
            float f = (float)_tableView.ContentArea.Height - _tableView.GetHeaderHeight ();
            cols = (int)Math.Ceiling (Count / f);
        }
        else
        {
            cols = (int)Math.Ceiling (((float)_tableView.ContentArea.Width - 1) / colWidth) - 2;
        }
    ...
    {
        if (!_tableView.ContentArea.Equals (_lastBounds)
        ...
        _lastBounds = _tableView.ContentArea;
