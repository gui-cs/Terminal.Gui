...
public class Slider<T> : View {
    ...
    /// <summary>
    ///     Calculates the spacing configuration (start, inner, end) as well as turning on/off legend abbreviation if needed.
    ///     Behaves differently based on <see cref="AutoSize"/> and <see cref="View.IsInitialized"/> .
    /// </summary>
        ...
        for (var i = 0; i < _options.Count; i++) {
            ...
            switch (_config._type) {
                ...
                case SliderType.Range:
                    if (isSet && ((i >= _setOptions[0] && i <= _setOptions[1])
                                  || (i >= _setOptions[1] && i <= _setOptions[0]))) {
                        isOptionSet = true;
                    }
    ...
    public override bool MouseEvent (MouseEvent mouseEvent) {
        ...
        if (_dragPosition.HasValue && mouseEvent.Flags.HasFlag (MouseFlags.ReportMousePosition)
                                   && mouseEvent.Flags.HasFlag (MouseFlags.Button1Pressed)) {
            // Continue Drag
            _dragPosition = new Point (mouseEvent.X, mouseEvent.Y);
            _moveRenderPosition = ClampMovePosition ((Point)_dragPosition);
    ...
    private Dictionary<int, SliderOption<T>> GetSetOptionDictionary () {
    ...
    internal bool ExtendPlus () {
        ...
        if (next != FocusedOption && !OnOptionFocused (
                next,
    ...
    internal bool ExtendMinus () {
        ...
        if (prev != FocusedOption && !OnOptionFocused (
                prev,
                new SliderEventArgs<T> (
                    GetSetOptionDictionary (),
                    FocusedOption
