...
#if TREEVIEW_ENABLE_EXPAND_PARENTS
// This is disabled as it's a POC/Prototype
    /// <summary>
    ///     Expands the parent nodes of the specified object if it is contained in the tree.
    /// </summary>
    /// <param name="toReveal">The object to reveal by expanding its parent nodes.</param>
    /// <param name="isMatch">A function to determine if a node matches the target object.</param>
    /// <param name="matchedObject">The object that was matched, if any.</param>
    /// <returns>True if the parent nodes were successfully expanded; otherwise, false.</returns>
    public bool ExpandParents (T? toReveal, Func<T, T, bool> isMatch, out T? matchedObject)
    {
        matchedObject = null;

        if (toReveal is null || Roots is null)
        {
            return false;
        }
        ...
        {
            if (!TryExpandParents (root, toReveal, isMatch, out matchedObject))
            {
                continue;
            }
    ...
    private bool TryExpandParents (Branch<T> current, T target, Func<T, T, bool> isMatch, out T? matchedObject)
    {
        matchedObject = null;

        if (isMatch (current.Model, target))
        {
            matchedObject = current.Model;

            return true;
        }

        if (current.ChildBranches is null)
        {
            if (TreeBuilder is null || current.Depth >= MaxDepth)
            {
                current.ChildBranches = [];
            }
            else
            {
                IEnumerable<T> children = TreeBuilder.GetChildren (current.Model);
                current.ChildBranches = children.Select (o => new Branch<T> (this, current, o)).ToList ();
            }
        }

        foreach (Branch<T> child in current.ChildBranches)
        {
            if (!TryExpandParents (child, target, isMatch, out matchedObject))
            {
                continue;
            }
