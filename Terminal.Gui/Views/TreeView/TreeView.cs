	...
	public class TreeView<T> : View, ITreeView where T : class {
		...
		public T SelectedObject {
			...
			set {
				...
				if (!ReferenceEquals (oldValue, value)) OnSelectionChanged (new SelectionChangedEventArgs<T> (this, oldValue, value));
			}
		}
		...
		public CursorVisibility DesiredCursorVisibility {
			...
			set {
				if (desiredCursorVisibility != value) {
					...
					if (HasFocus) Application.Driver.SetCursorVisibility (DesiredCursorVisibility);
				}
			}
		}
		...
		public void AddObjects (IEnumerable<T> collection)
		{
			...
			foreach (var o in collection) if (!roots.ContainsKey (o)) {
					roots.Add (o, new Branch<T> (this, null, o));
					objectsAdded = true;
				}
		...
		public override void Redraw (Rect bounds)
		{
			...
			for (int line = 0; line < bounds.Height; line++) {
				...
				if (idxToRender < map.Count)                                    // Render the line
		...
		{
			var map = BuildLineMap ();
			for (int i = 0; i < map.Count; i++) if (map.ElementAt (i).Model.Equals (o)) return i;
		...
		private IEnumerable<Branch<T>> AddToLineMap (Branch<T> currentBranch)
		{
			...
			if (currentBranch.IsExpanded) foreach (var subBranch in currentBranch.ChildBranches.Values) foreach (var sub in AddToLineMap (subBranch)) yield return sub;
		}
		...
		public override void PositionCursor ()
		{
			if (CanFocus && HasFocus && Visible && SelectedObject != null) {
				...
				if (idx - ScrollOffsetVertical >= 0 && idx - ScrollOffsetVertical < Bounds.Height) Move (0, idx - ScrollOffsetVertical);
				else base.PositionCursor ();
		...
		protected virtual void CursorLeft (bool ctrl)
		{
			if (IsExpanded (SelectedObject))
				if (ctrl) CollapseAll (SelectedObject);
				else Collapse (SelectedObject);
			else {
		...
		public void AdjustSelection (int offset, bool expandSelection = false)
		{
			...
			if (SelectedObject == null) SelectedObject = roots.Keys.FirstOrDefault ();
			else {
				var map = BuildLineMap ();

				var idx = map.IndexOf (b => b.Model.Equals (SelectedObject));

				if (idx == -1)                                  // The current selection has disapeared!
					SelectedObject = roots.Keys.FirstOrDefault ();
				else {
					...
					if (expandSelection && MultiSelect) if (multiSelectedRegions.Any ()) {
						...
						} else                                                  // or start a new multi selection region
		...
		private void AdjustSelectionToNext (Func<Branch<T>, bool> predicate)
		{
			...
			for (int idxCur = (idxStart + 1) % map.Count; idxCur != idxStart; idxCur = (idxCur + 1) % map.Count) if (predicate (map.ElementAt (idxCur))) {
		...
		protected void CollapseImpl (T toCollapse, bool all)
		{
			...
			if (all) branch.CollapseAll ();
			else branch.Collapse ();

			if (SelectedObject != null && ObjectToBranch (SelectedObject) == null)                          // If the old selection suddenly became invalid then clear it
				SelectedObject = null;
		...
		public IEnumerable<T> GetAllSelectedObjects ()
		{
			...
			// with mouse click
			if (MultiSelect) foreach (var m in map.Select (b => b.Model).Where (IsSelected)) yield return m;
			else if (SelectedObject != null) yield return SelectedObject;
		}
