	...
	class Branch<T> where T : class {
		...
		public virtual void Draw (ConsoleDriver driver, ColorScheme colorScheme, int y, int availableWidth)
		{
			...
			if (tree.Style.ColorExpandSymbol || tree.Style.InvertExpandSymbolColors) {
				...
				if (tree.Style.ColorExpandSymbol) 					if (isSelected) 						color = tree.Style.HighlightModelTextOnly ? colorScheme.HotNormal : tree.HasFocus ? tree.ColorScheme.HotFocus : tree.ColorScheme.HotNormal;
else 						color = tree.ColorScheme.HotNormal;
else 					color = symbolColor;

				if (tree.Style.InvertExpandSymbolColors) 					color = new Attribute (color.Background, color.Foreground);
			...
			if (toSkip > 0) 				if (toSkip > lineBody.Length) 					lineBody = "";
else 					lineBody = lineBody.Substring (toSkip);
		...
		private IEnumerable<Rune> GetLinePrefix (ConsoleDriver driver)
		{
			...
			if (!tree.Style.ShowBranchLines) {
				for (int i = 0; i < Depth; i++) 					yield return new Rune (' ');
			...
			foreach (var cur in GetParentBranches ().Reverse ()) {
				if (cur.IsLast ()) 					yield return new Rune (' ');
else 					yield return driver.VLine;

				yield return new Rune (' ');
			}

			if (IsLast ()) 				yield return driver.LLCorner;
else 				yield return driver.LeftTee;
		}
		...
		public void Refresh (bool startAtTop)
		{
			...
			if (ChildBranches == null) 				FetchChildren ();
				...
				foreach (var newChild in newChildren) 					// If we don't know about the child yet we need a new branch
					if (!ChildBranches.ContainsKey (newChild)) 						ChildBranches.Add (newChild, new Branch<T> (tree, this, newChild));
		...
		internal void Rebuild ()
		{
			...
			if (ChildBranches != null) 				if (IsExpanded) 					//if we are expanded we need to updatethe visible children
					foreach (var child in ChildBranches) 						child.Value.Rebuild ();
		...
		internal bool IsHitOnExpandableSymbol (ConsoleDriver driver, int x)
		{
			...
			if (!IsExpanded && tree.Style.ExpandableSymbol != null) 				return x == GetLinePrefix (driver).Count ();

			// if we could theoretically collapse
			if (IsExpanded && tree.Style.CollapseableSymbol != null) 				return x == GetLinePrefix (driver).Count ();
		...
		internal void ExpandAll ()
		{
			...
			if (ChildBranches != null) 				foreach (var child in ChildBranches) 					child.Value.ExpandAll ();
		}
		...
		internal void CollapseAll ()
		{
			...
			if (ChildBranches != null) 				foreach (var child in ChildBranches) 					child.Value.CollapseAll ();
		}
	}
}