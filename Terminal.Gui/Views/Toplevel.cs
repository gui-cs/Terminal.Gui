...
namespace Terminal.Gui {
	...
	///     been called (which sets the <see cref="Running"/> property to <c>false</c>). 
	///   </para>
	///   <para>
	...
	public class Toplevel : View {
		...
		{
			if (IsMdiContainer) 				SetSubViewNeedsDisplay ();
		...
		{
			IsLoaded = true;
			foreach (Toplevel tl in Subviews.Where (v => v is Toplevel)) 				tl.OnLoaded ();
		...
		{
			foreach (Toplevel tl in Subviews.Where (v => v is Toplevel)) 				tl.OnReady ();
		...
		{
			foreach (Toplevel tl in Subviews.Where (v => v is Toplevel)) 				tl.OnUnloaded ();
		...
		{
			if (Application.MouseGrabView == this && dragPosition.HasValue) 				e.Cancel = true;
		}
		...
		private void MovePreviousView ()
		{
			...
			} else 				FocusNearestView (SuperView?.TabIndexes?.Reverse (), Direction.Backward);
		}
		...
		private void MoveNextView ()
		{
			...
			if (old != Focused && old != Focused?.Focused) {
				old?.SetNeedsDisplay ();
				Focused?.SetNeedsDisplay ();
			} else 				FocusNearestView (SuperView?.TabIndexes, Direction.Forward);
		}

		private void QuitToplevel ()
		{
			if (Application.MdiTop != null) 				Application.MdiTop.RequestStop ();
		...
		View GetDeepestFocusedSubview (View view)
		{
			...
			foreach (var v in view.Subviews) 				if (v.HasFocus) 					return GetDeepestFocusedSubview (v);
			return view;
		}
		...
		void FocusNearestView (IEnumerable<View> views, Direction direction)
		{
			...
			foreach (var v in views) {
				...
				if (found && v != this) {
					if (direction == Direction.Forward) 						SuperView?.FocusNext ();
else 						SuperView?.FocusPrev ();
					focusProcessed = true;
					if (SuperView.Focused != null && SuperView.Focused != this) 						return;
				} else if (found && !focusProcessed && idx == views.Count () - 1) 					views.ToList () [0].SetFocus ();
				idx++;
			}
		}
			...
			out int nx, out int ny, out MenuBar mb, out StatusBar sb)
		{
			...
			if (top.Frame.Width <= l) {
				...
				if (nx + mfLength > top.Frame.X + top.Frame.Width) 					nx = Math.Max (top.Frame.Right - mfLength, 0);
			} else 				nx = x;
			...
			if (top?.SuperView == null || top == Application.Top || top?.SuperView == Application.Top) {
				...
				var t = top.SuperView;
				while (t is not Toplevel) 					t = t.SuperView;
				m = ((Toplevel)t).MenuBar?.Visible == true;
				mb = ((Toplevel)t).MenuBar;
			}
			if (top?.SuperView == null || top == Application.Top || top?.SuperView == Application.Top) 				l = m ? 1 : 0;
else 				l = 0;
			ny = Math.Max (y, l);
			if (top?.SuperView == null || top == Application.Top || top?.SuperView == Application.Top) {
			...
			if (top?.SuperView == null || top == Application.Top || top?.SuperView == Application.Top) 				l = s ? Driver.Rows - 1 : Driver.Rows;
else 				l = s ? top.SuperView.Frame.Height - 1 : top.SuperView.Frame.Height;
			ny = Math.Min (ny, l);
			if (top.Frame.Height <= l) {
				ny = ny + top.Frame.Height >= l ? Math.Max (l - top.Frame.Height, m ? 1 : 0) : ny;
				if (ny + mfLength > top.Frame.Y + top.Frame.Height) 					ny = Math.Max (top.Frame.Bottom - mfLength, 0);
			}
		...
		internal void PositionToplevels ()
		{
			...
			foreach (var top in Subviews) 				if (top is Toplevel) 					PositionToplevel ((Toplevel)top);
		}
		...
		public override void Redraw (Rect bounds)
		{
			...
			if (!NeedDisplay.IsEmpty || ChildNeedsDisplay || LayoutNeeded) {
				...
				if (this == Application.MdiTop) 					foreach (var top in Application.MdiChildes.AsEnumerable ().Reverse ()) 						if (top.Frame.IntersectsWith (bounds)) 							if (top != this && !top.IsCurrentTop && !OutsideTopFrame (top) && top.Visible) {
								top.SetNeedsLayout ();
								top.SetNeedsDisplay (top.Bounds);
								top.Redraw (top.Bounds);
							}

				foreach (var view in Subviews) 					if (view.Frame.IntersectsWith (bounds) && !OutsideTopFrame (this)) {
						view.SetNeedsLayout ();
		...
		public override bool MouseEvent (MouseEvent mouseEvent)
		{
			...
			if (!dragPosition.HasValue && (mouseEvent.Flags == MouseFlags.Button1Pressed
				...
				mouseEvent.Flags == MouseFlags.Button3Pressed) 				if (dragPosition.HasValue) {
...
else 						SuperView.SetNeedsDisplay ();
		...
		public virtual void RequestStop ()
		{
			if (IsMdiContainer && Running
				...
				Running = false;
				Application.RequestStop (this);
			} else 				Application.RequestStop (Application.Current);
		}
		...
		public override void PositionCursor ()
		{
			if (!IsMdiContainer) {
				...
				if (Focused == null) {
					EnsureFocus ();
					if (Focused == null) 						Driver.SetCursorVisibility (CursorVisibility.Invisible);
				}
				return;
			}

			if (Focused == null) 				foreach (var top in Application.MdiChildes) 					if (top != this && top.Visible) {
						top.SetFocus ();
						return;
					}
			base.PositionCursor ();
		...
		public View GetTopMdiChild (Type type = null, string [] exclude = null)
		{
			...
			foreach (var top in Application.MdiChildes) {
				if (type != null && top.GetType () == type
					&& exclude?.Contains (top.Data.ToString ()) == false) 					return top;
else if (type != null && top.GetType () != type
					|| exclude?.Contains (top.Data.ToString ()) == true) 					continue;
				return top;
			}
			return null;
		}
	...
	public class ToplevelEqualityComparer : IEqualityComparer<Toplevel> {
		...
		public int GetHashCode (Toplevel obj)
		{
			...
			int hCode = 0;
			if (int.TryParse (obj.Id.ToString (), out int result)) 				hCode = result;
