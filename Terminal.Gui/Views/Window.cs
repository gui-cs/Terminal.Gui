	...
	public partial class Window : Toplevel {
		...
		public ustring Title {
			...
			set {
				if (!OnTitleChanging (title, value)) {
					var old = title;
					title = value;
					if (Border != null) 						Border.Title = title;
		...
		public override Border Border {
			...
			set {
				...
				Rect frame;
				if (contentView != null && (contentView.Width is Dim || contentView.Height is Dim)) 					frame = Rect.Empty;
else 					frame = Frame;
		...
		class ContentView : View {
			...
			{
				if (MostFocused == null && CanFocus && Visible) 					EnsureFocus ();
		...
		void Initialize (ustring title, Rect frame, int padding = 0, Border border = null)
		{
			...
			if (border == null) 				Border = new Border () {
				...
				Border = border;
				if (ustring.IsNullOrEmpty (border.Title)) 					border.Title = title;
			}
			AdjustContentView (frame);
		}
		...
		void AdjustContentView (Rect frame)
		{
			...
			if (frame == Rect.Empty) {
				...
				if (contentView == null) 					contentView = new ContentView (this) {
						X = wp.X,
						Y = wp.Y,
				...
				if (contentView == null) 					contentView = new ContentView (cFrame, this);
else 					contentView.Frame = cFrame;
			}
			if (Subviews?.Count == 0) 				base.Add (contentView);
			if (Border.Child != contentView) 				Border.Child = contentView;
		}
		...
		public override void Remove (View view)
		{
			...
			if (view == contentView) {
				...
				contentView.Dispose ();
				return;
			} else 				contentView.Remove (view);

			if (contentView.InternalSubviews.Count < 1) 				CanFocus = false;
			RemoveMenuStatusBar (view);
			if (view != contentView && Focused == null) 				FocusFirst ();
		}
		...
		public override void OnCanFocusChanged ()
		{
			if (contentView != null) 				contentView.CanFocus = CanFocus;
			base.OnCanFocusChanged ();
		}
