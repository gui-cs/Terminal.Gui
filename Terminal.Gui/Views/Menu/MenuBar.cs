...
///     * Un-shifted hot keys defined for the menu bar items will be invoked, only if the menu they belong to is open (the
///     menu bar item's text is visible).
///     * Alt-shifted hot keys defined for the menu bar items will be invoked, only if the menu they belong to is open (the
///     menu bar item's text is visible).
...
public class MenuBar : View {
    ...
    /// <remarks>
    ///     <para>
    ///     If the user presses any <see cref="MenuItem.HotKey"/>s defined in the <see cref="MenuBarItem"/>s, the menu bar will
    ///     be activated and the sub-menu will be opened.
    ///     </para>
    ///     <para>
    ...
    public override void OnDrawContent (Rect contentArea) {
        ...
        for (int i = 0; i < Menus.Length; i++) {
            ...
            DrawHotString (
                           ...
                           hotColor,
                           normalColor);
            pos += _leftPadding + menu.TitleLength + (menu.Help.GetColumns () > 0
                ...
                if (cx >= pos && cx < pos + _leftPadding + Menus[i].TitleLength + Menus[i].Help.GetColumns ()
                    ...
                    if (me.Flags == MouseFlags.Button1Clicked) {
                        ...
                        if (!UseSubMenusSingleFrame || UseSubMenusSingleFrame && openCurrentMenu != null
                                                                              ...
                                                                              != null
                                                                              && openCurrentMenu._barItems.Parent.Parent
                                                                              != Menus[i]) {
                            Activate (i);
                        }
                    }
