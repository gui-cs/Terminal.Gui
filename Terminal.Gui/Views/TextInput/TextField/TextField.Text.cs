...
public partial class TextField
{
    ...
    protected override bool OnPaste (string text)
    {
        ...
        var oldTextString = StringExtensions.ToString (oldText);
        List<string> pastedText = text.ToStringList ();

        List<string> proposedText =
        [
            .. oldText.GetRange (0, selStart), .. pastedText, .. oldText.GetRange (selStart + selectedLength, oldText.Count - (selStart + selectedLength))
        ];
    ...
    /// <summary>
    ///     Returns <see langword="true"/> if the current cursor position is at the end of the <see cref="TextField"/>,
    ...
    {
        var costs = new int [proposedText.Count + 1, finalText.Count + 1];

        for (var i = 0; i <= proposedText.Count; i++)
        {
            costs [i, 0] = i;
        }

        for (var j = 0; j <= finalText.Count; j++)
        {
            costs [0, j] = j;
        }

        for (var i = 1; i <= proposedText.Count; i++)
        {
            for (var j = 1; j <= finalText.Count; j++)
            {
        ...
        while (proposedIndex > 0 || finalIndex > 0)
        {
            if (proposedIndex > 0
                ...
                == costs [proposedIndex - 1, finalIndex - 1] + (proposedText [proposedIndex - 1] == finalText [finalIndex - 1] ? 0 : 1))
            {
    ...
    private static void GetActualPastedText (string text,
                                             List<string> proposedText,
                                             int pasteStart,
                                             int pastedLength,
                                             List<string> finalText,
                                             out string actualPastedText,
                                             out int insertionPoint)
    {
        ...
        if (proposedText.Count == finalText.Count)
        {
            ...
            actualPastedText = actualPastedLength > 0 ? StringExtensions.ToString (finalText.GetRange (pasteStart, actualPastedLength)) : string.Empty;
    ...
    private static string GetActualPastedTextFromEditOperations (List<string> proposedText,
                                                                 int pasteStart,
                                                                 int pastedLength,
                                                                 List<string> finalText,
                                                                 out int insertionPoint)
    {
        ...
        {
            if (ConsumesFinal (operation) && IsWithinPasteRange (operation, proposedIndex, pasteStart, pasteEnd))
            {
                if (pastedStart == -1)
                {
    ...
    private void UpdateCursor ()
    {
        ...
        TextModel.CursorColumn (_text, InsertionPoint, 0, out List<int> glyphWidths, out _);
        _ = TextModel.GetColumnWidthsBeforeStart (glyphWidths, ScrollOffset, out int _, out int viewportX);
        var colsWidth = 0;
