...
namespace Terminal.Gui {
	...
	/// 
	/// By default, <see cref="Modal"/> is <c>true</c>. In this case launch the Wizard with <c>Application.Run(wizard)</c>. 
	/// 
	/// See <see cref="Modal"/> for more details.
	/// </remarks>
	/// <example>
	/// <code>
	/// using Terminal.Gui;
	...
	public class Wizard : Dialog {
		...
		/// Set <see cref="HelpText"/> to set the help text. If the help text is empty the help pane will not
		/// be shown. 
		/// 
		...
		/// 
		/// Subscribe to the <see cref="View.VisibleChanged"/> event to be notified when the step is active; see also: <see cref="StepChanged"/>.
		/// 
		...
		public class WizardStep : FrameView {
			...
			/// </summary>
			/// <remarks>The Title is only displayed when the <see cref="Wizard"/> is used as a modal pop-up (see <see cref="Modal"/>.</remarks>
			...
			/// <summary>
			/// Sets or gets help text for the <see cref="WizardStep"/>.If <see cref="HelpText"/> is empty
			...
			{
				Title = title; // this.Title holds just the "Wizard Title"; base.Title holds "Wizard Title - Step Title"
				...
				scrollBar.ChangedPosition += (s, e) => {
					...
					if (helpTextView.TopRow != scrollBar.Position) scrollBar.Position = helpTextView.TopRow;
					helpTextView.SetNeedsDisplay ();
				};

				scrollBar.OtherScrollBarView.ChangedPosition += (s, e) => {
					helpTextView.LeftColumn = scrollBar.OtherScrollBarView.Position;
					if (helpTextView.LeftColumn != scrollBar.OtherScrollBarView.Position) scrollBar.OtherScrollBarView.Position = helpTextView.LeftColumn;
					helpTextView.SetNeedsDisplay ();
				};

				scrollBar.VisibleChanged += (s, e) => {
					if (scrollBar.Visible && helpTextView.RightOffset == 0) helpTextView.RightOffset = 1;
					else if (!scrollBar.Visible && helpTextView.RightOffset == 1) helpTextView.RightOffset = 0;
				};

				scrollBar.OtherScrollBarView.VisibleChanged += (s, e) => {
					if (scrollBar.OtherScrollBarView.Visible && helpTextView.BottomOffset == 0) helpTextView.BottomOffset = 1;
					else if (!scrollBar.OtherScrollBarView.Visible && helpTextView.BottomOffset == 1) helpTextView.BottomOffset = 0;
				};
			...
			internal void ShowHide ()
			{
				...
				if (contentView.InternalSubviews?.Count > 0) if (helpTextView.Text.Length > 0) {
						contentView.Width = Dim.Percent (70);
						helpTextView.X = Pos.Right (contentView);
						helpTextView.Width = Dim.Fill ();

					} else contentView.Width = Dim.Percent (100);
				else if (helpTextView.Text.Length > 0) helpTextView.X = 0;
				else {
					// Error - no pane shown
				}
		...
		private void NextfinishBtn_Clicked (object sender, EventArgs e)
		{
			if (CurrentStep == GetLastStep ()) {
				...
				if (!args.Cancel) {
					...
					if (IsCurrentTop) Application.RequestStop (this);
					else {
		...
		/// to instead fire the <see cref="Cancelled"/> event when Wizard is being used as a non-modal (see <see cref="Modal"/>.
		...
		public void GoNext ()
		{
			...
			if (nextStep != null) GoToStep (nextStep);
		}
		...
		public WizardStep GetNextStep ()
		{
			...
			if (CurrentStep == null)                                // Get first step, assume it is next
				step = steps.First;
			else {
			...
			while (step != null) {
				if (step.Value.Enabled) return step.Value;
				step = step.Next;
			}
			return null;
		}
		...
		{
			LinkedListNode<WizardStep> step = null;
			if (CurrentStep == null)                                // Get last step, assume it is previous
				step = steps.Last;
			else {
				// Get the step before current
				step = steps.Find (CurrentStep);
				if (step != null) step = step.Previous;
			}
		...
		/// </summary>
		/// <remarks>
		/// The Title is only displayed when the <see cref="Wizard"/> <see cref="Modal"/> is set to <c>false</c>.
		/// </remarks>
		public new ustring Title {
			get {
		...
		/// To prevent a modal (<see cref="Modal"/> is <c>true</c>) Wizard from
		...
		{
			if (OnStepChanging (currentStep, newStep) || newStep != null && !newStep.Enabled) return false;
			...
			if (backBtn.HasFocus) backBtn.SetFocus ();
			else nextfinishBtn.SetFocus ();

			if (OnStepChanged (oldStep, currentStep))                               // For correctness we do this, but it's meaningless because there's nothing to cancel
				return false;
		...
		private void UpdateButtonsAndTitle ()
		{
			...
			if (CurrentStep == GetLastStep ()) nextfinishBtn.Text = CurrentStep.NextButtonText != ustring.Empty ? CurrentStep.NextButtonText : Strings.wzFinish; // "Fi_nish";
			else nextfinishBtn.Text = CurrentStep.NextButtonText != ustring.Empty ? CurrentStep.NextButtonText : Strings.wzNext; // "_Next...";
		...
		public new bool Modal {
			...
			set {
				base.Modal = value;
				foreach (var step in steps) SizeStep (step);
				if (base.Modal) {
					...
					if (SuperView != null) ColorScheme = SuperView.ColorScheme;
					else ColorScheme = Colors.Base;
					CanFocus = true;
