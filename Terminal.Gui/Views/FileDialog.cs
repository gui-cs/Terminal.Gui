...
namespace Terminal.Gui {
	internal class DirListView : View {
		...
		void UnMarkAll ()
		{
			for (int i = 0; i < infos.Count; i++) if (infos [i].Item3) infos [i] = (infos [i].Item1, infos [i].Item2, false);
		}
		...
		{
			if (allowsMultipleSelection) if (FilePaths.Count > 0) FileChanged?.Invoke (string.Join (splitString, GetFilesName (FilePaths)));
				else FileChanged?.Invoke (infos [selected].Item2 && !canChooseDirectories ? "" : Path.GetFileName (infos [selected].Item1));
			else {
		...
		public bool GetValidFilesName (string files, out string result)
		{
			...
			foreach (var file in filesList) {
				...
				if (idx > -1 && string.Equals (infos [idx].Item1, file, StringComparison.OrdinalIgnoreCase)) {
					...
					if (allowsMultipleSelection && !infos [idx].Item3) infos [idx] = (infos [idx].Item1, infos [idx].Item2, true);
		...
		void PerformMultipleSelection (int? firstSelected = null)
		{
			if (allowsMultipleSelection) {
				...
				for (int i = first; i <= last; i++) if (canChooseFiles && infos [i].Item2 == false ||
					    canChooseDirectories && infos [i].Item2 &&
					     infos [i].Item1 != "..") infos [i] = (infos [i].Item1, infos [i].Item2, !infos [i].Item3);
				OnSelectionChanged ();
				SetNeedsDisplay ();
			}
		}
		...
		public IReadOnlyList<string> FilePaths {
			get {
				if (allowsMultipleSelection) {
					...
					foreach (var item in infos) if (item.Item3)
							res.Add (MakePath (item.Item1));
					if (res.Count == 0 && infos.Count > 0 && infos [selected].Item1 != "..") res.Add (MakePath (infos [selected].Item1));
					return res;
				} else {
	...
	public class FileDialog : Dialog {
			...
			List<string> allowedTypes = null) : base (title)//, Driver.Cols - 20, Driver.Rows - 5, null)
		{
			...
			nameEntry.TextChanged += (s, e) => {
				if (nameEntry.Text.IsEmpty) this.prompt.Enabled = false;
				else this.prompt.Enabled = true;
			};
