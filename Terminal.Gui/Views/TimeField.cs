...
namespace Terminal.Gui {
	...
	public class TimeField : TextField {
		...
		bool SetText(ustring text) {
			if( !DateTime.TryParseExact(text.ToString(), Format, CultureInfo.CurrentCulture, DateTimeStyles.None, out DateTime result) ) return false;
			Text = text;
			return true;
		}

		void IncCursorPosition() {
			if( CursorPosition==FieldLen ) return;
			if(Text[++CursorPosition]==':') CursorPosition++;
		}
