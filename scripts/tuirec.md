...
2. Launch the target app/scenario with a dedicated entry point.
   - Prefer a scenario runner or focused runner over `dotnet run` of a large app shell.
   - This reduces timing and startup variability in captures.
...
- `tuirec` CLI options can change over time. Run `tuirec --help` and follow the current command syntax.
