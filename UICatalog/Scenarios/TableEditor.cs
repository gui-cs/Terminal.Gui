...
public class TableEditor : Scenario {
	...
	readonly List<IDisposable> _toDispose = new ();
	...
	ColorScheme _redColorScheme;
	ColorScheme _redColorSchemeAlt;
	...
	public override void Setup ()
	{
		...
		_tableView.SelectedCellChanged += (s, e) => {
			selectedCellLabel.Text = $"{_tableView.SelectedRow},{_tableView.SelectedColumn}";
		};
		...
		_tableView.MouseClick += (s, e) => {
			...
			_tableView.ScreenToCell (e.MouseEvent.X, e.MouseEvent.Y, out var clickedCol);
	...
	void ShowAllColumns ()
	{
		foreach (var colStyle in _tableView.Style.ColumnStyles) {
	...
	void SortColumn (int clickedCol, string sort, bool isAsc)
	{
		...
		var sortedCopy = _currentTable.DefaultView.ToTable ();
	...
	string TrimArrows (string columnName) =>
		columnName.TrimEnd ((char)CM.Glyphs.UpArrow.Value, (char)CM.Glyphs.DownArrow.Value);

	string StripArrows (string columnName) =>
	...
	string GetProposedNewSortOrder (int clickedCol, out bool isAsc)
	{
		...
		var sort = _currentTable.DefaultView.Sort;
		var colName = _tableView.Table.ColumnNames [clickedCol];
	...
	void ShowHeaderContextMenu (int clickedCol, MouseEventEventArgs e)
	{
		...
		var contextMenu = new ContextMenu {
			...
			MenuItems = new MenuBarItem ([
				...
				new MenuItem ($"Sort {StripArrows (sort)}", "",
					() => SortColumn (clickedCol, sort, isAsc))
			])
		};
	...
	{
		var col = GetColumn ();
		RunColumnWidthDialog (col, "MinAcceptableWidth", (s, v) => s.MinAcceptableWidth = v,
	...
	void RunColumnWidthDialog (int? col, string prompt, Action<ColumnStyle, int> setter,
	...
	void TableViewKeyPress (object sender, Key e)
	{
		...
		if (e.KeyCode == KeyCode.Delete) {
			if (_tableView.FullRowSelect) {
				...
				foreach (var toRemove in _tableView.GetAllSelectedCells ().Select (p => p.Y).Distinct ()
						 .OrderByDescending (i => i)) {
					_currentTable.Rows.RemoveAt (toRemove);
				}
			} else {
				// otherwise set all selected cells to null
				foreach (var pt in _tableView.GetAllSelectedCells ()) {
	...
	void ToggleCheckboxes (bool radio)
	{
		...
		if (_tableView.Table is TreeTableSource<FileSystemInfo> treeSource) {
			source = new CheckBoxTableSourceWrapperByObject<FileSystemInfo> (_tableView, treeSource,
	...
	void ToggleAlternatingColors ()
	{
		...
		if (_miAlternatingColors.Checked == true) {
			_tableView.Style.RowColorGetter = a => {
				return a.RowIndex % 2 == 0 ? _alternatingColorScheme : null;
			};
		} else {
	...
	void CloseExample () => _tableView.Table = null;
	...
	void SetTable (DataTable dataTable) => _tableView.Table = new DataTableSource (_currentTable = dataTable);
	...
	void OpenTreeExample ()
	{
		...
		var source = new TreeTableSource<FileSystemInfo> (_tableView, "Name", tree,
	...
	void SetDemoTableStyles ()
	{
		...
		var negativeRight = new ColumnStyle {
			...
			ColorGetter = a => a.CellValue is double d
				...
				d <= 0.0000001
					? a.RowIndex % 2 == 0 && _miAlternatingColors.Checked == true
						? _redColorSchemeAlt
						: _redColorScheme
					:
	...
	void EditCurrentCell (object sender, CellActivatedEventArgs e)
	{
		...
		var o = _currentTable.Rows [e.Row] [tableCol];
	...
	bool HasCheckboxes () => _tableView.Table is CheckBoxTableSourceWrapperBase;

	string GetUnicodeCategory (uint u) =>
		Ranges.FirstOrDefault (r => u >= r.Start && u <= r.End)?.Category ?? "Unknown";
	...
	public static DataTable BuildDemoDataTable (int cols, int rows)
	{
		...
		for (var i = 0; i < rows; i++) {
			var row = new List<object> {
				...
				DBNull.Value,
				"Les Mise" + Char.ConvertFromUtf32 (Int32.Parse ("0301", NumberStyles.HexNumber)) +
				"rables"
			};
