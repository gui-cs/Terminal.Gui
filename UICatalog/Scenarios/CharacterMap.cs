...
namespace UICatalog.Scenarios {
	...
	public class CharacterMap : Scenario {
		...
		public override void Setup ()
		{
			...
			jumpEdit.TextChanged += (s) => {
				...
				var found = radioItems.First (x => x.end >= result).radioLabel;
				wasFromJumpEdit = true;
				jumpList.SelectedItem = jumpList.Source.ToList ().IndexOf (found);
				jumpList.EnsuresVisibilitySelectedItem ();
			};
		}
	}
	...
	class CharMap : ScrollView {
		...
		public CharMap ()
		{
			...
			AddCommand (Command.Accept, () => {
				MessageBox.Query ("Glyph", $"{new Rune (SelectedGlyph)} U+{SelectedGlyph:x4}", "Ok");
				return true;
			});
	...
	class UnicodeRange {
		...
		public static List<UnicodeRange> Ranges = new List<UnicodeRange> {
			...
			new UnicodeRange (0x24B6, 0x24CF        , "Circled Latin Capital Letters"),
