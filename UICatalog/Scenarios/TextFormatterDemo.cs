...
namespace UICatalog.Scenarios {
    ...
    public class TextFormatterDemo : Scenario {
        public override void Setup () {
            ...
            Label blockText = new Label () {
                                               ColorScheme = Colors.ColorSchemes["TopLevel"], X = 0, Y = 0, Height = 10,
                                               Width = Dim.Fill (0), AutoSize = false
                                           };
            ...
            var label = new Label ($"Demonstrating single-line (should clip):")
                        { Y = Pos.Bottom (unicodeCheckBox) + 1 };
            Win.Add (label);
            foreach (var alignment in alignments) {
