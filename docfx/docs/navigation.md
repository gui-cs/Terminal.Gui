...
At the application level, navigation is encapsulated within the <xref:Terminal.Gui.ApplicationNavigation> helper class which is publicly exposed via the <xref:Terminal.Gui.Application.Navigation> property.

<xref:Terminal.Gui.ApplicationNavigation.GetFocused> gets the most-focused View in the application. Will return `null` if there is no view with focus (an extremely rare situation). This replaces `View.MostFocused` in v1.

The <xref:Terminal.Gui.ApplicationNavigation.FocusedChanged> and <xref:Terminal.Gui.ApplicationNavigation.FocusedChanging> events are raised when the most-focused View in the application is changing or has changed. `FocusedChanged` is useful for apps that want to do something with the most-focused view (e.g. see `AdornmentsEditor`). `FocusChanging` is useful for apps that want to override what view can be focused across an entire app. 

The <xref:Terminal.Gui.ApplicationNavigation.AdvanceFocus> method causes the focus to advance (forward or backwards) to the next View in the application view-hierarchy, using `behavior` as a filter.
...
<xref:Terminal.Gui.View.AdvanceFocus> is the primary method for developers to cause a view to gain or lose focus.

Various events are raised when a View's focus is changing. For example, <xref:Terminal.Gui.View.HasFocusChanging> and <xref:Terminal.Gui.View.HasFocusChanged>.
...
<xref:Terminal.Gui.View.HasFocusChanging> and <xref:Terminal.Gui.View.HasFocusChanged> are raised when a View's focus is changing.
