...
```csharp
// These belong to the session, not the view
public event EventHandler<ToplevelEventArgs>? Activate;
public event EventHandler<ToplevelEventArgs>? Deactivate;

// ToplevelTransitionManager.cs - Hacky workaround
public class ToplevelTransitionManager
{
    ...
    public void RaiseReadyEventIfNeeded (IApplication? app)
    {
        ...
        if (top != null && !_readiedTopLevels.Contains (top))
        {
            ...
            // Track that this toplevel has been "readied"
        }
    }
}
```
...
public virtual void EndInit()
{
    ...
    // CWP Phase 3: Default behavior (initialization work)
    IsInitialized = true;
    // ... initialization work ...

    // CWP Phase 4: Post-notification
    OnInitialized();
    Initialized?.Invoke(this, EventArgs.Empty);
}
```
...
### Phase 0: Rename `Current` to `TopRunnable` (Optional)

- Issue #4399 (modified scope)
...
- **Clear separation of concerns**: Runnable, overlapped, and modal are independent
- **Interface-based design**: Any view can implement `IRunnable`
- **Reduced coupling**: Session management independent of view hierarchy

### 2. Type Safety

- **Generic results**: `IRunnable<TResult>` provides compile-time type safety
- **No casting**: Results are strongly typed
- **No sentinel values**: `null` naturally represents "not accepted" or "canceled"

### 3. Simplified Codebase

- **Fewer lifecycle events**: 4 core events (`IsRunningChanging`, `IsRunningChanged`, `IsModalChanging`, `IsModalChanged`) instead of 10+ scattered Toplevel events
- **Consistent patterns**: All lifecycle follows Cancellable Work Pattern
- **Property-based state**: `IsRunning` and `IsModal` are derived from stack state, eliminating manual synchronization bugs

### 4. Flexibility

- **Any view can be runnable**: Not limited to `Toplevel` derivatives
- **Decoupled from layout**: Can run views without overlapped arrangement
- **Custom typed results**: Define any result type for dialogs (`int`, `string`, enums, custom classes)

### 5. Maintainability

- **Standard patterns**: Follows Terminal.Gui's existing Cancellable Work Pattern
- **Clear lifecycle**: Initialize → Start → Run → Stop
- **Testability**: Interface-based design simplifies testing

---

## Open Questions

1. **Result Lifetime**: Should `IRunnable<TResult>.Result` be cleared when starting a new run?
   - **Answer**: Yes, `Result` is cleared in `RaiseStartingEvent()` (see `Runnable<TResult>` implementation)
2. **Error Handling**: How should initialization/starting failures be communicated?
   - **Answer**: `RaiseStartingEvent()` returns `bool` (true = cancelled). Initialization throws `InvalidOperationException` if already initialized.
3. **RunnableSessionStack Type**: How to handle heterogeneous `IRunnable<TResult>` types in the stack?
   - **Answer**: Create non-generic base `IRunnable` interface with common members, then `IRunnable<TResult> : IRunnable`
   - `RunnableSessionStack` stores `IRunnable`, preserving type safety at usage sites

## Conclusion

The `IRunnable<TResult>` architecture proposal provides a clean, type-safe, and flexible foundation for Terminal.Gui's session management. By:

- **Decoupling** runnable behavior from `Toplevel` and `ViewArrangement.Overlapped`
- **Unifying** all sessions under single `IRunnable<TResult>` interface (no false modal/non-modal distinction)
- **Following** Terminal.Gui's Cancellable Work Pattern consistently
- **Providing** type-safe results with generic `TResult` parameter
- **Offering** three convenient forms of `Run()` for different use cases

We create a more maintainable, testable, and extensible codebase that eliminates unnecessary coupling and complexity while maintaining the power of the current system.
